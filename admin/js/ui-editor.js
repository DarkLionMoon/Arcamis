...
async function openInterfaceUI() {
  ...
  h +=
    '<div class="panel-head"><h3>Barra inferiore (mobile)</h3><span class="hint">Max 5 voci · visibile solo su mobile</span></div>';
  h +=
    '<div class="panel-sub">Trascina o usa le frecce per riordinare. "Pagina wiki…" collega una voce a una pagina del sito.</div>';
  ...
  h +=
    '<div class="fld"><label><input type="checkbox" id="ui-drawer-search"' +
    (_uiCfg.drawerSearch !== false ? ' checked' : '') +
    '> Mostra la ricerca nel menu</label></div>';
  h +=
    '<div class="panel-sub">Le sezioni e le voci del menu si gestiscono da <b>Navigazione</b> (sezioni e assegnazione pagine).</div>';
  h += '</div>';

  /* ── Anteprima sito con toggle dispositivo ── */
  h += '<div class="panel" style="margin-bottom:16px">';
  h +=
    '<div class="panel-head"><h3>Anteprima sito</h3>' +
  ...
  h +=
    '<div class="panel-sub">Caricata dopo il deploy (~30s dal salvataggio). Le modifiche alla barra si vedono a fine deploy.</div>';
  h +=
    '<div style="background:#0b0d16;border:1px solid var(--line);border-radius:10px;padding:10px;overflow:auto;text-align:center">' +
...
function _uiRenderRows() {
  ...
  _uiCfg.bottomNav.forEach(function (item, i) {
    ...
    h +=
      '<div class="kv-row ui-row" data-i="' +
      i +
      ...
      '<input class="in ui-f-icon" style="width:52px;text-align:center;font-size:18px" value="' +
      escAttr(item.icon || '') +
      '" maxlength="4" title="Icona (emoji)" onchange="_uiField(' +
      i +
      ",'icon',this.value)\">" +
      '<input class="in ui-f-label" style="flex:1;min-width:90px" value="' +
      escAttr(item.label || '') +
      '" placeholder="Etichetta" maxlength="12" onchange="_uiField(' +
      i +
      ",'label',this.value)\">" +
      '<select class="in ui-f-action" style="width:170px" onchange="_uiField(' +
      i +
      ",'action',this.value);_uiRenderRows();_uiRenderPreview()\">" +
      _uiActionOptions(item.action) +
      '</select>' +
      (isPage
        ? '<select class="in" style="flex:1;min-width:140px" onchange="_uiField(' +
          i +
          ",'target',this.value)\">" +
          _uiPageOptions(item.target) +
          '</select>'
        : '') +
      (isUrl
        ? '<input class="in" style="flex:1;min-width:140px" placeholder="https://…" value="' +
          escAttr(item.target || '') +
          '" onchange="_uiField(' +
          i +
          ",'target',this.value)\">"
        : '') +
      '<button class="btn btn-soft btn-sm" title="Su" onclick="_uiMove(' +
      i +
      ',-1)">↑</button>' +
      '<button class="btn btn-soft btn-sm" title="Giù" onclick="_uiMove(' +
      i +
      ',1)">↓</button>' +
      '<button class="btn btn-d btn-sm" title="Elimina" onclick="_uiDel(' +
      i +
      ')">✕</button>' +
      '</div>';
  });
...
function _uiRenderPreview() {
  ...
  if (!box) return;
  var cells = _uiCfg.bottomNav
    .map(function (item, i) {
      return (
        '<div style="display:flex;flex-direction:column;align-items:center;gap:2px;padding:6px 0;' +
        (i === 0 ? 'color:#c89b3c;' : 'color:rgba(200,155,60,.45);') +
        '">' +
        '<span style="font-size:17px">' +
        esc(item.icon || '📄') +
        '</span>' +
        '<span style="font-family:Cinzel,serif;font-size:7.5px;letter-spacing:.05em;text-transform:uppercase">' +
        esc(item.label || '') +
        '</span>' +
        '</div>'
      );
    ...
    .join('');
  box.innerHTML =
    '<div style="background:#04050e;border:1px solid rgba(200,155,60,.25);border-radius:10px;display:grid;grid-template-columns:repeat(' +
    Math.max(_uiCfg.bottomNav.length, 1) +
    ',1fr);overflow:hidden">' +
    cells +
    '</div>';
}
...
function _uiAddSlot() {
  if (_uiCfg.bottomNav.length >= 5) {
    toast('Massimo 5 voci', 'error');
    return;
  }
...
function _uiBindDrag() {
  ...
  document.addEventListener('mouseup', function () {
    document.querySelectorAll('#ui-bn-rows .ui-row').forEach(function (r) {
      r.style.opacity = '';
    });
    dragIdx = null;
  });
...
async function _uiSave() {
  if (!_uiCfg || !_uiCfg.bottomNav.length) {
    toast('Aggiungi almeno una voce alla barra', 'error');
    return;
  }
  ...
  for (var i = 0; i < _uiCfg.bottomNav.length; i++) {
    ...
    if (!it.label) {
      it.label = it.action === 'home' ? 'Home' : 'Voce';
    }
  }
  ...
  try {
    ...
    var reg = JSON.parse(b64decode(d.content));
    reg.ui = _uiCfg;
    await ghPut('content/pages/registry.json', 'admin: update interfaccia', JSON.stringify(reg, null, 2) + '\n', d.sha);
    _uiSha = d.sha;
    _uiDirty = false;
