...
var LAYOUT_REGISTRY = {
  ...
  collezione: {
    ...
    blockFields: ['Nome', 'Sottotitolo', 'Categoria'],
    requiredFields: ['Nome'],
    editorMode: 'pantheon',
  },
  ...
  personaggio: {
    ...
    blockFields: ['Nome', 'Razza', 'Classe', 'Livello', 'Allineamento', 'CA', 'PF', 'Velocità'],
    requiredFields: ['Nome'],
    fields: [
  ...
  bestiario: {
    ...
    blockFields: ['Nome', 'Tipo', 'CR', 'Taglia', 'Allineamento'],
    requiredFields: ['Nome', 'CR'],
    fields: [
      { key: 'nome', label: 'Nome', type: 'text', required: true },
      {
        key: 'tipo',
      ...
      },
      {
        ...
        label: 'Taglia',
        type: 'select',
        options: ['Piccolissimo', 'Piccolo', 'Media', 'Grande', 'Enorme', 'Colossale'],
      },
      {
        key: 'allineamento',
        label: 'Allineamento',
  ...
  oggetti: {
    v: 'oggetti',
    l: 'Oggetti & Incantesimi',
    i: '⚔️',
    template:
      "## Nome dell'Oggetto\n\nDescrizione generale.\n\n- **Tipo:** Arma\n- **Rarità:** Raro\n- **Attunamento:** Sì\n- **Fonte:** PHB\n\n### Effetto\n\nDescrizione dell'effetto magico.\n\n### Limitazioni\n\n- Limite 1\n\n### Storia\n\nOrigine e leggenda.",
    blockFields: ['Nome', 'Tipo', 'Rarità/Livello', 'Fonte'],
    requiredFields: ['Nome'],
    fields: [
      { key: 'nome', label: 'Nome', type: 'text', required: true },
      {
      ...
      },
      {
        ...
        label: 'Rarità',
        type: 'select',
        options: ['Comune', 'Non comune', 'Raro', 'Molto raro', 'Leggendario', 'Arcaico'],
      },
  ...
  luoghi: {
    ...
    blockFields: ['Nome', 'Tipo', 'Popolazione', 'Governo'],
    requiredFields: ['Nome'],
    fields: [
      { key: 'nome', label: 'Nome', type: 'text', required: true },
      {
  ...
  cronache: {
    v: 'cronache',
    l: 'Cronache — Sessioni, Quest, Eventi',
    i: '📅',
    template:
      '## Titolo della Cronaca\n\nRiassunto in una o due frasi.\n\n- **Tipo:** Sessione\n- **Data:** 2025-01-15\n- **Luogo:** Gandora\n- **Stato:** Completata\n- **Difficoltà:** Media\n\n### Riassunto\n\nResoconto dettagliato degli eventi.\n\n### Eventi chiave\n\n- Evento 1\n- Evento 2\n\n### Risvolti\n\nConseguenze e sviluppi futuri.',
    blockFields: ['Titolo', 'Tipo', 'Data', 'Luogo', 'Stato'],
    ...
    fields: [
      ...
      { key: 'luogo', label: 'Luogo', type: 'text' },
      {
        ...
        label: 'Stato',
        type: 'select',
        options: ['In corso', 'Completata', 'Fallita', 'Annullata', 'Programmata'],
      },
      {
        key: 'difficolta',
        label: 'Difficoltà',
        type: 'select',
        options: ['Facile', 'Media', 'Difficile', 'Mortale', 'Leggendaria'],
      },
  ...
  contenuto: {
    v: 'contenuto',
    l: 'Contenuto — Testo e regole',
    i: '📄',
    template:
      '## Introduzione\n\nTesto introduttivo.\n\n## Sezione 1\n\nContenuto della sezione.\n\n## Sezione 2\n\n- Elemento 1\n- Elemento 2\n\n> Citazione importante.\n\n---\n\n## Sezione 3\n\nAltro contenuto.',
    blockFields: ['Titolo', 'Sottotitolo'],
    requiredFields: [],
    fields: [
  ...
  fazioni: {
    v: 'fazioni',
    l: 'Fazioni & Organizzazioni',
    i: '🏴',
    template:
      '## Nome della Fazione\n\nDescrizione introduttiva.\n\n- **Simbolo:** Descrizione\n- **Sede:** Luogo principale\n- **Leader:** Nome\n- **Membri:** Numero\n\n### Struttura\n\nCome sono organizzati.\n\n### Obiettivi\n\n- Obiettivo 1\n- Obiettivo 2\n\n### Alleati e Nemici\n\n- **Alleati:** ...\n- **Nemici:** ...\n\n### Storia\n\nCome è nata la fazione.',
    blockFields: ['Nome', 'Simbolo', 'Sede', 'Leader', 'Membri'],
    requiredFields: ['Nome'],
    fields: [
      { key: 'nome', label: 'Nome', type: 'text', required: true },
  ...
  materiale: {
    v: 'materiale',
    l: 'Materiale — Classi, Specie, Talenti',
    i: '📋',
    template:
      '## Specie\n\n- Umano (PHB)\n- Elfo (PHB)\n- Mezzorco (PHB)\n\n## Classi\n\n- Barbarian (PHB)\n- Berserker (PHB)\n- Totem Warrior (PHB)\n\n- Fighter (PHB)\n- Champion (PHB)\n- Battle Master (PHB)\n\n## Talenti\n\n- Talento 1 (PHB)\n- Talento 2 (XGE)\n- Talento 3 (TCE)\n\n## Spell\n\n- Spell 1 (PHB)\n- Spell 2 (XGE)\n- Spell 3 (TCE)\n\n> 💡 Nota su eventuali restrizioni.',
    blockFields: ['Nome', 'Fonte', 'Categoria'],
    requiredFields: [],
    fields: [
...
var LAYOUTS = [{ v: '', l: '(Auto — rileva da chiave)', i: '🔍' }].concat(
  Object.keys(LAYOUT_REGISTRY).map(function (k) {
    return LAYOUT_REGISTRY[k];
  })
);
var LAYOUT_TEMPLATES = (function () {
  var t = {};
  Object.keys(LAYOUT_REGISTRY).forEach(function (k) {
    t[k] = LAYOUT_REGISTRY[k].template;
  });
  return t;
})();
...
var EDITOR_CMDS = {
  ...
  callout: openCalloutDialog,
  details: insertFoldTemplate,
...
var EDITOR_TB_TITLE = {
  ...
  h3: 'Titolo 3',
  quote: 'Citazione',
  callout: 'Callout ([!NOTE], [!LORE], …)',
  details: 'Sezione richiudibile',
  hr: 'Divisore',
  ul: 'Lista',
  ol: 'Lista numerata',
  todo: 'Checklist',
...
function _layoutLabel(v) {
  var l = LAYOUTS.find(function (x) {
    return x.v === v;
  });
  return l ? l.i + ' ' + l.l : 'layout: ' + (v || '—');
}

function runCmd(cmd) {
  if (EDITOR_CMDS[cmd]) EDITOR_CMDS[cmd]();
}

/* ── APERTURA PAGINA ── */
async function openPage(k) {
  _mapActive = false;
  var meta = PAGES.find(function (p) {
    return p.k === k;
  });
  ...
  h += '<div class="ed-head">';
  h +=
    '<div class="ed-title"><span class="eti" id="ed-icon">' +
    esc(json.icon || '📄') +
    '</span><span id="ed-title">' +
    esc(json.title || k) +
    '</span><span class="etp">pages/' +
    esc(k) +
    '</span></div>';
  ...
  h +=
    '<label style="display:flex;align-items:center;gap:4px;font-size:11px;color:var(--dim)">📅 <input type="datetime-local" id="e-publish-at" class="in" style="width:auto;padding:2px 6px;font-size:11px" title="Pubblica automaticamente alla data"></label>';
  h += '<div class="grow"></div>';
  h += '<div class="ed-actions">';
  h +=
    '<button class="btn btn-soft btn-sm" onclick="saveDraft()" title="Salva come bozza (non pubblicata)">💾 <span class="lbl">SALVA BOZZA</span></button>';
  h +=
    '<button class="btn btn-p btn-sm" id="publish-btn" onclick="publishDraft()" style="display:none" title="Pubblica bozza come versione live">🚀 <span class="lbl">PUBBLICA</span></button>';
  h += '<span class="ed-sep"></span>';
  h +=
    '<button class="btn btn-soft btn-sm" onclick="toggleJson()" title="Modifica il JSON grezzo della pagina">JSON</button>';
  h +=
    '<button class="btn btn-soft btn-sm" onclick="openHistory()" title="Cronologia commit e ripristino versione">📜 <span class="lbl">Git Storia</span></button>';
  h +=
    '<button class="btn btn-soft btn-sm" onclick="openKVVersions()" title="Versioni salvate in backup (KV)">💾 <span class="lbl">Backup</span></button>';
  h +=
    '<button class="btn btn-soft btn-sm" onclick="checkLinks()" title="Verifica i link interni del contenuto">🔗 <span class="lbl">Link</span></button>';
  h += '<button class="btn btn-soft btn-sm" onclick="copyPageUrl()" title="Copia URL pulito della pagina">URL</button>';
  h +=
    '<button class="btn btn-soft btn-sm" onclick="openPageOnSite()" title="Apri la pagina sul sito in una nuova scheda">🌐 <span class="lbl">Apri</span></button>';
  h +=
    '<button class="btn btn-soft btn-sm" onclick="clonePageModal()" title="Clona questa pagina come template per una nuova">📋 <span class="lbl">Clona</span></button>';
  h +=
    '<button class="btn btn-soft btn-sm" onclick="openRenameModal()" title="Rinomina sezione / pagina">✏️ <span class="lbl">Rinomina</span></button>';
  h += '<span class="ed-sep"></span>';
  h +=
    '<button class="btn btn-d btn-sm" id="del-btn" onclick="deletePage()" title="Elimina questa pagina (azione irreversibile)">ELIMINA</button>';
  h +=
    '<button class="btn btn-p btn-sm" id="save-btn" onclick="savePage()" title="Salva subito (Ctrl+S)">SALVA</button>';
  h += '</div></div>';
  h += buildToolbar();
  ...
  h += '<div class="ed-meta">';
  h +=
    '<label>Titolo<input id="e-title" class="in mm-t" value="' +
    escAttr(json.title || '') +
    '" oninput="onMetaInput()" placeholder="Titolo pagina"></label>';
  h +=
    '<label>Icona<input id="e-icon" class="in mm-icon" value="' +
    escAttr(json.icon || '') +
    '" oninput="onMetaInput()" maxlength="6" placeholder="📄"></label>';
  h +=
    '<label>Layout<select id="e-layout" class="in mm-layout" onchange="onLayoutChange(this.value)">' +
    layoutOpts +
    '</select></label>';
  h +=
    '<label class="ed-toc-toggle">Sommario<input type="checkbox" id="e-toc" ' +
    (json.toc ? 'checked' : '') +
    ' onchange="_current.toc=this.checked"></label>';
  h +=
    '<label style="grid-column:1/-1">Descrizione SEO (meta description)<input id="e-description" class="in" value="' +
    escAttr(json.description || '') +
    '" oninput="onMetaInput()" placeholder="Descrizione breve per i motori di ricerca (max 160 caratteri)" maxlength="160"></label>';
  h += '</div>';
  if (st) {
    h +=
      '<textarea id="e-md" style="display:none" placeholder="Contenuto Markdown (sincronizzato dai blocchi)...">' +
      esc(json.content || '') +
      '</textarea>';
    ...
    h +=
      '<textarea id="e-md" oninput="onMdInput()" onkeydown="onMdKey(event)" onblur="onMdBlur()" placeholder="Scrivi il contenuto in Markdown...">' +
      esc(json.content || '') +
      '</textarea>';
  }
  ...
  h +=
    '<div class="pane-head">Anteprima <span class="ph-info" id="e-pv-info">' +
    esc(_layoutLabel(json.layout || '')) +
    '</span></div>';
  h += '<div id="e-preview"></div>';
  h += '</div>';
  h +=
    '<div class="md-outline" id="e-outline"><div class="ol-head"><span>☰ Sommario</span><button class="ol-close" onclick="toggleOutline()" title="Chiudi sommario">✕</button></div><div class="ol-body" id="e-ol-body"></div></div>';
  h += '</div>';
  h +=
    '<iframe id="site-frame"></iframe><div id="site-hint"><span>🔎 Anteprima del sito</span><span class="sh-k">· clicca SITO per tornare all\'editor</span></div>';
  h += '<div class="json-pane" id="json-pane"><div class="pane-head">JSON</div><textarea id="e-json"></textarea></div>';
  h +=
    '<div class="ed-statusbar"><span id="e-sb">' +
    esc(_layoutLabel(json.layout || '')) +
    ' · Autosave ogni 5s · SALVA / Ctrl+S salva · Ctrl+Shift+S backup versione · Ctrl+Shift+C commit custom</span><span class="st-r">Ctrl+B grassetto · Ctrl+I corsivo · Ctrl+K link · Ctrl+H cerca · Ctrl+G riga · Ctrl+Shift+F schermo intero</span></div>';
  h += '</div>';
...
function buildToolbar() {
  ...
  if (_current) {
    h +=
      '<button class="btn btn-soft btn-sm" onclick="_stToggle()" title="Passa all\'editor a blocchi (disponibile su ogni pagina)">🧱 Blocchi</button>';
    h +=
      '<button class="btn btn-soft btn-sm" onclick="saveKVVersion()" title="Salva una snapshot di backup in KV (max 20 per pagina)">📦 Salva Versione</button>';
  }
  EDITOR_TB.forEach(function (g) {
    g.forEach(function (cmd) {
      h +=
        '<button class="tb-btn2" data-cmd="' +
        cmd +
        '" title="' +
        EDITOR_TB_TITLE[cmd] +
        '">' +
  ...
  h += '<span class="tb-sep2"></span>';
  h +=
    '<button class="tb-btn2" id="tb-fullscreen" title="Schermo intero / Zen mode (Ctrl+Shift+F)" onclick="toggleFullScreen()">⛶</button>';
  h += '</div>';
  return h;
}
...
function onMdInput() {
  _snapUndo();
  _modified = true;
  setBadge('dirty', 'modificato');
  renderPreview();
  var md = document.getElementById('e-md');
  if (md) {
    _autosaveStore(md.value);
    _lastSavedContent = md.value;
  }
  updateStats();
}
...
function onMdKey(e) {
  ...
  if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key.toLowerCase() === 'c') {
    e.preventDefault();
    window._useCustomCommitMsg = true;
    savePage();
    return;
  }
  ...
  if (!e.ctrlKey && !e.metaKey && !e.altKey && _autoPair(e, ta)) {
    onMdInput();
    return;
  }
  if (e.key === 'Enter' && !e.shiftKey && !e.ctrlKey && !e.metaKey && _mdContinueList(ta)) {
    e.preventDefault();
    onMdInput();
    return;
  }
...
function mdToHtml(md) {
  ...
  md = md.replace(/\[([^\]]+)\]\(([^)]+)\)/g, function (m, txt, url) {
    return '<a href="' + _safeUrl(url) + '" target="_blank" rel="noopener noreferrer">' + txt + '</a>';
  });
...
function renderPreview() {
  _updateLineno();
  if (window.__stMode) {
    if (typeof _stSyncPreview === 'function') _stSyncPreview();
    return;
  }
  var md = document.getElementById('e-md');
  ...
  var hd = _currentHead();
  pv.innerHTML =
    '<div class="e-pv"><div class="e-pv-head"><span class="epv-icon">' +
    esc(hd.icon) +
    '</span><div><div class="epv-title">' +
    esc(hd.title) +
    '</div><div class="epv-sub">' +
    esc(_current.k || '') +
    '</div></div></div>' +
    mdToHtml(md.value) +
    '</div>';
  ...
  for (var i = 0; i < ls.length; i++) {
    if (/^\s*[-*+]\s+\[[ xX]\]\s?/.test(ls[i])) tl.push(i);
  }
  for (var j = 0; j < todos.length; j++) {
    if (tl[j] !== undefined) todos[j].setAttribute('data-line', tl[j]);
  }
  buildOutline();
}
function buildOutline() {
  var pv = document.getElementById('e-preview');
  var body = document.getElementById('e-ol-body');
  if (!pv || !body) return;
  ...
  for (var i = 0; i < heads.length; i++) {
    ...
    h +=
      '<button class="ol-item ' + tag + '" data-off="' + off + '" title="' + esc(txt) + '">' + esc(txt) + '</button>';
  }
  body.innerHTML = h || '<div style="padding:10px;color:var(--dim);font-size:11px">Nessuna sezione</div>';
}
function jumpToLine(off) {
  var pv = document.getElementById('e-preview');
  if (pv) {
    pv.scrollTop = off - 90;
    pv.focus();
...
function toggleOutline() {
  var ol = document.getElementById('e-outline');
  if (!ol) return;
  ol.classList.toggle('open');
  var btn = document.getElementById('tb-outline');
...
function setViewMode(m) {
  ...
  if (tb) {
    var vs = tb.querySelectorAll('[data-view]');
    for (var i = 0; i < vs.length; i++) vs[i].classList.toggle('active', vs[i].getAttribute('data-view') === m);
  }
  var body = document.getElementById('editor-body');
  if (!body) return;
  var mdP = document.getElementById('md-pane'),
    pvP = document.getElementById('pv-pane');
  var div = document.getElementById('e-divider'),
  ...
  if (m === 'site') {
    ...
    if (frame) {
      ...
      if (_current && _current.k) {
        _pageCleanPath().then(function (p) {
          frame.src = p;
        });
      }
    }
  ...
  if (m === 'md') {
    ...
    if (pvP) {
      ...
      pvP.style.flex = '1 1 auto';
    }
    if (div) div.style.display = 'none';
  } else {
    if (mdP) {
      mdP.style.display = 'flex';
      mdP.style.flex = _mdSplit + ' 1 0%';
    }
    if (pvP) {
      pvP.style.display = 'flex';
      pvP.style.flex = '1 1 0%';
    }
...
function _bindDivider() {
  ...
  div.addEventListener('mousedown', function (e) {
    ...
    function onMove(ev) {
      ...
      _mdSplit = Math.max(20, Math.min(80, pct));
      var mdP = document.getElementById('md-pane');
      if (mdP) mdP.style.flex = _mdSplit + ' 1 0%';
    }
    function onUp() {
...
function zoomPreviewImg(img) {
  ...
  img.style.width = '';
  img.style.margin = '';
  if (pv.classList.contains('zoomed')) {
...
function _bindEditorEvents() {
  ...
  document.addEventListener('click', function (ev) {
    ...
    if (t && t.closest('#e-toolbar')) {
      runCmd(t.getAttribute('data-cmd'));
      return;
    }
    t = ev.target.closest('[data-view]');
    if (t) {
      setViewMode(t.getAttribute('data-view'));
      return;
    }
    t = ev.target.closest('.ol-item');
    if (t) {
      jumpToLine(parseInt(t.getAttribute('data-off'), 10));
      return;
    }
    t = ev.target.closest('.n-todo');
    if (t && t.closest('#e-preview')) {
      _toggleTodo(t);
      ev.preventDefault();
      return;
    }
    t = ev.target.closest('a');
    if (t && t.closest('#e-preview') && !t.classList.contains('n-anchor')) {
      ev.preventDefault();
  ...
  window.addEventListener('resize', function () {
    if (document.getElementById('e-preview')) buildOutline();
  });
  var pv = document.getElementById('e-preview');
  if (pv) {
    pv.addEventListener('scroll', function () {
      ...
      var items = ol.querySelectorAll('.ol-item');
      if (!items.length) return;
      var scrollTop = pv.scrollTop;
      var bestIdx = 0;
...
function onLayoutChange(val, silent) {
  ...
  if (window.__stMode) {
    if (
      !silent &&
      !confirm(
        'Cambiare layout? I blocchi verranno riparseti con il nuovo template. Le modifiche non salvate andranno perse.'
      )
    ) {
  ...
  if (
    !silent &&
    (!current || confirm('Vuoi applicare il template per questo layout? Il contenuto attuale verra sostituito.')) &&
    md &&
    tpl
  ) {
    md.value = tpl;
...
/* ── SALVATAGGIO PAGINA ── */
function getTitle() {
  var el = document.getElementById('e-title');
  return el ? el.value.trim() : '';
}
function getIcon() {
  var el = document.getElementById('e-icon');
  return el ? el.value.trim() : '';
}
function _saveDelay(ms) {
...
async function savePage() {
  ...
  if (!remote.ok) {
    ...
    if (!resolved) {
      setStatus('idle', 'annullato');
      return;
    }
    if (resolved === 'remote') {
      _current.sha = remote.remoteSha;
    }
  }
  ...
  for (var attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      try {
        ...
        var rp = reg.pages.find(function (x) {
          return x.k === _current.k;
        });
        if (rp && (rp.l !== _current.title || rp.i !== _current.icon)) {
          ...
          await ghPut(
            ...
            'admin: sync ' + _current.k + ' metadata to registry',
            JSON.stringify(reg, null, 2) + '\n',
            rd.sha
          );
          var mi = PAGES.find(function (p) {
            return p.k === _current.k;
          });
          if (mi) {
            mi.l = _current.title;
            mi.i = _current.icon;
  ...
  toast(
    'Salvataggio fallito dopo ' + maxRetries + ' tentativi: ' + ((lastErr && lastErr.message) || 'errore sconosciuto'),
    'error'
  );
  if (btn) btn.disabled = false;
}
...
function _lineDiff(localText, remoteText) {
  ...
  for (var i = 0; i < max; i++) {
    ...
    if (l === r) {
      html +=
        '<div class="diff-line diff-same"><span class="diff-ln">' +
        (i + 1) +
        '</span><span class="diff-t">' +
        esc(l || '') +
        '</span></div>';
    } else {
      if (l !== null)
        html +=
          '<div class="diff-line diff-del"><span class="diff-ln">' +
          (i + 1) +
          '</span><span class="diff-t">- ' +
          esc(l) +
          '</span></div>';
      if (r !== null)
        html +=
          '<div class="diff-line diff-add"><span class="diff-ln">' +
          (i + 1) +
          '</span><span class="diff-t">+ ' +
          esc(r) +
          '</span></div>';
...
function _showConflictModal(localContent, remoteContent, remoteSha) {
  return new Promise(function (resolve) {
    ...
    var body =
      '<div class="conflict-info">⚠️ Il file è stato modificato da un\'altra sessione. Confronta le modifiche qui sotto.</div>' +
      '<div class="diff-stats"><span class="diff-stat-local">' +
      localLines +
      ' righe (la tua versione)</span>' +
      '<span class="diff-stat-remote">' +
      remoteLines +
      ' righe (versione remota)</span></div>' +
      '<div class="diff-container" id="diff-scroll">' +
      diffHtml +
      '</div>';
    var actions =
      '<button class="btn btn-soft" onclick="closeModal(\'' +
      id +
      '\');window._conflictResolve(null)">Annulla</button>' +
      '<button class="btn btn-soft" onclick="closeModal(\'' +
      id +
      "');window._conflictResolve('remote')\">Ripristina remota</button>" +
      '<button class="btn btn-p" onclick="closeModal(\'' +
      id +
      "');window._conflictResolve('overwrite')\">Sovrascrivi</button>";
...
async function _checkDraftStatus() {
  ...
  try {
    var r = await fetch('/api/admin?action=get_draft&pageKey=' + encodeURIComponent(_current.k), {
      credentials: 'include',
    });
    var j = await r.json();
...
async function saveDraft() {
  ...
  try {
    ...
    toast(
      publishAt
        ? 'Bozza salvata! Pubblicazione schedulata per ' + publishAt + '.'
        : 'Bozza salvata! Non è ancora visibile sul sito.',
      'success'
    );
    await _logAudit('save_draft', _current.k, { publishAt: publishAt });
  } catch (e) {
    setStatus('err', 'errore');
    toast('Errore bozza: ' + e.message, 'error');
  }
}
async function publishDraft() {
  ...
  if (
    !confirm(
      'Pubblicare questa bozza?\n\nLa versione attuale sul sito verrà sostituita con il contenuto della bozza e partirà il deploy.'
    )
  )
    return;
  ...
  try {
    ...
    setStatus('err', 'errore');
    toast('Errore pubblicazione: ' + e.message, 'error');
  }
}
...
function openEmojiDialog() {
  ...
  var grid = _emojiList
    .map(function (e) {
      return '<button type="button" class="em-cell" data-emoji="' + e + '">' + e + '</button>';
    })
    .join('');
  modalHtml(
    'emoji-modal',
    '😀 Inserisci emoji',
    '<div class="em-grid">' + grid + '</div>',
    '<button class="btn btn-soft" onclick="closeModal(\'emoji-modal\')">Annulla</button>'
  );
  var cells = document.querySelectorAll('#emoji-modal .em-cell');
  for (var i = 0; i < cells.length; i++)
    cells[i].addEventListener('click', function () {
      insMd(this.getAttribute('data-emoji'));
...
function openCalloutDialog() {
  ...
  modalHtml(
    'co-modal',
    '❗ Inserisci callout',
    '<div class="fld"><label>Tipo</label><select id="co-type" class="in">' +
      opts +
      '</select></div>' +
...
async function insertImgFromUpload() {
  if (!_imgDataUri) {
    toast('Scegli prima un file immagine', 'error');
    return;
  }
  var btn = document.getElementById('img-upload-btn');
  var status = document.getElementById('img-status');
...
/* ── VISTA JSON ── */
function toggleJson() {
  var jp = document.getElementById('json-pane');
  var body = document.getElementById('editor-body');
  ...
  if (jp.classList.contains('active')) {
    ...
    try {
      ...
      if (ls && j.layout !== undefined) {
        ls.value = j.layout || '';
        _current.layout = j.layout || '';
        onLayoutChange(j.layout || '', true);
      }
      _current.title = j.title || _current.title;
      _current.icon = j.icon || _current.icon;
      onMdInput();
    } catch (e) {
      toast('JSON non valido: ' + e.message, 'error');
    }
    setViewMode(_viewMode);
  } else {
    var json = {
      ...
      title: getTitle(),
      icon: getIcon(),
      content: document.getElementById('e-md').value,
      layout: _current.layout || '',
      lastModified: new Date().toISOString(),
    };
...
function openNewPageModal(prefLabel, prefSlug) {
  ...
  var layoutOpts = '';
  LAYOUTS.forEach(function (l) {
    if (l.v) layoutOpts += '<option value="' + l.v + '">' + l.i + ' ' + l.l + '</option>';
  });
  var secOpts = '';
  SECTIONS.forEach(function (s) {
    secOpts += '<option value="' + s.v + '">' + s.l + '</option>';
  });
  var subOpts = _subOptionsForSec('', '');
  modalHtml(
    'np-modal',
    '➕ Nuova pagina / sezione',
    '<div class="fld"><label>Nome della sezione</label><input id="np-label" class="in" placeholder="es. La Gilda dei Mercanti" value="' +
      escAttr(prefLabel || '') +
      '" onkeydown="if(event.key===\'Enter\')createNewPage()"></div>' +
      ...
      '<div class="fld"><label>Slug / URL</label><input id="np-slug" class="in" placeholder="auto (es. la-gilda-dei-mercanti)" value="' +
      escAttr(prefSlug || '') +
      '" onkeydown="if(event.key===\'Enter\')createNewPage()"></div>' +
      '<div class="fld"><label>Sezione del menu</label><select id="np-sec" class="in">' +
      secOpts +
      '</select></div>' +
      '<div class="fld"><label>Sottosezione (opzionale)</label><select id="np-sub" class="in">' +
      subOpts +
      '</select></div>' +
      '<div class="fld"><label>Layout</label><select id="np-layout" class="in">' +
      layoutOpts +
      '</select></div>',
  ...
  document.getElementById('np-slug').addEventListener('input', function () {
    _slugTouched = true;
  });
...
async function createNewPage() {
  ...
  if (!label) {
    toast('Inserisci un nome per la sezione', 'error');
    return;
  }
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug)) {
    toast('Slug non valido: solo minuscole, numeri e trattini', 'error');
    return;
  }
  if (
    ...
    toast('Esiste già una pagina con slug "' + slug + '"', 'error');
    return;
  }
  var id = 'pag-' + slug;
  ...
  try {
    var json = {
      ...
      title: label,
      icon: icon,
      content: buildNewPageContent(layout, label),
      layout: layout,
      lastModified: new Date().toISOString(),
    };
...
function clonePageModal() {
  ...
  var secOpts = '';
  SECTIONS.forEach(function (s) {
    secOpts += '<option value="' + s.v + '"' + (meta && meta.sec === s.v ? ' selected' : '') + '>' + s.l + '</option>';
  });
  ...
  modalHtml(
    'cl-modal',
    '📋 Clona pagina',
    '<div class="fld"><label>Nome della nuova pagina</label><input id="cl-label" class="in" value="' +
      escAttr(cloneLabel) +
      '" onkeydown="if(event.key===\'Enter\')clonePage()"></div>' +
      '<div class="fld"><label>Icona (emoji)</label><input id="cl-icon" class="in" value="' +
      escAttr(_current.icon || '📄') +
      '" onkeydown="if(event.key===\'Enter\')clonePage()"></div>' +
      '<div class="fld"><label>Slug / URL</label><input id="cl-slug" class="in" value="' +
      escAttr(cloneSlug) +
      '"></div>' +
      '<div class="fld"><label>Sezione del menu</label><select id="cl-sec" class="in">' +
      secOpts +
      '</select></div>' +
      '<div class="fld"><label>Sottosezione</label><select id="cl-sub" class="in">' +
      subOpts +
      '</select></div>' +
      '<div class="fld"><label>Layout</label><select id="cl-layout" class="in">' +
      layoutOpts +
      '</select></div>' +
...
async function clonePage() {
  ...
  if (!slug) {
    toast('Inserisci uno slug', 'error');
    return;
  }
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug)) {
    toast('Slug non valido', 'error');
    return;
  }
  if (
    PAGES.some(function (p) {
    ...
    toast('Slug già esistente', 'error');
    return;
  }
  ...
  try {
    ...
    var json = {
      ...
      title: label,
      icon: icon,
      content: srcContent || buildNewPageContent(layout, label),
      layout: layout,
      lastModified: new Date().toISOString(),
    };
    var d0 = await ghGet('content/pages/registry.json');
    ...
    setStatus('err', 'errore');
    toast('Errore clonazione: ' + e.message, 'error');
  }
}
...
async function removeFromRegistry(slug) {
  ...
  if (!Array.isArray(reg.pages)) reg.pages = [];
  var pg = reg.pages.find(function (p) {
    return p.k === slug;
  });
  var id = pg ? pg.id : null;
  var before = JSON.stringify(reg);
  reg.pages = reg.pages.filter(function (p) {
    return p.k !== slug;
  });
  (reg.sections || []).forEach(function (s) {
    if (Array.isArray(s.pages))
      s.pages = s.pages.filter(function (x) {
        return x !== id;
      });
  });
  if (id) {
    if (Array.isArray(reg.lavori))
      reg.lavori = reg.lavori.filter(function (w) {
        return w.id !== id;
      });
    if (Array.isArray(reg.legacySlugs))
      reg.legacySlugs = reg.legacySlugs.filter(function (x) {
        return x.id !== id;
      });
    if (Array.isArray(reg.layoutDatabases))
      reg.layoutDatabases = reg.layoutDatabases.filter(function (x) {
        return x.id !== id;
      });
    if (Array.isArray(reg.pathOverrides))
      reg.pathOverrides = reg.pathOverrides.filter(function (x) {
        return x.id !== id;
      });
    (reg.indexDatabases || []).forEach(function (db) {
      if (Array.isArray(db.ids))
        db.ids = db.ids.filter(function (x) {
          return x !== id;
        });
    });
  }
  ...
  if (out !== before)
    await ghPut('content/pages/registry.json', 'admin: remove page ' + slug + ' from registry', out, d.sha);
}
async function addToRegistry(slug, label, icon, id, sec, sub) {
  ...
  await ghPut(
    'content/pages/registry.json',
    'admin: add page ' + slug + ' to registry',
    JSON.stringify(reg, null, 2) + '\n',
    d.sha
  );
}
async function renameInRegistry(slug, label, icon) {
  var d = await ghGet('content/pages/registry.json');
  var reg = JSON.parse(b64decode(d.content));
  var p = reg.pages.find(function (x) {
    return x.k === slug;
  });
...
async function deletePage() {
  ...
  var choice = await uiConfirmChoice(msg, {
  ...
  try {
    ...
    document.getElementById('main').innerHTML =
      '<div class="empty"><span class="ei">🗑</span>' + esc(label) + ' eliminata</div>';
    setTitle('');
...
async function trashPage() {
  ...
  try {
    ...
    /* Save to KV trash */
    var r = await _authPost('/api/admin?action=trash_page', {
      pageKey: slug,
      pageData: pageData,
    ...
    buildSidebar();
    document.getElementById('main').innerHTML =
      '<div class="empty"><span class="ei">🗑️</span>' +
      esc(label) +
      ' spostata nel cestino <button class="btn btn-soft btn-sm" onclick="openTrash()" style="margin-left:8px">Apri cestino</button></div>';
    setTitle('');
...
function _rewriteIndexForPage(html, id, label, icon) {
  ...
  html = html.replace(
    new RegExp(
      '(<div class="tn-item"[^>]*gp\\(\'' +
        id +
        "','[^']*','[^']*'\\)[^>]*>)<span class=\"tn-ii\">[^<]*</span>[^<]*(</div>)"
    ),
    '$1<span class="tn-ii">' + IH + '</span>' + LH + '$2'
  );
  html = html.replace(
    new RegExp(
      '(<div class="mn-item"[^>]*gp\\(\'' +
        id +
        "','[^']*','[^']*'\\)[^>]*>)<span class=\"mn-ii\">[^<]*</span>[^<]*(</div>)"
    ),
  ...
  html = html.replace(
    new RegExp(
      '(<div class="lcard"[^>]*gp\\(\'' +
        id +
        "','[^']*','[^']*'\\)[^>]*><div class=\"shine\"></div><div class=\"cthumb\"[^>]*>)<span>[^<]*</span>"
    ),
    '$1<span>' + IH + '</span>'
  );
  html = html.replace(
    new RegExp(
      '(<div class="lcard"[^>]*gp\\(\'' + id + "','[^']*','[^']*'\\)[^>]*>[^\\n]*?<div class=\"ctit\">)[^<]*(</div>)"
    ),
    '$1' + LH + '$2'
  );
  html = html.replace(
...
function openRenameModal() {
  ...
  modalHtml(
    'rn-modal',
    '✏️ Rinomina sezione / pagina',
    '<div class="fld"><label>Nome</label><input id="rn-label" class="in" value="' +
      escAttr(label) +
      '" onkeydown="if(event.key===\'Enter\')saveRename()"></div>' +
      '<div class="fld"><label>Icona (emoji)</label><input id="rn-icon" class="in" value="' +
      escAttr(icon) +
      '" onkeydown="if(event.key===\'Enter\')saveRename()"></div>',
            ...
            .map(function (c, i) {
              ...
              var date =
                c.commit && c.commit.author && c.commit.author.date
                  ? new Date(c.commit.author.date).toLocaleString('it-IT')
                  : '';
              var sha = c.sha ? c.sha.slice(0, 8) : '';
              var prevSha = i < commits.length - 1 ? commits[i + 1].sha : '';
              var diffBtn = prevSha
                ? '<button class="btn btn-soft btn-sm" onclick="showDiff(\'' +
                  prevSha +
                  "','" +
                  c.sha +
                  '\')">DIFF</button>'
                : '';
              return (
                '<div class="row" style="cursor:default;background:var(--panel);border:1px solid var(--line)">' +
                ...
                '<div class="ract">' +
                diffBtn +
                '<button class="btn btn-soft btn-sm" onclick="restoreVersion(\'' +
                c.sha +
                '\')">RIPRISTINA</button></div></div>'
              );
            })
            .join('')
        : '<div class="list-empty">Nessun commit trovato</div>';
    modalHtml(
      'hist-modal',
      '📜 Storia — ' + esc(_current.title || _current.k),
...
async function showDiff(sha1, sha2) {
  ...
  try {
    var r = await _authPost('/api/admin?action=get_diff', {
      sha1: sha1,
      sha2: sha2,
      path: CONTENT + '/pages/' + _current.k + '.json',
    });
    var j = await r.json();
    if (!j.ok) throw new Error(j.error || 'Diff non disponibile');
    var lines = j.diff || [];
    var h =
      '<div class="diff-view" style="font-family:var(--mono);font-size:12px;max-height:60vh;overflow-y:auto;padding:0">';
    h +=
      '<div style="padding:8px 12px;border-bottom:1px solid var(--line);color:var(--dim);font-size:11px">' +
      sha1.slice(0, 8) +
      ' → ' +
      sha2.slice(0, 8) +
      '</div>';
    if (!lines.length) {
      ...
      lines.forEach(function (ln) {
        ...
        h +=
          '<div class="' +
          cls +
          '" style="padding:2px 12px;white-space:pre-wrap;border-bottom:1px solid rgba(255,255,255,0.03)">' +
          esc(prefix + ln.slice(1)) +
          '</div>';
      });
    }
    h += '</div>';
    modalHtml(
      ...
      '🔀 Diff — ' + _current.k,
      h,
      '<button class="btn btn-soft" onclick="closeModal(\'diff-modal\')">Chiudi</button>',
      'md-wide'
    );
  } catch (e) {
    toast('Errore diff: ' + e.message, 'error');
  }
  setStatus('idle', 'pronto');
}
async function restoreVersion(sha) {
  if (
    !confirm(
      "Ripristinare questa versione nell'editor?\n\nNon viene salvata automaticamente: controlla il contenuto e poi premi SALVA."
    )
  )
    return;
  try {
    ...
    _current.title = json.title;
    _current.icon = json.icon;
    _current.layout = json.layout || '';
    ...
    var ls = document.getElementById('e-layout');
    if (ls) {
      ls.value = json.layout || '';
      onLayoutChange(ls.value, true);
    }
    renderPreview();
    _modified = true;
    setBadge('dirty', 'ripristinata — salva per applicare');
...
async function saveKVVersion() {
  ...
  var content = md ? md.value : '';
  if (!content) {
    toast('Nessun contenuto da salvare', 'error');
    return;
  }
...
async function openKVVersions() {
  ...
  try {
    var r = await fetch('/api/admin?action=get_versions&pageKey=' + encodeURIComponent(_current.k), {
      credentials: 'include',
    });
    var j = await r.json();
    if (j.error) throw new Error(j.error);
    ...
    if (!versions.length) {
      modalHtml(
        'kvver-modal',
        '💾 Backup versioni — ' + esc(_current.title || _current.k),
    ...
    var rows = versions
      .map(function (v, i) {
        ...
        return (
          ...
          esc(date) +
          '</div>' +
          '<div class="rs" style="font-family:var(--mono);font-size:11px;opacity:.7">' +
          preview +
          '…</div></div>' +
          '<div class="ract">' +
          '<button class="btn btn-soft btn-sm" onclick="previewKVVersion(' +
          i +
          ')">ANTEPRIMA</button>' +
          '<button class="btn btn-p btn-sm" onclick="restoreKVVersion(' +
          i +
          ')">RIPRISTINA</button>' +
          '</div></div>'
        );
...
async function previewKVVersion(index) {
  try {
    ...
    if (!v || !v.preview) {
      toast('Versione non trovata', 'error');
      return;
    }
    modalHtml(
      'kvver-preview',
      'Anteprima versione',
      '<div style="font-family:var(--mono);font-size:12px;white-space:pre-wrap;max-height:60vh;overflow-y:auto;padding:16px">' +
        esc(v.preview) +
        '</div>',
      '<button class="btn btn-soft" onclick="closeModal(\'kvver-preview\')">Chiudi</button>',
      'md-wide'
    );
  } catch (e) {
    toast('Errore: ' + e.message, 'error');
...
async function restoreKVVersion(index) {
  var ok = await uiConfirm(
    "Ripristinare questa versione dalla backup?\n\nLa versione corrente verrà salvata come backup automatico prima del ripristino. Il contenuto verrà caricato nell'editor — ricordati di premere SALVA.",
    { ok: 'RIPRISTINA' }
  );
  if (!ok) return;
...
async function checkLinks() {
  ...
  links = links.filter(function (l) {
    return l.indexOf('/images/') !== 0 && l.indexOf('/audio/') !== 0 && l.indexOf('/scripts/') !== 0;
  });
  if (!links.length) {
    toast('Nessun link interno trovato nel contenuto', 'success');
    return;
  }
    ...
    var rows = bad.length
      ? bad
          .map(function (l) {
            return (
              '<div class="row" style="cursor:default;background:var(--panel);border:1px solid var(--line)">' +
              '<div class="rico">🔗</div><div class="rmain"><div class="rt" style="color:var(--red)">' +
              esc(l) +
              '</div>' +
          ...
          .join('')
      : '<div class="list-empty">Tutti i ' + links.length + ' link interni risultano validi ✓</div>';
    modalHtml(
      'lk-modal',
      '🔗 Link interni — ' + links.length + ' trovati, ' + bad.length + ' sospetti',
      '<div class="list-body">' + rows + '</div>',
...
async function openImages() {
  ...
  try {
    ...
    var h = viewHead(
      '🖼️',
      'Immagini',
      'Archivio condiviso in /images/',
      '<button class="btn btn-soft" onclick="openImageUpload()">⬆ CARICA</button>' +
        '<button class="btn btn-p" onclick="openImages()">⟳ AGGIORNA</button>'
    );
    h +=
      '<div class="img-controls" style="display:flex;gap:8px;align-items:center;margin-bottom:12px;padding:8px 16px;background:var(--panel);border:1px solid var(--line);border-radius:8px">';
    h +=
      '<input id="img-search" class="in" style="width:200px" placeholder="Cerca immagine…" oninput="_imgFilter=this.value;openImages()" value="' +
      escAttr(_imgFilter) +
      '">';
    h +=
      '<select id="img-sort" class="in" style="width:auto;padding:5px 8px;font-size:11px" onchange="_imgSortBy=this.value;openImages()">';
    ...
    h += '</select>';
    h +=
      "<button class=\"btn btn-soft btn-sm\" onclick=\"_imgViewMode=_imgViewMode==='grid'?'list':'grid';openImages()\">" +
      (_imgViewMode === 'grid' ? '☰ Lista' : '▦ Griglia') +
      '</button>';
    h +=
      '<button class="btn btn-soft btn-sm" onclick="_imgAnalyzeUsage(this)" title="Scansiona pagine, mappa e home">🧭 Analizza utilizzi</button>';
    h +=
      '<button class="btn btn-soft btn-sm" onclick="openAltTextAudit()" title="Audit testo alternativo immagini">♿ Alt-text</button>';
    h += '</div>';
    if (_imgUsage) {
      var unused = items.filter(function (it) {
        return !_imgUsage[it.name];
      }).length;
      h +=
        '<div style="padding:6px 16px 0;color:var(--dim);font-size:11.5px">Analisi: <b style="color:var(--acc)">' +
        (items.length - unused) +
        '</b> usate · <b style="color:' +
        (unused ? 'var(--red)' : 'inherit') +
        '">' +
        unused +
        '</b> non utilizzate</div>';
    }
    ...
    if (_imgViewMode === 'list') {
      h +=
        '<div class="panel"><div class="panel-head"><h3>Immagini</h3><span class="hint">' +
        items.length +
        ' file</span></div>';
      ...
      items.forEach(function (it) {
        ...
        h +=
          '<div class="row">' +
          '<div class="rico" style="overflow:hidden;width:40px;height:40px;border-radius:4px"><img src="' +
          esc(it.download_url || '') +
          '" alt="" style="width:100%;height:100%;object-fit:cover"></div>' +
          '<div class="rmain"><div class="rt img-rename" contenteditable="true" spellcheck="false" data-original="' +
          escAttr(name) +
          '" data-sha="' +
          escAttr(it.sha || '') +
          '" onblur="renameImage(this)" onkeydown="if(event.key===\'Enter\'){event.preventDefault();this.blur()}">' +
          esc(name) +
          '</div>' +
          '<div class="rs">' +
          size +
          ...
          (_imgUsage
            ? _imgUsage[name]
              ? ' · <span style="color:var(--green,#7dc87d)">usata ×' + _imgUsage[name] + '</span>'
              : ' · <span style="color:var(--red)">mai usata</span>'
            : '') +
          '</div></div>' +
          '<div class="ract">' +
          '<button class="btn btn-soft btn-sm" onclick="copyImageUrl(\'' +
          escJsAttr(name) +
          '\')">🔗 URL</button>' +
          '<button class="btn btn-d btn-sm" onclick="deleteImage(\'' +
          escJsAttr(name) +
          "','" +
          esc(it.sha) +
          '\')">🗑</button>' +
      ...
      if (items.length > _imgPageSize) {
        ...
        h +=
          '<div id="img-load-more" style="text-align:center;padding:16px"><button class="btn btn-soft" onclick="_imgLoadMore()">Carica altre ' +
          Math.min(rem, _imgPageSize) +
          ' immagini (' +
          rem +
          ' rimaste)</button></div>';
      }
...
async function _imgAnalyzeUsage(btn) {
  ...
  try {
    ...
    try {
      bump(b64decode((await ghGet('content/mappins.json')).content));
    } catch (e) {}
    try {
      bump(b64decode((await ghGet('index.html')).content));
    } catch (e) {}
    try {
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = '🧭 Analizza utilizzi';
    }
  }
}
async function openAltTextAudit() {
  ...
  try {
      ...
      lines.forEach(function (line, num) {
        ...
        if (withAlt) {
          withAlt.forEach(function (m) {
            var parts = m.match(/!\[([^\]]*)\]\(([^)]+)\)/);
            if (parts && parts[1] && parts[1].length < 5)
              results.push({ page: pg.k || pg.slug, line: num + 1, src: parts[2], alt: parts[1], type: 'short' });
          });
        }
      });
    });
    ...
    var h =
      '<div style="padding:8px 0 12px;font-size:13px"><b>' +
      results.length +
      '</b> immagini con alt-text potenzialmente problematico</div>';
    h +=
      '<div style="font-size:12px;color:var(--dim);margin-bottom:12px">🟢 Alt-text presente e valido · 🟡 Alt-text vuoto (aggiungi descrizione) · 🔴 Alt-text troppo corto</div>';
    results.slice(0, 50).forEach(function (r) {
      ...
      h +=
        '<div class="cover-row" style="cursor:default;' +
        cls +
        '">' +
        '<div class="si" style="font-size:18px">' +
        icon +
        '</div>' +
        '<div class="cmain"><div class="ct" style="font-size:13px">' +
        esc(r.page) +
        '</div>' +
        '<div class="cs" style="font-family:var(--mono);font-size:11px">' +
        esc(r.src) +
        '</div>' +
        '<div class="cs">riga ' +
        r.line +
        (r.alt ? ' · alt: "' + esc(r.alt) + '"' : ' · alt vuoto') +
        '</div></div></div>';
    });
    if (results.length > 50)
      h +=
        '<div style="padding:8px;font-size:12px;color:var(--dim)">… e altre ' +
        (results.length - 50) +
        ' occorrenze</div>';
    modalHtml(
      'alt-audit',
      '♿ Audit Alt-Text',
      h,
      '<button class="btn btn-soft" onclick="closeModal(\'alt-audit\')">Chiudi</button>',
      'md-wide'
    );
    setStatus('idle', 'pronto');
  } catch (e) {
...
function _imgCard(it) {
  ...
  return (
    ...
    esc(name) +
    '" loading="lazy">' +
    '<div class="ic-body"><div class="ic-name">' +
    esc(name) +
    '</div>' +
    '<div class="ic-meta">' +
    size +
    ...
    '</div>' +
    (_imgUsage
      ? '<div class="ic-meta" style="' +
        (_imgUsage[name] ? 'color:#7dc87d' : 'color:var(--red)') +
        '">' +
        (_imgUsage[name] ? 'usata ×' + _imgUsage[name] : 'mai usata') +
        '</div>'
      : '') +
...
async function copyImageUrl(name) {
  try {
    ...
    toast('Errore copia: ' + e.message, 'error');
  }
}
...
function toggleBulkMode() {
  ...
  if (btn) btn.textContent = _bulkMode ? '✕ ESCI' : 'GESTISCI';
  renderDashboard();
  if (_bulkMode) {
    toast('Modalità gestione attiva — Shift+Click per selezionare range', 'success');
  }
}
function toggleBulkItem(cb, e) {
  var k = cb.getAttribute('data-key');
  var idx = parseInt(cb.getAttribute('data-idx'), 10);
  var isShift = e && e.shiftKey;
  if (isShift && _bulkLastIdx !== -1) {
    var rows = document.querySelectorAll('.row[data-type="page"]');
    var start = Math.min(_bulkLastIdx, idx);
    var end = Math.max(_bulkLastIdx, idx);
    for (var i = start; i <= end; i++) {
      var row = rows[i];
      if (row) {
        var rk = row.getAttribute('data-key');
        var rcb = row.querySelector('.bulk-cb');
        if (rcb && !_bulkSelected[rk]) {
          _bulkSelected[rk] = 1;
          rcb.checked = true;
        }
      }
    ...
    if (cb.checked) _bulkSelected[k] = 1;
    else delete _bulkSelected[k];
  }
  _bulkLastIdx = idx;
  var count = Object.keys(_bulkSelected).length;
  var el = document.getElementById('bulk-count');
  if (el) el.textContent = count + ' ' + (count === 1 ? 'selezionata' : 'selezionate');
  var selAll = document.getElementById('bulk-select-all');
  if (selAll) selAll.checked = count === Object.keys(PAGES).length && count > 0;
  document.querySelectorAll('.row[data-type="page"]').forEach(function (row) {
    var rk = row.getAttribute('data-key');
    row.classList.toggle('bulk-selected', !!_bulkSelected[rk]);
  });
}
function toggleBulkAll(cb) {
  var checked = cb.checked;
  var rows = document.querySelectorAll('.row[data-type="page"] .bulk-cb');
  _bulkSelected = {};
  rows.forEach(function (rcb) {
    rcb.checked = checked;
    if (checked) _bulkSelected[rcb.getAttribute('data-key')] = 1;
  });
  var count = checked ? rows.length : 0;
  var el = document.getElementById('bulk-count');
  if (el) el.textContent = count + ' ' + (count === 1 ? 'selezionata' : 'selezionate');
  document.querySelectorAll('.row[data-type="page"]').forEach(function (row) {
...
async function bulkDelete() {
  ...
  if (!keys.length) {
    toast('Seleziona almeno una pagina', 'error');
    return;
  }
  if (
    !(await uiConfirm('Eliminare ' + keys.length + ' pagina/e selezionate?\n\nOperazione irreversibile.', {
      danger: true,
      ok: 'Elimina',
    }))
  )
...
async function bulkDuplicate() {
  ...
  for (var i = 0; i < keys.length; i++) {
    try {
      ...
      var newKey = keys[i] + '-copia-' + Date.now().toString(36).slice(-4);
      json.k = newKey;
      json.title = json.title + ' (copia)';
      json.lastModified = new Date().toISOString();
      var newPath = CONTENT + '/pages/' + newKey + '.json';
      await ghPut(newPath, 'admin: duplicate ' + keys[i], JSON.stringify(json, null, 2));
      var meta = PAGES.find(function (p) {
        return p.k === keys[i];
      });
      if (meta) {
        PAGES.push({ k: newKey, l: json.title, i: json.icon || meta.i, sec: meta.sec, c: 1 });
      }
      ok++;
    } catch (e) {
      err++;
  ...
  _bulkMode = false;
  buildSidebar();
  setStatus('ok', 'duplicato ' + ok + '/' + keys.length);
  toast(ok + ' pagine duplicate' + (err ? ', ' + err + ' errori' : ''), 'success');
  renderDashboard();
  startDeployTimer();
}
async function bulkMove(sec) {
  ...
  if (!sec) {
    toast('Seleziona una sezione di destinazione', 'error');
    return;
  }
  if (!(await uiConfirm('Muovere ' + keys.length + ' pagina/e nella sezione "' + sec + '"?'))) return;
...
async function bulkChangeLayout() {
  ...
  if (!layout) {
    toast('Seleziona un layout', 'error');
    return;
  }
  if (!(await uiConfirm('Applicare il layout "' + layout + '" a ' + keys.length + ' pagina/e?'))) return;
...
async function bulkChangeIcon() {
  var keys = Object.keys(_bulkSelected);
  if (!keys.length) {
    toast('Seleziona almeno una pagina', 'error');
    return;
  }
  var icon = prompt('Inserisci la nuova icona (emoji):', '📄');
  if (!icon) return;
  if (!(await uiConfirm('Cambiare icona a "' + icon + '" per ' + keys.length + ' pagina/e?'))) return;
  setStatus('saving', 'cambio icona…');
  var ok = 0,
    err = 0;
  for (var i = 0; i < keys.length; i++) {
    try {
      ...
      var json = JSON.parse(b64decode(d.content));
      json.icon = icon;
      await ghPut(path, 'admin: bulk icon ' + keys[i], JSON.stringify(json, null, 2), d.sha);
      var meta = PAGES.find(function (p) {
        return p.k === keys[i];
      });
      if (meta) meta.i = icon;
  ...
  _bulkMode = false;
  buildSidebar();
  setStatus('ok', 'icona aggiornata per ' + ok + '/' + keys.length);
  toast(ok + ' pagine aggiornate con icona ' + icon + (err ? ', ' + err + ' errori' : ''), 'success');
  renderDashboard();
  startDeployTimer();
}
function bulkExportSelected() {
  ...
  a.href = url;
  a.download = 'arcamis-export-' + keys.length + '-pages-' + new Date().toISOString().slice(0, 10) + '.json';
  ...
  reader.onload = function (e) {
    try {
      ...
      if (typeof pages === 'object' && !Array.isArray(pages))
        pages = Object.keys(pages).map(function (k) {
          return pages[k];
        });
      _importData = j;
...
async function exportAllPages() {
  ...
  try {
    ...
    var blob = new Blob([JSON.stringify({ pages: all, exportDate: new Date().toISOString() }, null, 2)], {
      type: 'application/json',
    });
    var url = URL.createObjectURL(blob);
    var a = document.createElement('a');
    a.href = url;
    a.download = 'arcamis-export-' + new Date().toISOString().slice(0, 10) + '.json';
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
    ...
    setStatus('err', 'errore');
    toast('Errore esportazione: ' + e.message, 'error');
  }
}
...
function _frSearch() {
  ...
  if (info)
    info.textContent = _frMatches.length
      ? _frMatches.length + ' risultati' + (_frIdx >= 0 ? ' — ' + (_frIdx + 1) + '/' + _frMatches.length : '')
      : 'Nessun risultato';
}
function _frHighlight(ta) {
  ...
  ta.selectionEnd = start + q.length;
...
function findNext() {
  ...
  var ta = document.getElementById('e-md');
  if (!ta) return;
  _frIdx = (_frIdx + 1) % _frMatches.length;
  var info = document.getElementById('fr-info');
  if (info) info.textContent = _frIdx + 1 + '/' + _frMatches.length;
  _frHighlight(ta);
}
function findPrev() {
  _frSearch();
  ...
  var ta = document.getElementById('e-md');
  if (!ta) return;
  _frIdx = (_frIdx - 1 + _frMatches.length) % _frMatches.length;
  var info = document.getElementById('fr-info');
  if (info) info.textContent = _frIdx + 1 + '/' + _frMatches.length;
  _frHighlight(ta);
}
function replaceCurrent() {
  var ta = document.getElementById('e-md');
  var q = (document.getElementById('fr-find') || {}).value || '';
  var r = (document.getElementById('fr-repl') || {}).value;
  if (!ta || !q || _frIdx < 0 || !_frMatches.length) return;
  var start = _frMatches[_frIdx];
  ta.value = ta.value.substring(0, start) + r + ta.value.substring(start + q.length);
  _frSearch();
  onMdInput();
}
function replaceAll() {
...
function openGoToLine() {
  ...
  modalHtml(
    'gtl-modal',
    '📍 Vai alla riga',
    '<div class="fld"><label>Riga (1–' +
      total +
      ')</label><input id="gtl-line" class="in" type="number" min="1" max="' +
      total +
      '" value="1" onkeydown="if(event.key===\'Enter\')goToLine()"></div>',
    '<button class="btn btn-soft" onclick="closeModal(\'gtl-modal\')">Annulla</button>' +
      '<button class="btn btn-p" onclick="goToLine()">VAI</button>'
  );
  var inp = document.getElementById('gtl-line');
  if (inp) {
    inp.focus();
    inp.select();
  }
...
function goToLine() {
  ...
  if (isNaN(n) || n < 1 || n > total) {
    toast('Numero riga non valido (1–' + total + ')', 'error');
    return;
  }
  var pos = 0;
  for (var i = 0; i < n - 1; i++) {
...
function _buildLineMap() {
  ...
  var pvPositions = [];
  for (var k = 0; k < pvEls.length; k++) {
    pvPositions.push(pvEls[k].offsetTop);
  }
  for (var m = 0; m < totalMd; m++) {
    var hIdx = -1;
    for (var n = 0; n < headings.length; n++) {
      if (headings[n] <= m) hIdx = n;
    }
...
function _edFontSize(delta) {
  if (delta === 0) {
    try {
      localStorage.removeItem('arcEdFs');
    } catch (e) {}
  } else {
    var cur = 13;
    try {
      cur = parseInt(localStorage.getItem('arcEdFs') || '', 10) || 13;
    } catch (e) {}
    var next = Math.max(11, Math.min(22, cur + delta));
    try {
      localStorage.setItem('arcEdFs', String(next));
    } catch (e) {}
  }
  _applyEdPrefs();
}
...
function _applyEdPrefs() {
  ...
  var fs = 13;
  try {
    fs = parseInt(localStorage.getItem('arcEdFs') || '', 10) || 13;
  } catch (e) {}
  pane.style.setProperty('--ed-fs', fs + 'px');
  var wrap = false;
  try {
    wrap = localStorage.getItem('arcEdWrap') === '1';
  } catch (e) {}
  ...
  var ln = false;
  try {
    ln = localStorage.getItem('arcEdLn') === '1' && !window.__stMode;
  } catch (e) {}
...
function toggleFullScreen() {
  ...
  if (isFS) {
    _zenMode = false;
    _typewriterMode = false;
    app.classList.remove('full-screen', 'zen-mode', 'typewriter-mode');
    document.getElementById('sidebar').style.display = '';
    document.getElementById('topbar').style.display = '';
    var ol = document.getElementById('e-outline');
    if (ol) ol.classList.remove('open');
    var btn = document.getElementById('tb-fullscreen');
    if (btn) {
      btn.textContent = '⛶';
      btn.classList.remove('active', 'zen', 'typewriter');
      btn.title = 'Schermo intero / Zen mode (Ctrl+Shift+F)';
    }
    var sb = document.getElementById('e-sb');
    if (sb)
      sb.textContent =
        _layoutLabel(_current ? _current.layout : '') + ' · Autosave ogni 5s · SALVA / Ctrl+S salva subito';
    ...
    if (btn) {
      ...
      btn.title = 'Esci schermo intero (Esc o Ctrl+Shift+F)';
    }
    var sb = document.getElementById('e-sb');
    if (sb)
      sb.innerHTML = '<span class="fs-hint">Esc/Ctrl+Shift+F esce · Ctrl+Shift+Z zen · Ctrl+Shift+T typewriter</span>';
...
function toggleZenMode() {
  var app = document.getElementById('app');
  if (!app || !app.classList.contains('full-screen')) {
    toggleFullScreen();
    return;
  }
  _zenMode = !_zenMode;
  app.classList.toggle('zen-mode', _zenMode);
  var btn = document.getElementById('tb-fullscreen');
  if (btn) {
    btn.classList.toggle('zen', _zenMode);
    btn.textContent = _zenMode ? '⛶✨' : '⛶';
  }
  var sb = document.getElementById('e-sb');
  if (sb) {
    if (_zenMode)
      sb.innerHTML = '<span class="fs-hint">ZEN MODE · Esc/Ctrl+Shift+F esce · Ctrl+Shift+T typewriter</span>';
    else
      sb.innerHTML = '<span class="fs-hint">Esc/Ctrl+Shift+F esce · Ctrl+Shift+Z zen · Ctrl+Shift+T typewriter</span>';
  }
}
function toggleTypewriterMode() {
  ...
  app.classList.toggle('typewriter-mode', _typewriterMode);
  var btn = document.getElementById('tb-fullscreen');
  if (btn) {
    btn.classList.toggle('typewriter', _typewriterMode);
  }
  var sb = document.getElementById('e-sb');
  if (sb) {
    if (_typewriterMode)
      sb.innerHTML = '<span class="fs-hint">TYPEWRITER · Esc/Ctrl+Shift+F esce · Ctrl+Shift+Z zen</span>';
    else if (_zenMode)
      sb.innerHTML = '<span class="fs-hint">ZEN MODE · Esc/Ctrl+Shift+F esce · Ctrl+Shift+T typewriter</span>';
    else
      sb.innerHTML = '<span class="fs-hint">Esc/Ctrl+Shift+F esce · Ctrl+Shift+Z zen · Ctrl+Shift+T typewriter</span>';
  ...
  if (_typewriterMode) {
    var ta = document.getElementById('e-md');
    if (ta) {
      _centerEditorLine(ta);
      ta.addEventListener('input', _centerEditorLine);
      ta.addEventListener('scroll', _centerEditorLine);
    }
  } else {
    var ta = document.getElementById('e-md');
    if (ta) {
      ta.removeEventListener('input', _centerEditorLine);
      ta.removeEventListener('scroll', _centerEditorLine);
    }
  }
}
function _centerEditorLine(ta) {
  if (!ta || !_typewriterMode) return;
  var lineHeight = parseInt(getComputedStyle(ta).lineHeight) || 22;
  var middle = ta.clientHeight / 2;
  var cursorTop = (ta.selectionStart * lineHeight) / ta.value.length;
  ta.scrollTop = Math.max(0, cursorTop - middle + lineHeight);
}
...
function _mdContinueList(ta) {
  ...
  if (m) {
    ...
    if (!m[4].trim()) {
      ta.value = val.substring(0, ls) + val.substring(s);
...
function _autoPair(e, ta) {
  ...
  var ch = e.key;
  if (!_autoPairMap[ch]) return false;
  var s = ta.selectionStart,
    en = ta.selectionEnd;
  if (s !== en) {
    var sel = ta.value.substring(s, en);
...
function _bindEditorKeys() {
  if (_editorKeyBound) return;
  _editorKeyBound = true;
  document.addEventListener('keydown', function (ev) {
    if (!document.getElementById('e-md')) return;
    var ta = document.getElementById('e-md');
    if (!ta) return;
    var mod = ev.ctrlKey || ev.metaKey;
    if (mod && ev.key.toLowerCase() === 'h' && !ev.shiftKey) {
      ev.preventDefault();
      openFindReplace();
      return;
    }
    ...
    if (mod && ev.shiftKey && ev.key.toLowerCase() === 'z') {
      ev.preventDefault();
      toggleZenMode();
      return;
    }
