...
async function openCarousel() {
  ...
  try {
    ...
    var h = viewHead(
      '🎠',
      'Carousel homepage',
      'Le tre slide mostrate in cima alla home',
      '<button class="btn btn-p" onclick="openCarousel()">⟳ AGGIORNA</button>'
    );
    h +=
      '<div class="panel-sub">Le modifiche vengono salvate istantaneamente in KV e applicate al reload del sito.</div>';
...
function _carBtnsVal(idx, covers) {
  try {
    return JSON.parse(covers[SLIDES_DEF[idx].key + '_btns'] || 'null');
  } catch (e) {
    return null;
  }
}
function _carBtnHtml(idx, bi, b) {
  return (
    ...
    '-' +
    bi +
    '" class="in" placeholder="Etichetta bottone" value="' +
    escAttr(b.label !== undefined ? b.label : b.defLabel) +
    '">' +
    '<input id="cs-bth-' +
    idx +
    '-' +
    bi +
    '" class="in" placeholder="https://… o onclick…" value="' +
    escAttr(b.href !== undefined ? b.href : b.defHref) +
    '">' +
    '<button type="button" class="btn btn-d btn-sm" title="Rimuovi bottone" onclick="carDelBtn(' +
    idx +
    ',' +
    bi +
    ')">✕</button>' +
...
function _carSlideHtml(idx, covers) {
  ...
  var meta = {};
  try {
    meta = JSON.parse(covers[key + '_meta'] || 'null') || {};
  } catch (e) {}
  ...
  return (
    ...
    '<div class="se-head"><div class="vhi">' +
    idx +
    '</div><h3>' +
    esc(s.label) +
    '</h3>' +
    ...
    (curImg ? 'immagine ok' : 'nessuna immagine') +
    '</span></div>' +
    '<div class="se-prev" id="cs-prev-' +
    idx +
    '" style="' +
    (curImg ? "background-image:url('" + escAttr(curImg) + "')" : '') +
    '">' +
    (curImg ? '' : 'Nessuna immagine') +
    '</div>' +
    '<div class="se-grid">' +
    '<div class="fld"><label>Sfondo — URL</label><input id="cs-img-' +
    idx +
    '" class="in" value="' +
    escAttr(curImg) +
    '" oninput="document.getElementById(\'cs-prev-' +
    idx +
    "').style.backgroundImage=this.value?'url('+this.value+')':'none';document.getElementById('cs-prev-" +
    idx +
    "').textContent=this.value?'':'Nessuna immagine'\" placeholder=\"https://… o /images/…\"></div>" +
    '<div class="fld"><label>Sfondo — carica dal PC</label><div class="upload-zone" style="padding:10px"><input type="file" accept="image/*" aria-label="Carica immagine sfondo slide" onchange="carSlideUpload(event,' +
    idx +
    ')"><span class="uzi">🖼</span>Compressa e caricata su /images/</div></div>' +
    '<div class="fld"><label>Tag (piccolo sopra)</label><input id="cs-tag-' +
    idx +
    '" class="in" value="' +
    escAttr(tag) +
    '"></div>' +
    '<div class="fld"><label>Titolo (grande)</label><input id="cs-tit-' +
    idx +
    '" class="in" value="' +
    escAttr(tit) +
    '"></div>' +
    '</div>' +
    '<div class="fld" style="margin-top:10px"><label>Descrizione (paragrafo sotto il titolo)</label><textarea id="cs-desc-' +
    idx +
    '" class="in" rows="2">' +
    esc(desc) +
    '</textarea></div>' +
    ...
    '<div class="cs-btns" id="cs-btns-' +
    idx +
    '">' +
    _carBtnsHtml(idx, list) +
    '</div>' +
    '<button class="btn btn-soft btn-sm" onclick="carAddBtn(' +
    idx +
    ')">➕ Aggiungi bottone</button>' +
    '</div>' +
    '<div class="se-actions">' +
    '<button class="btn btn-p btn-sm" onclick="carSaveSlide(' +
    idx +
    ')" style="margin-right:auto">SALVA SLIDE</button>' +
    '<button class="btn btn-soft btn-sm" onclick="carSaveImg(' +
    idx +
    ')">Sfondo</button>' +
    '<button class="btn btn-soft btn-sm" onclick="carSaveText(' +
    idx +
    ')">Testi</button>' +
    '<button class="btn btn-soft btn-sm" onclick="carSaveBtns(' +
    idx +
    ')">Bottoni</button>' +
    '<button class="btn btn-d btn-sm" onclick="carRemoveImg(' +
    idx +
    ')">Rimuovi sfondo</button>' +
    '</div>' +
    '<div class="se-status" id="cs-st-' +
    idx +
    '"></div>' +
    '</div>'
  );
...
function _carStatus(idx, msg, ok) {
  ...
  if (el) {
    el.textContent = msg;
    el.className = 'se-status ' + (ok ? 'ok' : 'err');
  }
}
...
async function carSaveImg(idx) {
  ...
  if (!val && _carUploading && _carUploading.idx === idx) {
    ...
    try {
      ...
      _carStatus(idx, 'Errore upload: ' + e.message, false);
      return;
    }
  }
  if (!val) {
    _carStatus(idx, 'Inserisci un URL o carica un file', false);
    return;
  }
...
async function openCovers() {
  ...
  try {
    ...
    _coversState.pages = pages;
    _coversState.covers = covers;
    var h = viewHead(
      '🖌️',
      'Cover pagine',
      'Sfondi delle card in evidenza sulla homepage',
      '<button class="btn btn-p" onclick="openCovers()">⟳ AGGIORNA</button>'
    );
    h +=
      '<div class="panel"><div class="panel-head"><h3>Pagine</h3><span class="hint">' +
      pages.length +
      ' nel registro</span></div>';
    if (!pages.length) h += '<div class="empty"><span class="ei">🧭</span>Nessuna pagina nel registro data.js</div>';
    pages.forEach(function (p) {
      var cur = covers[p.id] || '';
      h +=
        '<div class="cover-row">' +
        '<div class="cover-thumb" id="cvt-' +
        esc(p.id) +
        '" style="' +
        (cur ? "background-image:url('" + escAttr(cur) + "')" : '') +
        '">' +
        (cur ? '' : '🖼') +
        '</div>' +
        '<div class="cmain"><div class="ct">' +
        esc(p.i) +
        ' ' +
        esc(p.l) +
        '</div>' +
        ...
        '</div></div>' +
        '<div class="ract">' +
        '<button class="btn btn-soft btn-sm" onclick="openCoverModal(\'' +
        escJsAttr(p.id) +
        "','" +
        escJsAttr(p.l) +
        "','" +
...
async function _getDataPages() {
  ...
  var arr = [];
  s.slice(start + 14, end)
    .split('\n')
    .forEach(function (line) {
      var m = line.match(/^\s*\{\s*k:\s*(?:"([^"]+)"|'([^']+)')/);
      if (!m) return;
      var obj = { k: m[1] || m[2] };
      var lm = line.match(/l:\s*(?:"([^"]*)"|'([^']*)')/);
      obj.l = lm ? lm[1] || lm[2] : obj.k;
      var im = line.match(/i:\s*(?:"([^"]*)"|'([^']*)')/);
      obj.i = im ? im[1] || im[2] : '📄';
      var idm = line.match(/id:\s*(?:"([^"]*)"|'([^']*)')/);
      obj.id = idm ? idm[1] || idm[2] : '';
      arr.push(obj);
    });
  return arr;
}
...
function openCoverModal(pageId, label, cur) {
  ...
  modalHtml(
    'cv-modal',
    '🖌️ Cover — ' + esc(label),
    '<div class="fld"><label>URL immagine</label><input id="cv-url" class="in" value="' +
      escAttr(cur) +
      "\" oninput=\"document.getElementById('cv-prev').style.backgroundImage=this.value?'url('+this.value+')':''\"></div>" +
      ...
      '<div class="cover-thumb" id="cv-prev" style="width:100%;height:120px;' +
      (cur ? "background-image:url('" + escAttr(cur) + "')" : '') +
      '">' +
      (cur ? '' : 'Nessuna immagine') +
      '</div>' +
...
function coverFile(event) {
  ...
  reader.onload = function (e) {
    compressImg(e.target.result, function (b64) {
      ...
      if (st) {
        st.textContent = 'Immagine pronta: premi SALVA';
...
async function saveCover() {
  ...
  try {
    if (!val && _coverUploading) {
      if (st) {
        st.textContent = 'Caricamento su GitHub…';
        st.className = 'md-status';
      }
      val = await arcUploadImage(
        _coverUploading,
        'cover-' + _coverCur.id.replace(/\W+/g, '-').slice(0, 40) + '-' + Date.now() + '.jpg'
      );
      _coverUploading = null;
    }
    if (!val) {
      if (st) {
        st.textContent = 'Inserisci un URL o carica un file';
    ...
    if (st) {
      st.textContent = ok ? '✓ Salvata' : '✕ Errore';
      st.className = 'md-status ' + (ok ? 'ok' : 'err');
    }
    if (ok) {
      closeModal('cv-modal');
  ...
  try {
    ...
    s.slice(start + 14, end)
      ...
      .forEach(function (line) {
        ...
        var idm = line.match(/id:\s*(?:"([^"]*)"|'([^']*)')/);
        obj.id = idm ? idm[1] || idm[2] : '';
        var sm = line.match(/sec:\s*(?:"([^"]*)"|'([^']*)')/);
        obj.sec = sm ? sm[1] || sm[2] : '';
        var um = line.match(/sub:\s*(?:"([^"]*)"|'([^']*)')/);
        obj.sub = um ? um[1] || um[2] : '';
        var pm = PAGES.find(function (p) {
          return p.k === obj.k;
        });
        if (pm) {
          obj.c = pm.c;
...
function _sectionLabel(v) {
  var s = SECTIONS.find(function (x) {
    return x.v === v;
  });
  return v ? s.l : v;
}
...
function _renderNav() {
  ...
  var h = viewHead(
    '🧭',
    'Navigazione',
    'Ordine, sezioni e voci del menu del sito',
  ...
  h +=
    '<div class="panel-sub">Le frecce riordinano le card della home e il menu. Cambiare sezione aggiorna l\'URL pulito delle pagine personalizzate; la sottosezione serve solo a raggruppare le voci nel menu.</div>';
  secKeys.forEach(function (v) {
    var items = groups[v];
    h +=
      '<div class="panel"><div class="panel-head"><h3>' +
      esc(_navGroupLabel(v)) +
      (v ? ' <span class="pill gold">' + esc(v) + '</span>' : '') +
      '</h3><span class="hint">' +
      items.length +
      ' voci</span></div>';
    var subs = {};
    ...
    subKeys.forEach(function (sub) {
      ...
      if (sub) {
        h +=
          '<div class="navm nav-sub" style="padding:8px 12px 2px 12px;border-bottom:none">' +
          '<div style="display:flex;align-items:center;gap:8px">' +
          '<span style="font-size:11px">🗂</span>' +
          '<span class="pill" style="background:rgba(200,155,60,.1);color:var(--gold)">' +
          esc(sub) +
          '</span>' +
      ...
      subItems.forEach(function (p, i) {
        ...
        h +=
          ...
          ' onclick="navMove(\'' +
          escJsAttr(p.k) +
          '\',-1)" title="Sposta su">↑</button>' +
          '<button class="btn btn-soft btn-sm" ' +
          (last ? 'disabled' : '') +
          ' onclick="navMove(\'' +
          escJsAttr(p.k) +
          '\',1)" title="Sposta giù">↓</button>' +
          '<select class="in" style="width:auto;padding:5px 8px;font-size:11px" onchange="navSetSec(\'' +
          escJsAttr(p.k) +
          '\',this.value)">' +
          _secOptions(p.sec) +
          '</select>' +
          '<select class="in" style="width:auto;padding:5px 8px;font-size:11px" onchange="navSetSub(\'' +
          escJsAttr(p.k) +
          '\',this.value)" title="Sottosezione">' +
          _subOptions(v, p.sub) +
          '</select>' +
          '<button class="btn btn-soft btn-sm" onclick="navOpen(\'' +
          escJsAttr(p.k) +
          '\')" title="Apri editor">✏️</button>' +
          '<button class="btn btn-soft btn-sm" onclick="navRename(\'' +
          escJsAttr(p.k) +
          '\')" title="Rinomina">🔄</button>' +
          (p.c
            ? '<button class="btn btn-d btn-sm" onclick="navDelete(\'' +
              escJsAttr(p.k) +
...
async function _writeRegistryPages(arr) {
  ...
  arr.forEach(function (x) {
    ...
    if (p) {
      p.sec = x.sec || '';
      if (x.sub !== undefined) p.sub = x.sub || '';
    }
  });
  var ordered = arr
    .map(function (x) {
    ...
    .filter(Boolean);
  var remaining = (reg.pages || []).filter(function (p) {
...
function _swapInNav(a, b) {
  var idx = _navData.pages.findIndex(function (p) {
    return p.k === a;
  });
  var jdx = _navData.pages.findIndex(function (p) {
    return p.k === b;
  });
...
async function navSetSec(slug, newSec) {
  if (
    !confirm(
      'Cambiare la sezione di "' +
        slug +
        '" in "' +
        (_sectionLabel(newSec) || 'principale') +
        '"?\n\nPer le pagine personalizzate cambierà anche l\'URL pulito.'
    )
  )
    return;
  var p = _navData.pages.find(function (x) {
    return x.k === slug;
  });
  if (!p) return;
  p.sec = newSec || '';
  var pi = PAGES.find(function (x) {
    return x.k === slug;
  });
  if (pi) pi.sec = newSec || '';
...
function navOpen(slug) {
  openByType('page', slug);
}
function navRename(slug) {
  ...
  modalHtml(
    'nr-modal',
    '🔄 Rinomina — ' + esc(p.k),
    '<div class="fld"><label>Nome</label><input id="nr-label" class="in" value="' +
      escAttr(p.l) +
      "\" onkeydown=\"if(event.key==='Enter')navSaveRename('" +
      escJsAttr(p.k) +
      '\')"></div>' +
      '<div class="fld"><label>Icona (emoji)</label><input id="nr-icon" class="in" value="' +
      escAttr(p.i) +
      '"></div>',
    '<button class="btn btn-soft" onclick="closeModal(\'nr-modal\')">Annulla</button>' +
      '<button class="btn btn-p" onclick="navSaveRename(\'' +
      escJsAttr(p.k) +
      '\')">RINOMINA</button>'
  );
...
async function navDelete(slug) {
  ...
  if (!p || !p.c) {
    toast('Le pagine predefinite si eliminano dal loro editor', 'error');
    return;
  }
  if (
    !confirm(
      'Eliminare definitivamente la pagina "' +
        p.l +
        '"?\n\nVerranno rimossi contenuto, voce del menu e URL dedicato. Operazione irreversibile.'
    )
  )
    return;
  ...
  try {
    ...
    for (var j = 0; j < _navData.pages.length; j++) {
      if (_navData.pages[j].k === slug) {
        _navData.pages.splice(j, 1);
        break;
      }
    }
    buildSidebar();
...
function _navCtxOf(line) {
  for (var i = 0; i < _NAV_CTX.length; i++) {
    if (line.indexOf(_NAV_CTX[i][0]) !== -1) return _NAV_CTX[i];
  }
  return ['altro', 'Altro'];
}
...
async function _fetchDataJsPages() {
  ...
  s.slice(start, end)
    ...
    .forEach(function (line) {
      ...
      obj.l = lm ? lm[1] || lm[2] : obj.k;
      var idm = line.match(/id:\s*(?:"([^"]*)"|'([^']*)')/);
      obj.id = idm ? idm[1] || idm[2] : 'pag-' + obj.k;
...
async function openNavAudit() {
  ...
  try {
    ...
    Object.keys(byId).forEach(function (id) {
      ...
      if (!k) {
        if (id.indexOf('pag-') === 0) {
          dead.push({ id: id, list: list });
          return;
        }
        unknownLegacy += list.length;
        okCount += list.length;
        return;
      }
      okCount += list.length;
      ...
      var ctxMap = {};
      list.forEach(function (o) {
        if (MENU_CTX[o.ctx]) (ctxMap[o.ctx] = ctxMap[o.ctx] || []).push(o);
      });
      Object.keys(ctxMap).forEach(function (c) {
        ctxMap[c].slice(1).forEach(function (o) {
          dup.push({ id: id, occ: o });
        });
      });
      /* etichette/iconcine incoerenti */
      var variants = {};
      list.forEach(function (o) {
        variants[o.label + '||' + o.icon] = (variants[o.label + '||' + o.icon] || 0) + 1;
      });
    ...
    redun.concat(dup).forEach(function (x) {
      delPreview[x.occ.ln] = 1;
    });
    var containers = [];
    ...
    _findContainerBlocks(linesArr).forEach(function (blk) {
      ...
      for (var li = blk.start; li <= blk.end; li++) {
        ...
        if (/gp\(|showHome\(|href=/.test(linesArr[li])) {
          hasAction = true;
          break;
        }
      }
      if (!hasAction)
        containers.push({ start: blk.start, end: blk.end, kind: blk.kind, label: _blockLabel(linesArr, blk) });
    });
    _navAudit = {
      html: html,
      sha: dHtml.sha,
...
function _divBlockEnd(lines, start) {
  ...
  for (i = start; i < lines.length; i++) {
    ...
    var opens = 0,
      closes = 0;
    _NA_DIV_OPEN.lastIndex = 0;
    while ((m = _NA_DIV_OPEN.exec(line))) opens++;
    _NA_DIV_CLOSE.lastIndex = 0;
    while ((m = _NA_DIV_CLOSE.exec(line))) closes++;
    depth += opens - closes;
    if (depth <= 0) return i;
  ...
  for (i = 0; i < lines.length; i++) {
    if (/class="(?:tn-drop|mn-section)\b/.test(lines[i])) {
      ...
      for (j = i + 1; j < Math.min(i + 4, lines.length); j++) {
        if (/class="cgrid"/.test(lines[j])) {
          end2 = _divBlockEnd(lines, j);
          break;
        }
        if (/\S/.test(lines[j]) && !/<\/div>/.test(lines[j])) break;
      }
      if (end2 > i) {
        out.push({ start: i, end: end2, kind: 'gruppo wiki' });
        i = end2;
      }
    }
  }
...
function _renderNavAudit(orphanPages) {
  ...
  var h = viewHead(
    '🩺',
    'Diagnostica navigazione',
    'Voci morte, doppioni, incoerenze e contenitori vuoti delle voci di menu statiche',
  ...
  h +=
    '<div class="panel-sub">La diagnostica confronta le voci statiche di <code>index.html</code> con le pagine registrate in <code>data.js</code>. ' +
  ...
  h +=
    ...
    '<span class="badge ' +
    (iss.dead.length ? 'badge-draft' : 'badge-idle') +
    '">' +
    iss.dead.length +
    ' pagine morte</span>' +
    '<span class="badge ' +
    (iss.redun.length ? 'badge-draft' : 'badge-idle') +
    '">' +
    iss.redun.length +
    ' doppioni</span>' +
    '<span class="badge ' +
    (iss.mismatch.length ? 'badge-draft' : 'badge-idle') +
    '">' +
    iss.mismatch.length +
    ' incoerenze</span>' +
    '<span class="badge ' +
    (iss.dup.length ? 'badge-draft' : 'badge-idle') +
    '">' +
    iss.dup.length +
    ' duplicati</span>' +
    '<span class="badge ' +
    (iss.containers.length ? 'badge-draft' : 'badge-idle') +
    '">' +
    iss.containers.length +
    ' contenitori orfani</span>' +
  ...
  if (anyFix) {
    h +=
      '<div style="padding:10px 16px"><button class="btn btn-p" id="na-apply" onclick="applyNavAuditFixes()">APPLICA CORREZIONI SELEZIONATE</button>' +
      '<span style="margin-left:10px;font-size:11px;color:var(--dim)">un unico commit su index.html</span></div>';
  }
  /* ── Morte ── */
  if (iss.dead.length) {
    h +=
      '<div class="panel"><div class="panel-head"><h3>🔴 Pagine inesistenti</h3><span class="hint">' +
      iss.dead.length +
      ' id senza pagina</span></div>';
    h +=
      '<div class="panel-sub">Queste voci di menu portano a nessuna pagina (click senza effetto o errore). Rimuovile, oppure ricrea la pagina con lo stesso indirizzo.</div>';
    iss.dead.forEach(function (d) {
      var first = d.list[0];
      var ctxs = d.list
        .map(function (o) {
          return _naCtxChip(o.ctxLabel);
        })
        .join(' ');
      var slug = d.id.replace(/^pag-/, '');
      h +=
        '<div class="row navm">' +
        '<div class="rico"><input type="checkbox" data-na-dead="' +
        escAttr(d.id) +
        '" checked title="Rimuovi tutte le voci"></div>' +
        '<div class="rmain"><div class="rt" style="color:var(--red)">' +
        esc(first.icon) +
        ' ' +
        esc(first.label) +
        '</div>' +
        '<div class="rs">id <code>' +
        esc(d.id) +
        '</code> · ' +
        d.list.length +
        ' voc' +
        ...
        ctxs +
        '</div></div>' +
        '<div class="ract"><button class="btn btn-soft btn-sm" onclick="openNewPageModal(\'' +
        escJsAttr(first.label) +
        "','" +
        escJsAttr(slug) +
        '\')">➕ Crea pagina</button></div>' +
        '</div>';
    });
    h += '</div>';
  }
  ...
  if (iss.redun.length) {
    h +=
      '<div class="panel"><div class="panel-head"><h3>🟠 Voci duplicate dal menu dinamico</h3><span class="hint">' +
      iss.redun.length +
      ' voci</span></div>';
    h +=
      '<div class="panel-sub">Le sezioni dei menu sono popolate automaticamente dal registry: queste voci statiche generano un doppione visibile. Rimuovile (la pagina resta nel menu).</div>';
    iss.redun.forEach(function (r) {
      h +=
        '<div class="row navm">' +
        '<div class="rico"><input type="checkbox" data-na-line="' +
        r.occ.ln +
        '" checked></div>' +
        '<div class="rmain"><div class="rt">' +
        esc(r.k.i) +
        ...
        esc(r.k.l) +
        '</div>' +
        '<div class="rs">voce statica ' +
        _naCtxChip(r.occ.ctxLabel) +
        ' · già presente nella sezione «' +
        esc(_sectionLabel(r.k.sec)) +
        '» del menu dinamico</div></div>' +
        '</div>';
    });
    h += '</div>';
  }
  ...
  if (iss.mismatch.length) {
    h +=
      '<div class="panel"><div class="panel-head"><h3>🟠 Etichette / icone incoerenti</h3><span class="hint">' +
      iss.mismatch.length +
      ' pagine</span></div>';
    h +=
      '<div class="panel-sub">Lo stesso id appare con testi o icone diverse. Scegli la versione corretta: verrà applicata a tutte le occorrenze.</div>';
    iss.mismatch.forEach(function (m) {
      var variants = m.variants
        .map(function (v, i) {
          var parts = v.split('||');
          return (
            '<label class="na-var"><input type="radio" name="na-mm-' +
            escAttr(m.id) +
            '" data-mm-label="' +
            escAttr(parts[0]) +
            '" data-mm-icon="' +
        ...
        .join('');
      h +=
        '<div class="row navm"><div class="rico">' +
        esc(m.list[0].icon) +
        '</div>' +
        '<div class="rmain"><div class="rt">' +
        esc(m.id) +
        '</div><div class="rs na-variants">' +
        variants +
        '</div></div></div>';
    });
    h += '</div>';
  ...
  if (iss.dup.length) {
    h +=
      '<div class="panel"><div class="panel-head"><h3>🟡 Duplicati nello stesso contesto</h3><span class="hint">' +
      iss.dup.length +
      '</span></div>';
    iss.dup.forEach(function (d) {
      h +=
        ...
        esc(d.occ.label) +
        '</div>' +
        '<div class="rs">ripetuto in ' +
        _naCtxChip(d.occ.ctxLabel) +
        '</div></div></div>';
    });
    h += '</div>';
  }
  ...
  if (iss.containers.length) {
    h +=
      '<div class="panel"><div class="panel-head"><h3>⚪ Contenitori orfani</h3><span class="hint">' +
      iss.containers.length +
      ' blocchi</span></div>';
    h +=
      '<div class="panel-sub">Dopo le correzioni selezionate questi blocchi resterebbero vuoti (dropdown senza voci, gruppi di card senza card). ' +
    ...
    iss.containers.forEach(function (c) {
      h +=
        '<div class="row navm">' +
        '<div class="rico na-check"><input type="checkbox" data-na-block="' +
        c.start +
        ':' +
        c.end +
        '" checked></div>' +
        ...
        '–' +
        (c.end + 1) +
        " · nessuna voce valida all'interno</div></div>" +
        '</div>';
    });
    h += '</div>';
  }
  ...
  h +=
    '<div class="panel"><div class="panel-head"><h3>ℹ️ Informativa</h3></div><div class="panel-sub" style="padding:0 14px 12px">' +
    st.ok +
    ' voci valide · ' +
    st.legacy +
    ' id legacy non verificabili · ' +
    st.orphans.length +
    ' pagine registrate senza voce statica' +
    (st.orphans.length
      ? ' (raggiungibili dai menu dinamici e dalla ricerca: ' +
        esc(
          st.orphans
            .slice(0, 6)
            .map(function (p) {
...
async function applyNavAuditFixes() {
  ...
  var nDead = Object.keys(deadSel).length,
    nLines = Object.keys(lineDel).length,
    nBlocks = blockDel.length,
    nMm = Object.keys(mmSel).length;
  if (!nDead && !nLines && !nBlocks && !nMm) {
    toast('Seleziona almeno una correzione', 'error');
    return;
  }
  ...
  try {
    ...
    iss.redun.concat(iss.dup).forEach(function (x) {
      if (lineDel[x.occ.ln]) delSet[x.occ.ln] = 1;
    });
    blockDel.forEach(function (b) {
      for (var i = b.s; i <= b.e; i++) delSet[i] = 1;
    });
    var kept = [];
    lines.forEach(function (l, i) {
      if (!delSet[i]) kept.push(l);
    });
    var html = kept.join('\n');
...
async function openSettings() {
  ...
  var h = viewHead(
    '⚙️',
    'Impostazioni',
    'Configurazione di sezioni, editor e deploy',
    '<button class="btn btn-p" onclick="openSettings()">⟳ AGGIORNA</button>'
  );
  h +=
    '<div class="panel"><div class="panel-head"><h3>Sezioni del menu</h3><span class="hint">usate per raggruppare pagine e generare gli URL</span></div>' +
  ...
  SECTIONS.forEach(function (s, i) {
    h +=
      '<div class="kv-row" id="sec-row-' +
      i +
      '">' +
      '<div class="kv-main"><input id="sec-l-' +
      i +
      '" class="in" value="' +
      escAttr(s.l) +
      '" placeholder="Etichetta"></div>' +
      '<input id="sec-v-' +
      i +
      '" class="in" style="width:170px;font-family:var(--mono)" value="' +
      escAttr(s.v) +
      '" placeholder="valore URL">' +
      '<button class="btn btn-soft btn-sm" title="Sposta su" onclick="moveSection(' +
      i +
      ',-1)">↑</button>' +
      '<button class="btn btn-soft btn-sm" title="Sposta giù" onclick="moveSection(' +
      i +
      ',1)">↓</button>' +
      '<button class="btn btn-d btn-sm" title="Elimina sezione" onclick="delSection(' +
      i +
      ')">✕</button>' +
      '</div>';
  });
  ...
  h +=
    '<div class="panel"><div class="panel-head"><h3>Editor pagine</h3><span class="hint">preferenze salvate in questo browser</span></div>' +
    ...
    '<option value="md"' +
    (defaultView === 'md' ? ' selected' : '') +
    '>Solo Markdown</option>' +
    '<option value="pv"' +
    (defaultView === 'pv' ? ' selected' : '') +
    '>Solo anteprima</option>' +
    '</select></div></div>' +
  ...
  h +=
    '<div class="panel"><div class="panel-head"><h3>Banner del sito</h3><span class="hint">messaggio mostrato prima di entrare nel sito</span></div>' +
    '<div class="kv-rows">' +
  ...
  h +=
    '<div class="panel"><div class="panel-head"><h3>Disclaimer / Progetto fanmade</h3><span class="hint">overlay mostrato prima di entrare nel sito</span></div>' +
    '<div class="kv-rows">' +
  ...
  h +=
    '<div class="panel"><div class="panel-head"><h3>Notifiche Discord</h3><span class="hint">webhook per notifiche automatiche</span></div>' +
    '<div class="kv-rows">' +
  ...
  h +=
    '<div class="panel"><div class="panel-head"><h3>Statistiche</h3><span class="hint">panoramica visitatori</span></div>' +
    '<div id="analytics-body"><div class="empty" style="padding:22px"><span class="ei">⏳</span>Caricamento…</div></div></div>';
  h += '<div class="grid-2">';
  h +=
    '<div class="panel"><div class="panel-head"><h3>Repository</h3></div>' +
    '<div class="kv-rows">' +
    '<div class="kv-row"><div class="kv-main"><div class="kt">GitHub</div><div class="ks">' +
    esc(GH_REPO) +
    ' · ramo ' +
    esc(GH_BRANCH) +
    '</div></div></div>' +
    ...
    '</div></div>';
  h +=
    '<div class="panel"><div class="panel-head"><h3>Deploy</h3></div>' +
    '<div class="kv-rows">' +
...
async function refreshGHTokenStatus() {
  ...
  try {
    ...
    var j = await r.json();
    if (j.configured)
      el.textContent = j.source === 'env' ? "Configurato (variabile d'ambiente)" : 'Configurato (server)';
    else el.textContent = 'Non configurato — il proxy GitHub non funziona';
  } catch (e) {
    el.textContent = 'non verificabile';
  }
}
...
async function saveBanner() {
  ...
  try {
    var r = await _authPost('/api/admin?action=set_site_settings', {
      settings: { banner_enabled: bOn.checked, banner_text: bTxt.value.trim() },
    });
    var j = await r.json();
...
async function saveDisclaimer() {
  ...
  try {
    var r = await _authPost('/api/admin?action=set_site_settings', {
      settings: { disclaimer_enabled: dOn.checked, disclaimer_text: dTxt.value.trim() },
    });
    var j = await r.json();
    if (!r.ok) throw new Error(j.error || 'Errore ' + r.status);
    if (st) {
...
async function saveGhToken() {
  ...
  if (!token) {
    if (st) {
      st.textContent = 'Inserisci il token';
      st.className = 'md-status err';
    }
    return;
  }
...
async function pingDeployStatus() {
  ...
  try {
    ...
    el.textContent = 'Endpoint non raggiungibile';
  }
}
...
function addSection() {
  ...
  rows.insertAdjacentHTML(
    ...
    '<div class="kv-row" id="sec-row-' +
      ...
      '<div class="kv-main"><input id="sec-l-' +
      i +
      '" class="in" value="" placeholder="Etichetta sezione"></div>' +
      '<input id="sec-v-' +
      i +
      '" class="in" style="width:170px;font-family:var(--mono)" value="" placeholder="valore URL">' +
      '<button class="btn btn-soft btn-sm" title="Sposta su" onclick="moveSection(' +
      i +
      ',-1)">↑</button>' +
      '<button class="btn btn-soft btn-sm" title="Sposta giù" onclick="moveSection(' +
...
async function saveSections() {
  ...
  for (var i = 0; i < els.length; i++) {
    ...
    if (first) {
      rows.push({ v: v, l: l || '— Nessuna (solo URL) —' });
      first = false;
      continue;
    }
    if (!l) continue;
    if (!/^[a-z0-9]*$/.test(v)) {
      toast('Valore sezione non valido: solo minuscole e numeri', 'error');
      return;
    }
    rows.push({ v: v, l: l });
  }
  ...
  try {
    ...
    if (regOut !== b64decode(d0.content))
      await ghPut('content/pages/registry.json', 'admin: update sections', regOut, d0.sha);
    SECTIONS.length = 0;
...
async function testWebhook() {
  ...
  try {
    ...
    if (st) {
      st.textContent = '✓ Messaggio inviato! Controlla Discord';
      st.className = 'md-status ok';
    }
    setTimeout(function () {
      if (st) st.textContent = '';
...
async function loadAnalytics() {
  ...
  try {
    ...
    if (!box) return;
    if (!pages.length) {
      box.innerHTML = '<div class="empty" style="padding:22px"><span class="ei">📊</span>Nessun dato disponibile</div>';
      return;
    }
    var labelById = {};
    ((window.ArcAdmin && window.ArcAdmin.pages) || []).forEach(function (p) {
      if (p.id) labelById[p.id] = p;
      if (p.k) labelById[p.k] = p;
    });
    var h =
      '<div class="kv-rows">' +
      '<div class="kv-row"><div class="kv-main"><div class="kt">Visualizzazioni totali</div><div class="ks">' +
    ...
    top.forEach(function (p, i) {
      ...
      h +=
        ...
        '<div class="act-main"><div class="act-t"><b>#' +
        (i + 1) +
        '</b> ' +
        esc(meta.i || '📄') +
        ' ' +
        esc(meta.l || p.pageKey) +
        '</div>' +
        '<div class="act-s">' +
        esc(p.pageKey || '') +
        ' · ' +
        (p.views || 0) +
        ' visualizzazioni</div></div></div>';
    });
    box.innerHTML = h;
  } catch (e) {
    if (box)
      box.innerHTML =
        '<div class="empty" style="padding:22px"><span class="ei">⚠️</span>Statistiche non disponibili</div>';
  }
}
...
function _imgRenderGrid(items) {
  ...
  var html = '';
  for (var i = _imgPageOffset; i < end; i++) {
    html += _imgCard(items[i]);
  }
  ...
  if (end < items.length) {
    ...
    var btn =
      ...
      Math.min(remaining, _imgPageSize) +
      ' immagini (' +
      remaining +
      ' rimaste)</button></div>';
