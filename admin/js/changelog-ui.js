...
async function openChangelog() {
  ...
  h +=
    '<div class="panel-sub" style="margin-bottom:10px">Le versioni sono raggruppate per Versione → Sottoversione → Patch. Il contenuto è Markdown.</div>';
  h +=
    '<div class="panel"><div class="panel-head"><h3>Entry (' +
    _clData.length +
    ')</h3><span class="hint">ordinate per versione crescente nel sito</span></div>';
  ...
  if (!_clData.length) {
    ...
    var sorted = _clData
      ...
      .sort(function (a, b) {
        return String(b.versione + '.' + b.sottoversione).localeCompare(
          String(a.versione + '.' + a.sottoversione),
          undefined,
          { numeric: true }
        );
      });
    ...
    sorted.forEach(function (e) {
      ...
      h +=
        ...
        '<div class="rico">' +
        esc(_clVersionBadge(e)) +
        '</div>' +
        '<div class="rmain"><div class="rt">' +
        esc(e.title || '(senza titolo)') +
        '</div>' +
        '<div class="rs">v' +
        esc([e.versione, e.sottoversione, e.patch].filter(Boolean).join('.')) +
        ' · ' +
        esc(e.date || 'senza data') +
        ' · ' +
        (hasContent ? '✓ contenuto' : '⚠ senza contenuto') +
        '</div></div>' +
        '<div class="ract">' +
        '<button class="btn btn-soft btn-sm" onclick="_clEdit(' +
        e._i +
        ')">✎</button>' +
        '<button class="btn btn-d btn-sm" onclick="_clDelete(' +
        e._i +
        ')">🗑</button>' +
        '</div></div>';
    });
...
function _clEdit(idx) {
  ...
  var body =
    ...
    '<div class="grid-3">' +
    '<div class="fld"><label>Versione *</label><input id="clf-v" class="in" value="' +
    escAttr(e.versione || '') +
    '" placeholder="1"></div>' +
    '<div class="fld"><label>Sottoversione</label><input id="clf-sv" class="in" value="' +
    escAttr(e.sottoversione || '') +
    '" placeholder="0"></div>' +
    '<div class="fld"><label>Patch</label><input id="clf-p" class="in" value="' +
    escAttr(e.patch || '') +
    '" placeholder="1"></div>' +
    '</div>' +
    '<div class="grid-2">' +
    '<div class="fld"><label>Titolo *</label><input id="clf-t" class="in" value="' +
    escAttr(e.title || '') +
    '" placeholder="Titolo dell\'aggiornamento"></div>' +
    '<div class="fld"><label>Data</label><input id="clf-d" type="date" class="in" value="' +
    escAttr(e.date || '') +
    '"></div>' +
    '</div>' +
  ...
  var actions =
    (idx >= 0
      ? '<button class="btn btn-d" onclick="_clDelete(' + idx + ')">🗑 Elimina</button><div style="flex:1"></div>'
  ...
  modalHtml(
    'clf-modal',
    idx >= 0 ? '✎ Entry v' + [e.versione, e.sottoversione, e.patch].filter(Boolean).join('.') : '➕ Nuova entry',
    body,
    actions,
    'md-wide'
  );
...
async function _clSave() {
  ...
  if (!versione) {
    toast('La versione è obbligatoria', 'error');
    return;
  }
  if (!title) {
