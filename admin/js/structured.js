...
function _stCanUndo() {
  return _stHistoryIdx > 0;
}
function _stCanRedo() {
  return _stHistoryIdx < _stHistory.length - 1;
}
...
function _stPanWord() {
  return _stPanGeneric() ? 'elemento' : 'divinità';
}
function _stPanWordCap() {
  var w = _stPanWord();
  return w.charAt(0).toUpperCase() + w.slice(1);
}
function _stPanHeadDefault() {
  return _stPanGeneric() ? 'Collezione' : 'Divinità';
}
function _stPanKvTitle() {
  return _stPanGeneric() ? 'Dettagli' : 'Identità';
}
function _stPanEmptyIcon() {
  return _stPanGeneric() ? '🗂️' : '🛐';
}
...
function _stParsePantheon(md) {
  ...
  blocks.forEach(function (block) {
    ...
    function flush() {
      var t = buf.join('\n').trim();
      var f = _stFold(sec);
      if (f === 'identita' || f === 'dettagli') {
        ident = _stParseIdent(t);
      } else if (!_stPanGeneric() && f === 'personalita') {
        personality = t;
      } else if (!_stPanGeneric() && f === 'culto') {
        cult = t;
...
function _stParseSchede(md) {
  ...
  var shortFieldDefs = fieldDefs.filter(function (f) {
    return f.type !== 'wysiwyg';
  });
  var wysiwygFieldDefs = fieldDefs.filter(function (f) {
    return f.type === 'wysiwyg';
  });
  var cards = [];
...
function _stParsePage(md) {
  ...
  var fieldDefs = (reg && reg.fields) || [];
  var shortFields = fieldDefs.filter(function (f) {
    return f.type !== 'wysiwyg';
  });
  ...
  var wysiwygFields = fieldDefs.filter(function (f) {
    return f.type === 'wysiwyg';
  });
...
function _stEpi(c) {
  for (var i = 0; i < (c.ident || []).length; i++)
    if (/^(epiteto|sottotitolo)$/i.test(c.ident[i][0])) return c.ident[i][1];
  return '';
}
function _stMd(md) {
  if (!md) return '';
...
function _stPreviewHTML(data) {
  ...
  data.cards.forEach(function (c) {
    if (!c.img) {
      h +=
        '<div class="pan-section"><div class="pan-section-title">' +
        esc(c.name) +
        '</div>' +
        '<div class="pan-section-body">' +
        _stMd(c.extra) +
        '</div></div>';
      return;
    }
    var epi = _stEpi(c);
    h +=
      '<div class="pan-tile" style="cursor:default">' +
      '<img class="pan-tile-img" src="' +
      escAttr(c.img) +
      '" alt="' +
      escAttr(c.name) +
      '" loading="lazy"' +
      ...
      '>' +
      '<div class="pan-tile-caption"><span class="pan-tile-name">' +
      esc(c.name) +
      '</span>' +
      (epi ? '<span class="pan-tile-epi">' + esc(epi) + '</span>' : '') +
      '</div></div>';
  });
  h += '</div></div>';
  return h;
...
function _stPreviewSchede(data) {
  ...
  (data.cards || []).forEach(function (c) {
    h +=
      '<div class="sch-card" style="margin-bottom:16px;border:1px solid var(--line);border-radius:var(--rad);padding:14px">';
    h +=
      '<div class="sch-title" style="font-family:var(--brand);color:var(--acc2);font-size:17px;font-weight:700">' +
      esc(c.name) +
      '</div>';
    if (c.intro) h += '<div class="sch-intro" style="margin:8px 0">' + _stMd(c.intro) + '</div>';
    /* Structured fields */
    if (c.fields && typeof c.fields === 'object' && !Array.isArray(c.fields)) {
      var hasFields = shortFieldDefs.some(function (f) {
        return c.fields[f.key];
      });
      if (hasFields) {
        ...
        shortFieldDefs.forEach(function (f) {
          ...
          if (val)
            h +=
              '<div class="sch-field"><span class="sch-key" style="color:var(--acc2);font-weight:700">' +
              esc(f.label) +
              '</span> ' +
              '<span class="sch-val">' +
              esc(val) +
    ...
    (c.blocks || []).forEach(function (b) {
      h +=
        '<div class="sch-block-title" style="font-weight:700;color:var(--acc);margin-top:10px">' +
        esc(b.name) +
        '</div>';
...
function _stCardHTML(c, i) {
  ...
  if (!c.img && !c.deity) {
    return (
      '<div class="st-card st-card-sec" data-i="' +
      i +
      '" draggable="true"' +
      'ondragstart="_stDragStart(event,' +
      i +
      ')" ondragend="_stDragEnd(event)"' +
      'ondragover="_stDragOver(event,' +
      i +
      ')" ondragleave="_stDragLeave(event,' +
      i +
      ')"' +
      'ondrop="_stDrop(event,' +
      i +
      ')">' +
      '<div class="st-card-head"><span class="st-drag-handle" title="Trascina per riordinare" ontouchstart="_stTouchDragStart(event,' +
      i +
      ')">⠿</span>' +
      '<span class="st-idx">' +
      (i + 1) +
      '</span>' +
      '<input class="in st-name" value="' +
      escAttr(c.name) +
      '" placeholder="Nome della sezione" oninput="_stSync()">' +
      '<span class="st-head-actions">' +
      '<button type="button" class="btn btn-soft btn-icon btn-sm" title="Trasforma in ' +
      _stEntLabel() +
      ' (aggiungi immagine)" onclick="_stPick(' +
      i +
      ')">🖼</button>' +
      '<button type="button" class="btn btn-soft btn-icon btn-sm st-collapse" title="Comprimi/espandi" onclick="_stCollapse(' +
      i +
      ')">▾</button>' +
      '<button type="button" class="btn btn-soft btn-icon btn-sm" title="Duplica blocco" onclick="_stDuplicate(' +
      i +
      ')">⧉</button>' +
      '<button type="button" class="btn btn-soft btn-icon btn-sm" title="Sposta su" onclick="_stMove(' +
      i +
      ',-1)">↑</button>' +
      '<button type="button" class="btn btn-soft btn-icon btn-sm" title="Sposta giù" onclick="_stMove(' +
      i +
      ',1)">↓</button>' +
      '<button type="button" class="btn btn-d btn-icon btn-sm" title="Elimina blocco" onclick="_stDelete(' +
      i +
      ')">🗑</button>' +
      '</span></div>' +
      '<div class="st-card-body">' +
      '<label class="st-ta-l">Contenuto della sezione (markdown)<textarea class="in st-ta st-extra" oninput="_stSync()" placeholder="Scrivi qui la sezione…">' +
      esc(c.extra) +
      '</textarea></label>' +
      '</div></div>'
    );
  ...
  var rows = (c.ident && c.ident.length ? c.ident : _stPanNewRows())
    .map(function (p) {
      return (
        '<div class="st-kv"><input class="in k" value="' +
        escAttr(p[0]) +
        '" placeholder="Campo" oninput="_stSync()">' +
        '<input class="in v" value="' +
        escAttr(p[1]) +
        '" placeholder="Valore" oninput="_stSync()">' +
        '<button type="button" class="btn btn-d btn-icon btn-sm" title="Rimuovi campo" onclick="_stDelRow(this)">✕</button></div>'
      );
    })
    .join('');
  return (
    '<div class="st-card" data-i="' +
    i +
    '" draggable="true"' +
    'ondragstart="_stDragStart(event,' +
    i +
    ...
    '<input class="in st-name" value="' +
    escAttr(c.name) +
    '" placeholder="Nome della ' +
    _stPanWord() +
    ' / sezione" oninput="_stSync()">' +
    '<span class="st-head-actions">' +
    '<button type="button" class="btn btn-soft btn-icon btn-sm" title="Trasforma in sezione (rimuovi immagine)" onclick="_stConvertToSection(' +
    i +
    ')">➖</button>' +
    '<button type="button" class="btn btn-soft btn-icon btn-sm st-collapse" title="Comprimi/espandi" onclick="_stCollapse(' +
    i +
    ...
    '</span></div>' +
    '<div class="st-card-body">' +
    '<div class="st-img-wrap" id="st-imgwrap-' +
    i +
    '"' +
    (c.img
      ? ' title="Trascina per scegliere la parte visibile nella card" onmousedown="_stPosDown(event,' + i + ')"'
      : '') +
    '>' +
    (c.img
      ? '<img class="st-img" id="st-img-' +
        i +
        '" src="' +
        escAttr(c.img) +
        '" alt="' +
    ...
    '<div class="st-pos-mark" id="st-posmark-' +
    i +
    '" style="left:' +
    (c.pos ? c.pos[0] : 50) +
    '%;top:' +
    ...
    '"></div>' +
    '</div>' +
    '<input type="hidden" class="st-imgval" id="st-imgval-' +
    i +
    '" value="' +
    escAttr(c.img || '') +
    '">' +
    '<input type="hidden" class="st-posval" id="st-posval-' +
    i +
    '" value="' +
    (c.pos ? c.pos[0] + ',' + c.pos[1] : '') +
    '">' +
    '<div class="st-img-actions">' +
    '<button type="button" class="btn btn-soft btn-sm" onclick="_stPick(' +
    i +
    ')">🖼 Scegli</button>' +
    '<button type="button" class="btn btn-soft btn-sm" onclick="_stClearImg(' +
    i +
    ')">✕</button>' +
    '<span class="st-pos-hint">Trascina sull\'immagine per il ritaglio</span>' +
    '</div>' +
    '<label class="st-ta-l">Citazione<input class="in st-quote" value="' +
    escAttr(c.quote) +
    '" placeholder="Frase rappresentativa…" oninput="_stSync()"></label>' +
    '<p class="st-hint">Senza immagine il blocco diventa una sezione a tutta larghezza' +
    (_stPanGeneric() ? '.' : ' (es. gli Aspetti).') +
    '</p>' +
    '<div class="st-sub">' +
    _stPanKvTitle() +
    '</div>' +
    '<div class="st-kvrows" id="st-ident-' +
    i +
    '">' +
    rows +
    '</div>' +
    '<button type="button" class="btn btn-soft btn-sm" onclick="_stAddIdent(' +
    i +
    ')">＋ campo ' +
    _stPanKvTitle().toLowerCase() +
    '</button>' +
    (_stPanGeneric()
      ? ''
      : '<label class="st-ta-l">Personalità<textarea class="in st-ta st-personality" oninput="_stSync()">' +
        esc(c.personality) +
        '</textarea></label>' +
        '<label class="st-ta-l">Culto<textarea class="in st-ta st-cult" oninput="_stSync()">' +
        esc(c.cult) +
        '</textarea></label>') +
    '<label class="st-ta-l">Contenuto aggiuntivo (markdown)<textarea class="in st-ta st-extra" oninput="_stSync()">' +
    esc(c.extra) +
    '</textarea></label>' +
    '</div></div>'
  );
...
function _stSchedeCardHTML(c, i) {
  ...
  shortFieldDefs.forEach(function (f) {
    ...
    if (f.type === 'select') {
      var opts = (f.options || [])
        ...
        .join('');
      fieldsHtml +=
        '<label class="st-ta-l st-field-' +
        f.key +
        '">' +
        esc(f.label) +
        '<select class="in cms-select" data-field-key="' +
        escAttr(f.key) +
        '" onchange="_stSync()">' +
      ...
      fieldsHtml +=
        ...
        '">' +
        esc(f.label) +
        '<input class="in cms-input" type="number" data-field-key="' +
        escAttr(f.key) +
        '" value="' +
        escAttr(val) +
        '" oninput="_stSync()" placeholder="' +
        escAttr(f.placeholder || '0') +
        '"></label>';
      ...
      fieldsHtml +=
        ...
        '">' +
        esc(f.label) +
        '<div style="display:flex;gap:6px"><input class="in cms-input" data-field-key="' +
        escAttr(f.key) +
        '" value="' +
        escAttr(val) +
        '" oninput="_stSync()" placeholder="/images/…">' +
        '<button type="button" class="btn btn-soft btn-sm" onclick="_stPagePickImage(\'' +
        escAttr(f.key) +
        '\')">🖼</button></div></label>';
    } else {
      fieldsHtml +=
        ...
        '">' +
        esc(f.label) +
        '<input class="in cms-input" data-field-key="' +
        escAttr(f.key) +
        '" value="' +
        escAttr(val) +
        '" oninput="_stSync()" placeholder="' +
  ...
  wysiwygFieldDefs.forEach(function (f) {
    ...
    sectionsHtml +=
      ...
      '">' +
      esc(f.label) +
      '<textarea class="in st-ta cms-wysiwyg" data-field-key="' +
      escAttr(f.key) +
      '" oninput="_stSync()" placeholder="' +
      escAttr(f.placeholder || 'Scrivi ' + f.label.toLowerCase() + '…') +
      '">' +
      esc(val) +
      '</textarea>' +
  ...
  if (c.legacyFields && c.legacyFields.length) {
    legacyHtml =
      '<div class="st-sub">Campi aggiuntivi</div>' +
      '<div class="st-kvrows" id="st-ident-' +
      i +
      '">' +
      c.legacyFields
  ...
  var blocks = (c.blocks || [])
    .map(function (b, j) {
      return (
        '<div class="st-block">' +
        '<div class="st-block-head"><input class="in st-bname" value="' +
        escAttr(b.name) +
        '" placeholder="Titolo sezione" oninput="_stSync()">' +
        '<button type="button" class="btn btn-d btn-icon btn-sm" title="Rimuovi sezione" onclick="_stDelBlock(this)">✕</button></div>' +
        '<textarea class="in st-ta st-bmd" oninput="_stSync()" placeholder="Contenuto (markdown)">' +
        esc(b.md) +
        '</textarea>' +
        '</div>'
      );
  ...
  return (
    ...
    '<input class="in st-name" value="' +
    escAttr(c.name) +
    '" placeholder="Titolo della scheda" oninput="_stSync()">' +
    '<span class="st-head-actions">' +
    '<button type="button" class="btn btn-soft btn-icon btn-sm st-collapse" title="Comprimi/espandi" onclick="_stCollapse(' +
    i +
    ')">▾</button>' +
    '<button type="button" class="btn btn-soft btn-icon btn-sm" title="Duplica scheda" onclick="_stDuplicate(' +
    i +
    ')">⧉</button>' +
    '<button type="button" class="btn btn-soft btn-icon btn-sm" title="Sposta su" onclick="_stMove(' +
    i +
    ...
    i +
    ',1)">↓</button>' +
    '<button type="button" class="btn btn-d btn-icon btn-sm" title="Elimina scheda" onclick="_stDelete(' +
    i +
    ')">🗑</button>' +
    '</span></div>' +
    '<div class="st-card-body">' +
    '<label class="st-ta-l">Descrizione / intro<textarea class="in st-ta st-intro" oninput="_stSync()" placeholder="Breve descrizione della scheda…">' +
    esc(c.intro) +
    '</textarea></label>' +
...
function _stRead() {
  ...
  document.querySelectorAll('#st-list .st-card').forEach(function (c, idx) {
    ...
    cards.push({
      type: isDeity ? 'deity' : 'section',
      deity: isDeity || undefined,
      name: name,
      img: img,
      pos: pos,
      quote: (function (q) {
        return q ? q.value.trim() : '';
      })(c.querySelector('.st-quote')),
      ident: ident,
      personality: (function (t) {
        return t ? t.value : '';
      })(c.querySelector('.st-personality')),
      cult: (function (t) {
        return t ? t.value : '';
      })(c.querySelector('.st-cult')),
      extra: (function (t) {
        return t ? t.value : '';
      })(c.querySelector('.st-extra')),
    });
  });
  return { heading: heading, intro: intro, cards: cards };
}
...
function _stSyncPreview() {
  _stPreview(_stRead());
}

function initStructuredEditor(content) {
  _stMode = true;
  window.__stMode = true;
  ...
  if (pane) {
    ...
    if (!document.getElementById('st-grid')) {
      ...
      dg.className = 'st-grid';
      dg.id = 'st-grid';
      document.getElementById('st-list').appendChild(dg);
    }
  }
  var sh = document.getElementById('struct-head');
  if (sh) sh.style.display = '';
  var sl = document.getElementById('st-list');
  ...
  if (head) {
    if (_stKind === 'page') {
      ...
      head.innerHTML =
        ...
        '<label class="st-ta-l">Titolo introduzione<input class="in" id="st-heading" value="' +
        escAttr(data.heading) +
        '" style="max-width:220px" oninput="_stSync()"></label>' +
        '<label class="st-ta-l">Testo introduttivo (prima della griglia)<textarea class="in st-ta" id="st-intro" oninput="_stSync()" placeholder="' +
        (_stPanGeneric() ? 'Introduzione alla collezione…' : 'Introduzione al pantheon…') +
        '">' +
        esc(data.intro) +
        '</textarea></label>' +
        '</div>';
  ...
  if (ph)
    ph.innerHTML =
      'Editor <span class="ph-info" id="e-stats"></span>' +
      (_stKind !== 'page'
        ? '<input id="st-filter" class="in" placeholder="Filtra schede…" oninput="stFilter(this.value)" style="max-width:150px;margin-right:8px">' +
  ...
  if (wb && wtb && wtb.parentNode === wb && !document.getElementById('st-warn')) {
    ...
    wd.id = 'st-warn';
    wd.style.cssText =
      'display:none;margin:6px 14px 0;padding:6px 10px;border-radius:var(--rad);background:rgba(255,193,7,.12);border:1px solid rgba(255,193,7,.35);color:var(--warn,#d9a514);font-size:12px;line-height:1.6';
    wb.insertBefore(wd, wtb.nextSibling);
  }
  var tb = document.getElementById('e-toolbar');
  if (tb && tb.parentNode) {
    tb.outerHTML =
      '<div class="ed-toolbar" id="e-toolbar">' +
      '<div class="fmt-group">' +
      ...
      (_stKind === 'schede'
        ...
        : _stKind === 'page'
          ? ''
          : '<button class="btn btn-p btn-sm" onclick="_stAdd(\'deity\')">➕ ' +
            _stPanWordCap() +
            '</button>' +
...
function _stSchedeIntroHTML(data) {
  ...
  return (
    '<div class="st-intro-box">' +
    '<label class="st-ta-l">Testo introduttivo (prima delle schede)<textarea class="in st-ta" id="st-schede-intro" oninput="_stSync()" placeholder="Testo prima della prima scheda…">' +
    esc(data.intro) +
    '</textarea></label>' +
...
function _stPageFormHTML(data) {
  ...
  h +=
    '<p class="st-hint" style="margin:0 0 12px">Modifica i campi strutturati. Il contenuto viene salvato come Markdown compatibile con il sito.</p>';
...
function _stWysiwygCmd(btn, wrap, placeholder) {
  ...
  ta.setSelectionRange(
    start + wrap.length,
    sel ? start + wrap.length + sel.length : start + wrap.length + placeholder.length
  );
  ta.focus();
  _stSync();
}
...
function _stPagePickImage(fieldKey) {
  ...
  modalHtml(
    'st-pick-modal',
    '🖼 Scegli immagine',
    '<div class="fld"><label>URL diretto</label><div style="display:flex;gap:6px"><input id="st-pick-url" class="in" placeholder="/images/… oppure https://…" onkeydown="if(event.key===\'Enter\')stPagePickUrl(\'' +
      escJsAttr(fieldKey) +
      '\')">' +
      '<button class="btn btn-p btn-sm" onclick="stPagePickUrl(\'' +
      escJsAttr(fieldKey) +
      '\')">Usa</button></div></div>' +
  ...
  ghGet('images')
    .then(function (list) {
      ...
      if (!grid) return;
      if (!items.length) {
        grid.innerHTML = '<div class="list-empty">Nessuna immagine in /images/</div>';
        return;
      }
      grid.innerHTML = items
        .map(function (it) {
          return (
            '<div class="pick-item" onclick="stPagePickChoose(\'' +
            escJsAttr(fieldKey) +
            "','" +
            escJsAttr(it.name) +
            '\')">' +
            '<img class="pick-thumb" src="' +
            escAttr(it.download_url || '') +
            '" alt="' +
            escAttr(it.name) +
            '" loading="lazy">' +
            '<div class="pick-name">' +
...
function _stKeyHandler(e) {
  ...
  if (mod && e.key === 'z' && !e.shiftKey) {
    ...
    e.preventDefault();
    _stInsertFormat('**', 'testo');
  } else if (mod && e.key === 'i') {
    e.preventDefault();
    _stInsertFormat('*_', 'testo');
...
function _stKeyAdd() {
  ...
  if (_stKind === 'schede') {
    ...
    data.cards.splice(idx >= 0 ? idx + 1 : data.cards.length, 0, {
      name: 'Nuova ' + _stEntLabel(),
      intro: '',
      fields: newFields,
      blocks: [],
    });
    ...
    data.cards.splice(idx >= 0 ? idx + 1 : data.cards.length, 0, {
      type: type,
      deity: type === 'deity' || undefined,
      name: type === 'section' ? 'Nuova sezione' : _stEntName(),
      img: type === 'deity' ? '' : '',
      quote: '',
      ident: _stPanNewRows(),
      personality: '',
  ...
  if (last) {
    var nm = last.querySelector('.st-name');
    if (nm) {
      nm.focus();
      nm.select();
    }
...
function _stValidate(data) {
  ...
  (data.cards || []).forEach(function (c, i) {
    ...
    else if (seen[(c.name || '').trim()] > 1)
      warn.push('«' + c.name.trim() + '» è ripetuto ' + seen[(c.name || '').trim()] + ' volte');
    if (_stKind !== 'schede' && c.type === 'deity' && !c.img)
      warn.push(
        '«' +
          (c.name || '').trim() +
          '» è un' +
          (_stPanGeneric() ? ' elemento ma non ha immagine' : 'a divinità ma non ha immagine')
      );
    ...
    if (_stKind === 'schede' && c.fields && reqFields.length) {
      reqFields.forEach(function (rf) {
        ...
        if (typeof c.fields === 'object' && !Array.isArray(c.fields)) {
          ...
          /* Legacy format */
          found = c.fields.some(function (f) {
            return (f[0] || '').toLowerCase() === rf.toLowerCase() && (f[1] || '').trim();
          });
        }
...
function _stShowWarnings(data) {
  ...
  if (w) {
    ...
    w.innerHTML =
      '⚠ ' +
      (warn.length === 1 ? '1 avviso' : warn.length + ' avvisi') +
      ': ' +
      warn
        .map(function (x) {
          return esc(x);
...
function _stCardHasContent(c) {
  ...
  if (c.fields) {
    if (Array.isArray(c.fields)) {
      ...
      if (
        Object.keys(c.fields).some(function (k) {
          return c.fields[k];
        })
      )
        return true;
  ...
  if (
    c.legacyFields &&
    c.legacyFields.some(function (p) {
      return p[0] || p[1];
    })
  )
    return true;
...
function _stDelete(i) {
  ...
  if (
    c &&
    _stCardHasContent(c) &&
    !confirm('Eliminare il blocco «' + ((c.name || '').trim() || 'blocco ' + (i + 1)) + '»?')
  )
    return;
  _stPushUndo();
...
function _stAddIdent(i) {
  _stPushUndo();
  var box = document.getElementById('st-ident-' + i);
  if (!box) return;
  var div = document.createElement('div');
  div.className = 'st-kv';
  div.innerHTML =
    '<input class="in k" placeholder="Campo" oninput="_stSync()">' +
  ...
  box.appendChild(div);
  _stSync();
  var k = div.querySelector('.k');
  if (k) k.focus();
}
function _stDelRow(btn) {
  _stPushUndo();
  var kv = btn.closest('.st-kv');
  if (kv) kv.remove();
  _stSync();
}
function _stAddBlock(i) {
  _stPushUndo();
  var box = document.getElementById('st-blocks-' + i);
  if (!box) return;
  var div = document.createElement('div');
  div.className = 'st-block';
  div.innerHTML =
    '<div class="st-block-head"><input class="in st-bname" placeholder="Titolo sezione" oninput="_stSync()">' +
    '<button type="button" class="btn btn-d btn-icon btn-sm" title="Rimuovi sezione" onclick="_stDelBlock(this)">✕</button></div>' +
    '<textarea class="in st-ta st-bmd" oninput="_stSync()" placeholder="Contenuto (markdown)"></textarea>';
  box.appendChild(div);
  _stSync();
  var bn = div.querySelector('.st-bname');
  if (bn) bn.focus();
}
function _stDelBlock(btn) {
  _stPushUndo();
  var b = btn.closest('.st-block');
  if (b) b.remove();
  _stSync();
}
...
function _stTouchDragStart(e, i) {
  ...
  if (card) {
    ...
    ghost.style.cssText =
      'position:fixed;pointer-events:none;z-index:9999;opacity:.85;width:' +
      card.offsetWidth +
      'px;max-height:120px;overflow:hidden;border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,.3);transform:rotate(2deg);transition:none;';
...
function _stTouchDragEnd(e) {
  ...
  if (_stTouchDrag.ghost) {
    _stTouchDrag.ghost.remove();
    _stTouchDrag.ghost = null;
  }
  document.querySelectorAll('.st-card').forEach(function (c) {
    c.classList.remove('dragging', 'drag-over-top', 'drag-over-bottom');
  });
...
function _stPick(i) {
  ...
  modalHtml(
    ...
    '<div class="fld"><label>Cerca in /images/</label><input id="st-pick-q" class="in" placeholder="Filtra per nome…" oninput="stPickFilter(this.value)"></div>' +
      '<div class="fld"><label>URL diretto</label><div style="display:flex;gap:6px"><input id="st-pick-url" class="in" placeholder="/images/… oppure https://…" onkeydown="if(event.key===\'Enter\')stPickUrl(' +
      i +
      ')">' +
      '<button class="btn btn-p btn-sm" onclick="stPickUrl(' +
      i +
      ')">Usa</button></div></div>' +
  ...
  var q = document.getElementById('st-pick-q');
  if (q) q.focus();
  stLoadPicker(i);
}
function stLoadPicker(i) {
  ...
  ghGet('images')
    ...
    .catch(function (e) {
      grid.innerHTML = '<div class="list-empty">Errore: ' + esc(e.message) + '</div>';
    });
}
function stPickFilter(q) {
  ...
  if (!sel.length) {
    grid.innerHTML = '<div class="list-empty">Nessun risultato</div>';
    return;
  }
  grid.innerHTML = sel
    .map(function (it) {
      return (
        '<div class="pick-item" onclick="stPickChoose(\'' +
        escJsAttr(it.name) +
        '\')">' +
        '<img class="pick-thumb" src="' +
        escAttr(it.download_url || '') +
...
function stPickUpload(files) {
  ...
  if (!/^image\//.test(file.type)) {
    toast("Il file selezionato non è un'immagine", 'error');
    return;
  }
  var reader = new FileReader();
  reader.onload = function () {
    var name = (file.name || 'immagine')
      .toLowerCase()
      .replace(/[^a-z0-9._-]+/g, '-')
      .replace(/-+/g, '-');
      ...
      .catch(function (e) {
        ...
        if (/422|already exists|already\.committed/i.test(msg)) {
          ...
          ghPutBinary('/images/' + name2, 'admin: upload ' + name2, dataUri)
            ...
            .catch(function (e2) {
              toast('Errore upload: ' + ((e2 && e2.message) || 'n/d'), 'error');
            });
        } else {
  ...
  if (im) {
    if (url) {
      if (im.tagName === 'DIV') {
        ...
        n.src = url;
        n.alt = '';
        im.parentNode.insertBefore(n, im);
        im.remove();
      } else {
        im.src = url;
      }
...
function _stClearImg(i) {
  ...
  if (im && im.tagName === 'IMG') {
    ...
    d.className = 'st-img st-img-empty';
    d.id = 'st-img-' + i;
    d.textContent = _stPanEmptyIcon();
    im.parentNode.insertBefore(d, im);
    im.remove();
  }
  var wrap = document.getElementById('st-imgwrap-' + i);
  if (wrap) {
    wrap.removeAttribute('onmousedown');
