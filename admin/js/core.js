...
async function _checkRemoteSha(){
  ...
  try{
    ...
    var raw='';
    try{raw=b64decode(d.content||'')}catch(e){}
...
function _loginStartBackoff(){
  ...
  _loginCountdownTimer=setInterval(function(){
    ...
    if(errEl&&remaining>0){errEl.textContent='Attendere '+remaining+'s prima di riprovare...';errEl.style.display=''}
    if(remaining<=0){
      clearInterval(_loginCountdownTimer);_loginCountdownTimer=null;
...
async function doLogin(){
  ...
  if(Date.now()<_loginBackoffUntil){_loginStartBackoff();return}
  var btn=document.getElementById('login-btn');
  if(btn)btn.disabled=true;
  ...
  if(!ok){
    ...
    errEl.style.display='';
    if(_loginAttempts>=2){_loginStartBackoff()}
    else{if(btn)btn.disabled=false}
    return;
  }
  if(btn)btn.disabled=false;
  _loginAttempts=0;_loginBackoffUntil=0;
  if(_loginCountdownTimer){clearInterval(_loginCountdownTimer);_loginCountdownTimer=null}
...
function _dashGrid(label,type,items){
  ...
  if(_bulkMode&&type==='page'){
    ...
    if(typeof LAYOUT_REGISTRY!=='undefined')Object.keys(LAYOUT_REGISTRY).forEach(function(k){var l=LAYOUT_REGISTRY[k];layoutOpts+='<option value="'+escAttr(l.v)+'">'+l.i+' '+esc(l.l)+'</option>'});
    h+='<div id="bulk-bar" style="padding:10px 16px;display:flex;gap:8px;align-items:center;border-top:1px solid var(--line)">'
      ...
      +'<div class="grow"></div>'
      +'<button class="btn btn-soft btn-sm" onclick="bulkExportSelected()">Esporta</button>'
      +'<button class="btn btn-d btn-sm" onclick="bulkDelete()">Elimina</button>'
      ...
      +'<select class="in" id="bulk-layout" style="width:auto;padding:5px 8px;font-size:11px">'+layoutOpts+'</select>'
      +'<button class="btn btn-soft btn-sm" onclick="bulkChangeLayout()">Layout</button>'
      +'</div>';
  }
  return h+'</div>';
}
