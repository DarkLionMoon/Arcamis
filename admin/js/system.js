...
async function openUserManagement() {
  ...
  try {
    ...
    var h = viewHead(
      '👥',
      'Gestione utenti',
      'Account che possono accedere al pannello',
      '<button class="btn btn-p" onclick="openAddUserModal()">+ NUOVO UTENTE</button>'
    );
    h +=
      '<div class="panel"><div class="panel-head"><h3>Account</h3><span class="hint">' +
      users.length +
      ' utenti · password salvate come hash SHA-256</span></div>';
    ...
    users.forEach(function (u) {
      ...
      h +=
        ...
        esc(u.username) +
        (isMe ? ' <span class="pill blue">te</span>' : '') +
        '</div>' +
        '<div class="cs">ruolo: ' +
        esc(u.role || 'editor') +
        (u.updated ? ' · aggiornato ' + esc(dateFmt(u.updated)) : '') +
        '</div></div>' +
        '<div class="ract">' +
        '<button class="btn btn-soft btn-sm" onclick="openEditUserModal(\'' +
        escJsAttr(u.username) +
        '\')">✏️</button>' +
        (u.username !== _currentUser
          ? '<button class="btn btn-d btn-sm" onclick="deleteUserConfirm(\'' + escJsAttr(u.username) + '\')">🗑</button>'
          : '') +
        '</div></div>';
    });
    h += '</div>';
...
async function saveNewUser() {
  ...
  if (!username || username.length < 3) {
    if (st) {
      st.textContent = 'Username: almeno 3 caratteri';
  ...
  if (!/^[a-z0-9._-]+$/.test(username)) {
    if (st) {
      st.textContent = 'Username: solo minuscole, numeri, punto, trattino, underscore';
      st.className = 'md-status err';
    }
    return;
  }
  if (password.length < 6) {
    if (st) {
      st.textContent = 'Password: almeno 6 caratteri';
      st.className = 'md-status err';
    }
    return;
  }
  ...
  if (st) {
    st.textContent = ok ? '✓ Utente creato' : '✕ Errore (username già esistente?)';
...
function openEditUserModal(username) {
  ...
  modalHtml(
    'eu-modal',
    '✏️ Modifica — ' + esc(username),
    '<input type="hidden" id="eu-user" value="' +
      escAttr(username) +
      '">' +
...
async function saveEditUser() {
  ...
  if (st) {
    st.textContent = ok ? '✓ Aggiornato' : '✕ Errore';
    st.className = 'md-status ' + (ok ? 'ok' : 'err');
  }
  if (ok) {
...
async function openAudit() {
  ...
  try {
    ...
    var h = viewHead(
      '📋',
      'Registro attività',
      'Ultime attività degli admin (30 giorni)',
    ...
    h +=
      '<div class="panel"><div class="panel-head"><h3>Eventi</h3><span class="hint" id="aud-count">' +
      _auditEntries.length +
      ' registrati</span></div>' +
      '<div id="aud-body">' +
      _auditRender('', '', '') +
      '</div></div>';
    ...
    setStatus('err', 'errore');
    toast('Registro non disponibile: ' + e.message, 'error');
  }
}
...
function _auditRender(q, from, to) {
  ...
  if (q) {
    ...
    entries = entries.filter(function (e) {
      return (e.action || '').toLowerCase().indexOf(q) !== -1 || (e.target || '').toLowerCase().indexOf(q) !== -1;
    });
  }
  if (from) {
    entries = entries.filter(function (e) {
      return e.timestamp && e.timestamp >= from;
    });
  }
  if (to) {
    entries = entries.filter(function (e) {
      return e.timestamp && e.timestamp <= to + 'T23:59:59';
    });
  }
  var cnt = document.getElementById('aud-count');
  if (cnt) cnt.textContent = entries.length + ' filtrati';
  if (!entries.length)
    return '<div class="empty"><span class="ei">🗒️</span>Nessun evento' + (q ? ' per "' + esc(q) + '"' : '') + '</div>';
  return entries
    .map(function (e) {
      return (
        '<div class="act-item"><div class="act-dot" style="background:' +
        _auditColor(e.action) +
        '"></div>' +
...
function exportAuditCSV() {
  ...
  entries.forEach(function (e) {
    csv +=
      ...
      (e.role || '').replace(/"/g, '""') +
      '","' +
      (typeof e.extra === 'string' ? e.extra : JSON.stringify(e.extra || '')).replace(/"/g, '""') +
      '"\n';
  });
  ...
  a.href = url;
  a.download = 'arcamis-audit-' + new Date().toISOString().slice(0, 10) + '.csv';
...
document.addEventListener('keydown', function (e) {
  if (e.key === 'Escape') {
    document.querySelectorAll('.md-backdrop').forEach(function (el) {
      el.remove();
    });
  }
});
    ...
    fetch('/api/admin?action=check', { credentials: 'include' })
      ...
      .then(function (j) {
        if (j && j.ok) {
          if (j.role) {
            _userRole = j.role;
            sessionStorage.setItem('arcadmin_role', j.role);
          }
          _fetchCsrf();
...
async function openLinkGraph() {
  ...
  var h = viewHead(
    '🕸️',
    'Grafo link',
    'Mappa delle connessioni tra pagine. Mostra quali pagine linkano quali.',
    '<button class="btn btn-p" onclick="openLinkGraph()">⟳ AGGIORNA</button>'
  );
  h +=
    '<div class="panel"><div class="panel-head"><h3>Grafo</h3></div><div id="graph-body" style="padding:16px;text-align:center;color:var(--dim)">⏳ Analisi in corso…</div></div>';
  document.getElementById('main').innerHTML = h;
  try {
    var links = {};
    var allKeys = PAGES.map(function (p) {
      return p.k;
    });
    allKeys.forEach(function (k) {
      links[k] = { incoming: [], outgoing: [] };
    });
    ...
    var orphans = allKeys.filter(function (k) {
      return links[k] && links[k].incoming.length === 0;
    });
    var hubs = allKeys
      .slice()
      .sort(function (a, b) {
        return (links[b] ? links[b].outgoing.length : 0) - (links[a] ? links[a].outgoing.length : 0);
      })
      .slice(0, 5);
    ...
    h2 +=
      '<span>🔗 <b>' +
      Object.values(links).reduce(function (s, l) {
        return s + l.outgoing.length;
      }, 0) +
      '</b> link interni</span>';
    ...
    if (orphans.length) {
      h2 +=
        '<div style="margin-bottom:12px;font-size:13px;font-weight:600;color:var(--orange)">Pagine orfane (nessun link in entrata)</div>';
      orphans.forEach(function (k) {
        var meta = PAGES.find(function (p) {
          return p.k === k;
        });
        h2 +=
          '<div class="cover-row" style="cursor:default;border-color:rgba(200,150,60,.2)">' +
          '<div class="si">' +
          (meta ? meta.i : '📄') +
          '</div>' +
          '<div class="cmain"><div class="ct">' +
          ...
          '</div>' +
          '<div class="cs">' +
          (links[k] ? links[k].outgoing.length : 0) +
          ' link in uscita, 0 in entrata</div></div></div>';
      });
    }
    if (hubs.length) {
      h2 +=
        '<div style="margin:16px 0 12px;font-size:13px;font-weight:600;color:var(--teal3)">Pagine hub (più link in uscita)</div>';
      hubs.forEach(function (k) {
        ...
        h2 +=
          '<div class="cover-row" style="cursor:default;border-color:rgba(100,180,220,.2)">' +
          '<div class="si">' +
          (meta ? meta.i : '📄') +
          '</div>' +
          '<div class="cmain"><div class="ct">' +
          ...
          '</div>' +
          '<div class="cs">' +
          links[k].outgoing.length +
          ' link in uscita, ' +
          links[k].incoming.length +
          ' in entrata</div></div></div>';
      });
    }
    body.innerHTML = h2;
  } catch (e) {
...
async function openTrash() {
  ...
  var h = viewHead(
    '🗑️',
    'Cestino',
    'Pagine eliminate. Ripristinabili per 30 giorni.',
  ...
  try {
    ...
    if (!j.items || !j.items.length) {
      body.innerHTML =
        '<div style="padding:20px"><span style="font-size:28px">✅</span><div style="margin-top:8px">Il cestino è vuoto.</div></div>';
      return;
    }
    var h2 =
      '<div style="padding:0 0 8px;font-size:13px"><b>' +
      j.items.length +
      '</b> pagine nel cestino · scadono dopo 30 giorni</div>';
    j.items.forEach(function (item) {
      ...
      h2 +=
        '<div class="cover-row" style="cursor:default;border-color:rgba(200,80,80,.15)">' +
        '<div class="si">' +
        (item.pageIcon || '📄') +
        '</div>' +
        '<div class="cmain"><div class="ct">' +
        esc(item.pageTitle || item.pageKey) +
        '</div>' +
        '<div class="cs">eliminata ' +
        esc(item.deletedAt ? new Date(item.deletedAt).toLocaleString('it-IT') : '?') +
        ' · ' +
        remaining +
        'gg rimanenti</div></div>' +
        '<div class="ract">' +
        '<button class="btn btn-p btn-sm" onclick="restoreFromTrash(\'' +
        escAttr(item.pageKey) +
        '\')">♻️ RIPRISTINA</button>' +
        '<button class="btn btn-d btn-sm" onclick="permanentDeleteTrash(\'' +
        escAttr(item.pageKey) +
        '\')">❌ ELIMINA</button>' +
        '</div></div>';
    });
...
async function restoreFromTrash(pageKey) {
  var ok = await uiConfirm(
    'Ripristinare "' +
      pageKey +
      '" dal cestino?\n\nLa pagina verrà ripristinata su GitHub e il deploy avviato automaticamente.',
    { ok: 'RIPRISTINA' }
  );
  if (!ok) return;
...
async function openActiveSessions() {
  ...
  var h = viewHead(
    '🔐',
    'Sessioni attive',
    'Gestisci le sessioni di login attive',
  ...
  try {
    ...
    users.forEach(function (u) {
      ...
      h2 +=
        ...
        '<div class="si" style="background:var(--gold);color:#14100a;font-weight:700;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center">' +
        (u.username.charAt(0) || '?').toUpperCase() +
        '</div>' +
        '<div class="cmain"><div class="ct">' +
        esc(u.username) +
        '</div>' +
        '<div class="cs"><span class="badge ' +
        role +
        '">' +
        u.role +
        '</span>' +
        ...
        '<div class="ract">' +
        (u.username !== 'admin'
          ? '<button class="btn btn-d btn-sm" onclick="forceLogoutUser(\'' +
            escAttr(u.username) +
            '\')">🔓 Forza Logout</button>'
          : '') +
        '</div></div>';
    ...
    h2 +=
      '<div style="margin-top:16px;padding:12px;background:var(--panel);border:1px solid var(--line);border-radius:8px;font-size:12px;color:var(--dim)">' +
...
async function openLinkScanner() {
  ...
  var h = viewHead(
    '🔗',
    'Scanner link rotti',
    'Controlla tutti i link interni e le immagini di ogni pagina',
    '<button class="btn btn-p" onclick="openLinkScanner()">⟳ SCANSIONA</button>'
  );
  h +=
    '<div class="panel"><div class="panel-head"><h3>Risultati</h3></div><div id="ls-body" style="padding:16px;text-align:center;color:var(--dim)">⏳ Scansione di tutte le pagine in corso…</div></div>';
  document.getElementById('main').innerHTML = h;
  try {
    ...
    if (!j.broken.length && !j.warnings.length) {
      body.innerHTML =
        '<div style="padding:20px"><span style="font-size:28px">✅</span><div style="margin-top:8px">Nessun problema trovato! ' +
        j.checked +
        ' pagine controllate.</div></div>';
      return;
    }
    var h2 = '<div style="padding:8px 0 14px;display:flex;gap:16px;font-size:13px">';
    h2 += '<span>📄 <b>' + j.checked + '</b> pagine controllate</span>';
    if (j.summary.totalBroken)
      h2 += '<span style="color:var(--red)">❌ <b>' + j.summary.totalBroken + '</b> problemi</span>';
    if (j.summary.missingAlt)
      h2 += '<span style="color:var(--orange)">♿ <b>' + j.summary.missingAlt + '</b> alt mancanti</span>';
    h2 += '</div>';
    if (j.broken.length) {
      ...
      j.broken.forEach(function (b) {
        ...
        h2 +=
          '<div class="cover-row" style="cursor:default;border-color:rgba(200,80,80,.2)">' +
    ...
    if (j.warnings.length) {
      h2 +=
        '<div style="margin:16px 0 12px;font-size:13px;font-weight:600;color:var(--orange)">Alt text mancanti</div>';
...
async function openGlobalFindReplace() {
  ...
  var h = viewHead(
    '🔍',
    'Trova & Sostituisci',
    'Cerca e sostituisce testo trasversale a tutte le pagine',
  ...
  h +=
    '<div class="fld"><label>Testo da cercare</label><input id="gfr-find" class="in" placeholder="es. Vecchio nome NPC" onkeydown="if(event.key===\'Enter\')globalFRPreview()"></div>';
  h +=
    '<div class="fld"><label>Testo sostitutivo</label><input id="gfr-replace" class="in" placeholder="es. Nuovo nome NPC" onkeydown="if(event.key===\'Enter\')globalFRPreview()"></div>';
  ...
  h += '</div></div></div>';
  h +=
    '<div class="panel"><div class="panel-head"><h3>Risultati</h3></div><div id="gfr-results" style="padding:16px;color:var(--dim);font-size:12px">Inserisci un testo e premi Anteprima</div></div>';
  document.getElementById('main').innerHTML = h;
}
async function globalFRPreview() {
  var find = (document.getElementById('gfr-find').value || '').trim();
  if (!find) {
    toast('Inserisci un testo da cercare', 'error');
    return;
  }
...
async function globalFRApply() {
  ...
  if (find === replace) {
    toast('Cerca e sostituisci sono uguali', 'error');
    return;
  }
  var caseSensitive = document.getElementById('gfr-case').checked;
  var ok = await uiConfirm(
    'Sostituire "' +
      ...
      '" con "' +
      replace +
      '" in TUTTE le pagine?\n\nOperazione irreversibile. Verrà creato un commit per ogni pagina modificata.',
    { danger: true, ok: 'APPLICA' }
  );
  if (!ok) return;
  ...
  for (var i = 0; i < PAGES.length; i++) {
    try {
      ...
      if (newContent !== content) {
        ...
        await ghPut(
          path,
          'admin: find&replace "' + find + '"→"' + replace + '" in ' + PAGES[i].k,
...
async function openOrphanMedia() {
  ...
  var h = viewHead(
    '🗑️',
    'Media orfani',
    'Trova e rimuovi immagini in /images/ non referenziate da nessuna pagina',
    '<button class="btn btn-p" onclick="openOrphanMedia()">⟳ RICERCA</button>'
  );
  h +=
    '<div class="panel"><div class="panel-head"><h3>Risultati</h3></div><div id="orphan-body" style="padding:16px;text-align:center;color:var(--dim)">⏳ Analisi in corso…</div></div>';
  document.getElementById('main').innerHTML = h;
  try {
    ...
    if (!j.orphans || !j.orphans.length) {
      body.innerHTML =
        '<div style="padding:20px"><span style="font-size:28px">✅</span><div style="margin-top:8px">Nessun file orfano trovato! Tutte le ' +
        j.total +
        ' immagini sono referenziate.</div></div>';
      return;
    }
    var h2 =
      '<div style="padding:8px 0 14px;font-size:13px"><b>' +
      j.orphans.length +
      '</b> file orfani trovati su ' +
      j.total +
      ' totali (' +
      j.referenced +
      ' referenziati)</div>';
    h2 +=
      '<div style="display:flex;gap:8px;margin-bottom:12px"><button class="btn btn-d btn-sm" onclick="deleteSelectedOrphans()">🗑 ELIMINA SELEZIONATI</button><label style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--dim)"><input type="checkbox" id="orphan-select-all" onchange="toggleAllOrphans(this.checked)" checked> Seleziona tutti</label></div>';
    j.orphans.forEach(function (name) {
      h2 +=
        '<div class="cover-row" style="cursor:default">' +
        '<input type="checkbox" class="orphan-cb" value="' +
        escAttr(name) +
        '" checked style="accent-color:var(--red)">' +
        '<div class="cover-thumb" style="background-image:url(/images/' +
        escAttr(name) +
        ')"></div>' +
        '<div class="cmain"><div class="ct" style="font-family:var(--mono);font-size:12px">' +
        esc(name) +
        '</div>' +
...
function toggleAllOrphans(checked) {
  document.querySelectorAll('.orphan-cb').forEach(function (cb) {
    cb.checked = checked;
  });
}
async function deleteSelectedOrphans() {
  var selected = [];
  document.querySelectorAll('.orphan-cb:checked').forEach(function (cb) {
    selected.push(cb.value);
  });
  if (!selected.length) {
    toast('Nessun file selezionato', 'error');
    return;
  }
  var ok = await uiConfirm(
    'Eliminare ' +
      selected.length +
      ' file immagine orfani?\n\nOperazione irreversibile. I file verranno rimossi da /images/ su GitHub.',
    { danger: true, ok: 'ELIMINA' }
  );
  if (!ok) return;
  setStatus('saving', 'eliminazione…');
  try {
    var r = await fetch('/api/admin?action=delete_orphan_media', {
      method: 'POST',
      credentials: 'include',
