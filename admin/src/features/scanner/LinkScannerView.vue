    ...
    <AdminOnly>
      ...
      <div v-for="issue in issues" :key="issue.pageKey + issue.type" class="arc-changelog-item">
        <div style="flex:1">
          ...
          <p v-if="issue.broken?.length" style="font-size:12px;opacity:.75;margin:6px 0 0">
            → <code v-for="(b, i) in issue.broken.slice(0, 6)" :key="i">{{ b.target }}</code>
          </p>
          <p v-if="issue.missingAlt?.length" style="font-size:12px;opacity:.75;margin:6px 0 0">
            <code v-for="(m, i) in issue.missingAlt.slice(0, 6)" :key="i">{{ m.src }}</code>
          </p>
          <p v-if="issue.detail" style="font-size:12px;opacity:.6;margin:4px 0 0">{{ issue.detail }}</p>
        </div>
      </div>
