      ...
      case'child_page':
        ...
        while(cpScan<blocks.length){
          ...
          if(blocks[cpScan].type==='child_page'){
            ...
            var cptitleCheck=cptitle.toLowerCase();

            if(!mapPageIds.has(cpid)&&cptitleCheck!=='classi'){
...
async function _loadSingleDb(grid){
    ...
    try{
      ...
      try{
        ...
        var rs=await fetch('/data/db/'+dbId+'.json');

        if(rs.ok)data=await rs.json();
...
async function _gpRender(id,label,icon){
  ...
  /* Sanitizza ID anche qui (chiamate dirette da gpBack etc.) */
  ...
  try{
    ...
    if(!data){
      ...
      /* Prova prima il file statico, poi l'API */
      ...
      try{
        ...
        data=await fetchWithRetry(staticUrl);
