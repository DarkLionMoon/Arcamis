  ...
  (root || document).querySelectorAll('.lcard').forEach(function (card) {
    ...
    card.addEventListener('mousemove', function (e) {
      ...
      shine.style.background =
        ...
        '% ' +
        y +
        '%,rgba(255,255,255,.11) 0%,rgba(255,255,255,.04) 26%,transparent 58%)';
    });
      ...
      for (var i = 0; i < n; i++) {
        (function (idx) {
          ...
          sp.style.cssText =
            ...
            sy +
            'px;' +
            'background:rgba(140,100,220,' +
            bri +
            ');' +
            'box-shadow:0 0 5px rgba(140,100,220,' +
            bri +
            ');' +
            'animation-delay:' +
            idx * 90 +
...
(function () {
  ...
  for (var i = 0; i < _ppCount; i++) {
    ...
    p.style.cssText =
      ...
      's;' +
      '--po:' +
      (0.15 + Math.random() * 0.3).toFixed(2) +
      ';' +
      'animation-delay:' +
...
(function () {
  ...
  for (var i = 0; i < _sfCount; i++) {
    ...
    s.style.cssText =
      ...
      's;' +
      '--tb:' +
      (0.3 + Math.random() * 0.65).toFixed(2) +
      ';' +
      'animation-delay:' +
  ...
  ['60% 20%', '30% 65%', '80% 75%'].forEach(function (pos, idx) {
    ...
    b.style.cssText =
      ...
      (idx === 1 ? '100,40,180' : '60,20,140') +
      ',' +
      (0.06 + idx * 0.02) +
      '),transparent);' +
      'animation:nebula-pulse ' +
      (6 + idx * 2) +
...
(function () {
  if (!document.querySelector('style[data-nebula]')) {
    ...
    s.textContent =
      '@keyframes nebula-pulse{0%{opacity:.7;transform:translate(-50%,-50%) scale(1)}100%{opacity:1;transform:translate(-50%,-50%) scale(1.15)}}';
...
(function () {
  ...
  cw.addEventListener('mousemove', function (e) {
    ...
    var xp = (e.clientX - r.left) / r.width - 0.5;
    var yp = (e.clientY - r.top) / r.height - 0.5;
...
var _fio = new IntersectionObserver(
  function (entries) {
  ...
  window.toggleArcAudio = function () {
    ...
    if (!_aud) {
      ...
      _aud.onerror = function () {
        ...
        _on = false;
        if (btn) {
          btn.classList.remove('ara-on');
          btn.title = 'Audio non trovato — aggiungi ambient.mp3 in public/';
        }
      };
    }
    if (_on) {
