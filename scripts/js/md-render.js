...
(function () {
  ...
  function inline(t) {
    ...
    t = t.replace(/\[\^([a-zA-Z0-9_-]+)\]/g, function (m, id) {
      return fnRef(id);
    });
    ...
    t = t.replace(/!\[([^\]]*)\]\(([^)]+?)(?:\s+"([^"]*)")?\)/g, function (m, alt, u, cap) {
      ...
      var img =
        ...
        '" alt="' +
        a +
        '" loading="lazy" decoding="async" class="n-image n-zoomable"' +
        (window.arcZoom ? ' onclick="arcZoom(\'' + js + '\')"' : '') +
        '>';
      if (cap || alt)
        return '<figure class="n-image">' + img + '<figcaption>' + inline(dec(cap || alt)) + '</figcaption></figure>';
  ...
  function renderTable(tbl) {
    ...
    splitCells(tbl[0]).forEach(function (c, i) {
      out += '<th' + al(i) + '>' + inline(c.trim()) + '</th>';
    });
    out += '</tr></thead><tbody>';
    tbl.slice(2).forEach(function (r) {
      out += '<tr>';
      splitCells(r).forEach(function (c, i) {
        out += '<td' + al(i) + '>' + inline(c.trim()) + '</td>';
      });
      out += '</tr>';
    });
    out += '</tbody></table></div>';
  ...
  function renderLevel(nodes) {
    ...
    function flush() {
      if (cur) {
        html +=
          '<' + cur.type + ' class="' + (cur.type === 'ul' ? 'n-ul' : 'n-ol') + '">' + cur.html + '</' + cur.type + '>';
        cur = null;
      }
    }
    nodes.forEach(function (node) {
      ...
      if (node.task)
        li +=
          '<span class="n-todo"><span class="n-todo-box">' + (node.checked ? '☑' : '☐') + '</span>' + inner + '</span>';
      else li += inner;
      li += '</li>';
      if (!cur || cur.type !== node.type) {
        flush();
        cur = { type: node.type, html: '' };
      }
      cur.html += li;
    });
    flush();
  ...
  function renderListItems(lines) {
    ...
    var stack = [{ indent: -1, children: [] }],
      roots = stack[0].children;
  ...
  function blockquoteLines(lines) {
    ...
    if (cm) {
      var key = cm[1].toUpperCase(),
        fold = cm[2],
        title = cm[3];
      var cfg = CALLOUT_TYPES[key] || {
        label: key.charAt(0) + key.slice(1).toLowerCase(),
        icon: '✨',
        c: '#c89b3c',
        bg: 'rgba(200,155,60,.05)',
      };
      if (fold === '-' || fold === '+') {
        var body = inner.split('\n').slice(1).join('\n');
        return (
          '<details class="n-callout n-fold" style="--callout-c:' +
          cfg.c +
          ';--callout-bg:' +
          cfg.bg +
          '"' +
          (fold === '+' ? ' open' : '') +
          '>' +
          '<summary class="n-fold-head"><span class="n-callout-icon">' +
          cfg.icon +
          '</span>' +
          '<span class="n-fold-title">' +
          inline(dec(title || cfg.label || key)) +
          '</span>' +
      ...
      return (
        '<div class="n-callout" style="--callout-c:' +
        cfg.c +
        ';--callout-bg:' +
        cfg.bg +
        '">' +
        '<span class="n-callout-icon">' +
        cfg.icon +
        '</span>' +
        '<div class="n-callout-body">' +
        renderBlocks(body) +
    ...
    return (
      '<blockquote class="n-quote"><span class="n-quote-mark">"</span>' +
      inner.split('\n').map(inline).join('<br>') +
      '</blockquote>'
    );
  }
  ...
  function renderBlocks(src) {
    ...
    while (i < n) {
      ...
      if (fm) {
        ...
        html +=
          '<pre class="n-code">' +
          (lang ? '<span class="n-code-lang">' + esc(lang) + '</span>' : '') +
          esc(buf.join('\n')) +
          '</pre>';
        i = j + 1;
        continue;
  ...
  function renderFootnotes() {
    ...
    var items = ids.map(function (id) {
      var d = _fnDefs[id];
      return (
        '<li class="n-fn-item" id="fn-' +
        attr(id) +
        '">' +
        inline(dec(d.text)) +
        ' <a href="#fnref-' +
        attr(id) +
        '" class="n-anchor n-fn-back" title="Torna al testo">↩</a></li>'
      );
    });
    return (
      '<section class="n-footnotes"><div class="n-fn-title">Note</div><ol class="n-fn-list">' +
      items.join('') +
      '</ol></section>'
    );
  }
  ...
  function mdRender(md) {
    ...
    /* Difesa in profondità: se DOMPurify è caricato (admin/app), sanitizza l'output.
     Config consentito: solo immagini con src relativi/sicuri; nessuna URL javascript:. */
    if (window.DOMPurify) {
      html = window.DOMPurify.sanitize(html, {
        USE_PROFILES: { html: true },
        ALLOWED_URI_REGEXP: /^(?:(?:https?|mailto|ftp|data):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i,
      });
    }
    _fnDefs = {};
    _fnOrder = 0;
