...
function applyFont(s) {
  _fs = Math.max(0.85, Math.min(1.3, s));
...
function applyTheme(t) {
  ...
  _THEMES.forEach(function (th) {
    if (th !== 'dark') document.body.classList.remove('theme-' + th);
  });
...
window.onAfterPageRender = function (cb) {
  window._afterPageRenderCbs.push(cb);
};
window.afterPageRender = function () {
  window._afterPageRenderCbs.forEach(function (cb) {
    try {
      cb();
    } catch (e) {}
  });
    ...
    _loadSearchIndex(function (idx) {
      ...
      (idx || []).forEach(function (p) {
        ...
        if (tpos > -1) score = 100 - tpos;
          ...
          if (pos > -1) {
            score = 10;
            var st = Math.max(0, pos - 45),
              en = Math.min(tx.length, pos + q.length + 55);
      ...
      if (!res.length) {
        sr.innerHTML =
          '<div class="sri" style="color:var(--text3);font-style:italic;padding:12px 14px">Nessun risultato</div>';
        sr.classList.add('open');
        return;
      }
      sr.innerHTML = res
        .map(function (r) {
          var et = _escHtml(r.p.title),
            ei = _escHtml(r.p.icon || '📄'),
            eid = _escHtml(r.p.id);
          var snippet = r.snippet ? '<div class="sri-snippet">' + r.snippet + '</div>' : '';
          return (
            '<div class="sri" onclick="csearch();gp(\'' +
            eid +
            "','" +
            et +
            "','" +
            ...
            ei +
            '</span>' +
            '<div class="sri-body"><span class="sl">' +
            et +
            '</span>' +
            snippet +
            '</div>' +
    ...
    _loadSearchIndex(function (idx) {
      ...
      sr.innerHTML = res
        .map(function (r) {
          ...
          var snippet = r.snippet ? '<div class="sri-snippet">' + r.snippet + '</div>' : '';
          return (
            '<div class="sri" onclick="closeMobileNav();gp(\'' +
            eid +
            "','" +
            et +
            "','" +
...
var _recenti = [];
try {
  _recenti = JSON.parse(localStorage.getItem('arc_recenti') || '[]');
} catch (e) {
  _recenti = [];
}
function addRecente(id, title, icon) {
  _recenti = _recenti.filter(function (r) {
    return r.id !== id;
  });
  _recenti.unshift({ id: id, title: title, icon: icon });
  if (_recenti.length > 5) _recenti.pop();
  try {
    localStorage.setItem('arc_recenti', JSON.stringify(_recenti));
  } catch (e) {}
  _renderRecenti();
}
function _renderRecenti() {
  ...
  if (el && _recenti.length) {
    el.innerHTML =
      '<div class="portal-sh">Visitati di recente</div>' +
      _recenti
        .map(function (r) {
          return (
            '<div class="ov-rec-item" onclick="cv();gp(\'' +
            r.id +
            "','" +
            r.title +
            "','" +
  ...
  if (mn) {
    ...
    mn.style.display = '';
    mn.innerHTML =
      '<div class="mn-label">Visitati di recente</div>' +
      _recenti
        .map(function (r) {
          return (
            '<div class="mn-item" onclick="closeMobileNav();gp(\'' +
            r.id +
            "','" +
            r.title +
            "','" +
...
function markChangelogSeen() {
  ...
  document.querySelectorAll('.changelog-badge').forEach(function (b) {
...
function switchHelpTab(tab, el) {
  document.querySelectorAll('.ahp-tab').forEach(function (t) {
    t.classList.remove('active');
  });
  document.querySelectorAll('.ahp-body').forEach(function (b) {
    b.style.display = 'none';
  });
...
function toggleFaq(el) {
  ...
  document.querySelectorAll('.ahp-faq-item').forEach(function (i) {
    i.classList.remove('open');
  });
  if (!wasOpen) item.classList.add('open');
}
document.getElementById('ahp-dm-msg') &&
  document.getElementById('ahp-dm-msg').addEventListener('input', function () {
...
function sendHelpMsg() {
  ...
  if (!msg) {
    status.textContent = 'Scrivi un messaggio prima di inviare.';
    status.className = 'ahp-dm-status err';
    return;
  }
  status.textContent = 'Invio in corso...';
  status.className = 'ahp-dm-status';
  fetch('/api/send-help', {
    method: 'POST',
...
(function () {
  ...
  function _showDisclaimer(s) {
    if (!overlay) return;
    if (disclaimerAccepted || !s.disclaimer_enabled) {
