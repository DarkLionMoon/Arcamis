...
function _siteAttr(s) {
  return _siteEsc(s).replace(/"/g, '&quot;');
}
...
function _mdToHtml(md) {
  ...
  md = md.replace(/^(\|.+\|)\n(\|[-| :]+\|)\n((?:\|.+\|\n?)*)/gm, function (m, header, sep, body) {
    ...
    var rows = body
      ...
      .join('');
    return (
      '<div class="n-tbl-wrap"><table><thead><tr>' + ths + '</tr></thead><tbody>' + rows + '</tbody></table></div>'
    );
  });
  /* Headers */
  ...
  md = md.replace(/\[([^\]]+)\]\(([^)]+)\)/g, function (m, label, url) {
    ...
    return (
      ...
      '" target="_blank" rel="noopener" class="n-link">' +
      label +
      '</a>'
    );
  });
  ...
  md = md.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, function (m, alt, url) {
    ...
    return (
      ...
      '" alt="' +
      safeAlt +
      '" loading="lazy" decoding="async" class="n-image n-zoomable" onclick="arcZoom(\'' +
      jsUrl +
      '\')">'
    );
  });
...
function _detectPageLayout(pg, blocks, pageId, navKey) {
  ...
  var hasTable = blocks.some(function (b) {
    return b.type === 'table';
  });
  if (hasTable) return 'tabelle';

  var hasGallery = blocks.some(function (b) {
    return b.type === 'child_database' || b.type === 'collection_view';
  });
...
function gpBack(stackIdx) {
  ...
  history.pushState(
    { id: item.id, label: item.label, icon: item.icon, stack: navStack.slice() },
    '',
    _urlForId(item.id)
  );
...
function buildCrumb(currentLabel) {
  ...
  for (var ci = 0; ci < navStack.length - 1; ci++) {
    ...
    h +=
      ...
      '<span class="ph-bc-mid" onclick="gpBack(' +
      ci +
      ')">' +
      (it.icon || '') +
      '&nbsp;' +
...
function _maybeBuildToc(pbody, showToc) {
  ...
  if (headings.length < 2) {
    wrap.classList.remove('toc-active');
    return;
  }
...
async function _gpRender(id, label, icon) {
  ...
  try {
    ...
    if (_tvKey)
      fetch('/api/admin?action=track_view', {
        method: 'POST',
  ...
  if (typeof pages !== 'undefined' && pages && pages.length) {
    for (var _ci = 0; _ci < pages.length; _ci++) {
      ...
      if (_ck.length > _collBest) {
        _collKey = _ck;
        _slug = id.slice(_ck.length + 1);
        _collBest = _ck.length;
      }
    }
  }
  if (_collKey) {
    try {
      ...
      var _deity = _pd.deities.find(function (x) {
        return x.slug === _slug;
      });
      if (_deity) {
        ...
        _pbody.style.width = '';
        var _deityFooter =
          '<div class="n-page-footer"><div class="n-page-footer-gems">✦ &nbsp; ✦ &nbsp; ✦</div>' +
          '<div class="n-page-footer-text">' +
          _collLabel +
          ' — ' +
          _siteEsc(_deity.n) +
          '</div></div>';
        _pbody.innerHTML =
          '<div class="nc" style="animation:fi .22s ease forwards">' +
          _scrubHtmlString(_renderDeityPage(_deity, _collKey)) +
          _deityFooter +
          '</div>';
      ...
      /* fall through al renderer standard */
    }
  }
    ...
    try {
      ...
      if (_localResp.ok) {
        ...
        if (_localJson.content) {
          ...
          if (_layout === 'materiale') _localHtml = _renderMateriale(_localJson.content);
          ...
          else if (_layout === 'lore' || _layout === 'luoghi')
            _localHtml = _renderLore(_localJson.content, _localJson.title, _localJson.icon);
          else if (_layout === 'pantheon')
            _localHtml = _renderPantheon(_localJson.content, (_localPage && _localPage.k) || 'pantheon');
          else if (_layout === 'collezione')
            _localHtml = _renderPantheon(_localJson.content, (_localPage && _localPage.k) || 'pantheon');
            ...
            _localHtml = '<div id="imprese-root"></div>';
          } else {
            /* Auto-detect by page key (legacy fallback) */
            _localHtml =
              _localPage.k === 'materiale'
                ? _renderMateriale(_localJson.content)
                : _localPage.k === 'regole' || _localPage.k === 'gameplay'
                  ? _renderRegole(_localJson.content)
                  : _localPage.k === 'inizia' || _localPage.k === 'avanti'
                    ? _renderPersonaggio(_localJson.content)
                    : _localPage.k === 'pantheon' ||
                        _localPage.k === 'maestria' ||
                        _localPage.k === 'arcamis' ||
                        _localPage.k === 'selva' ||
                        _localPage.k === 'foresta' ||
                        _localPage.k === 'volonx' ||
                        _localPage.k === 'arpax' ||
                        _localPage.k === 'galleria'
                      ? _renderLore(_localJson.content, _localJson.title, _localJson.icon)
                      : _mdToHtml(_localJson.content);
          }
          ...
          var _emptyHtml =
            '<div class="n-empty"><div class="n-empty-icon">' +
            picon +
            '</div>' +
          ...
          var _footer =
            '<div class="n-page-footer"><div class="n-page-footer-gems">✦ &nbsp; ✦ &nbsp; ✦</div>' +
            '<div class="n-page-footer-text">Archivi di Arcamis — ' +
            ptitle +
            '</div></div>';
          pbody.innerHTML =
            '<div class="nc" style="animation:fi .22s ease forwards">' +
            (_scrubHtmlString(_localHtml || '') || _emptyHtml) +
            _footer +
            '</div>';
          if (_localPage.k === 'changelog' && window.loadChangelog) {
            ...
            if (_ft) _nc.insertBefore(_clWrap, _ft);
            else _nc.appendChild(_clWrap);
            window.loadChangelog(_clWrap);
          }
          applyGlossary(pbody);
          _maybeBuildToc(pbody, !!_localJson.toc);
          if (typeof afterPageRender === 'function') afterPageRender();
          if (_layout === 'imprese' && typeof window.initImprese === 'function') {
            var _impRoot = document.getElementById('imprese-root');
            if (_impRoot) window.initImprese(_impRoot);
          }
          return;
        }
      }
    } catch (_e) {
      /* Fall through to Notion API */
    }
  }

  /* ═══ CONTENUTO NON DISPONIBILE ═══ */
  /* Nessuna API Notion: se la pagina non è presente nel JSON locale
     mostriamo un placeholder invece di contattare l'API. */
  phTitle.textContent = label || 'Pagina';
  phIcon.textContent = icon || '📄';
  phEyebrow.textContent = 'Archivi di Arcamis';
  document.title = (label || 'Pagina') + ' — Arcamis';
  ...
  phIcon.style.opacity = '0.06';
  var pbody = document.getElementById('pbody');
  pbody.className = 'page-' + id + ' page-default';
  pbody.style.maxWidth = '';
  pbody.style.width = '';
  var unavailableHtml =
    '<div class="n-empty"><div class="n-empty-icon">🗂️</div>' +
    '<div class="n-empty-title">' +
    _siteEsc(label || 'Pagina') +
    '</div>' +
    '<div class="n-empty-msg">Questa pagina non è ancora disponibile nell\'archivio locale.</div></div>';
  var unavailableFooter =
    '<div class="n-page-footer"><div class="n-page-footer-gems">✦ &nbsp; ✦ &nbsp; ✦</div>' +
    '<div class="n-page-footer-text">Archivi di Arcamis</div></div>';
  pbody.innerHTML =
    '<div class="nc" style="animation:fi .22s ease forwards">' + unavailableHtml + unavailableFooter + '</div>';
  if (typeof afterPageRender === 'function') afterPageRender();
  return;
}
  ...
  function groupClasses(txt) {
    ...
    txt.split('\n').forEach(function (line) {
      ...
      var clean = item
        ...
        .trim();
      var isBase =
        /^(Artificer|Barbarian|Bard|Cleric|Druid|Fighter|Monk|Paladin|Ranger|Rogue|Sorcerer|Warlock|Wizard)$/i.test(
          clean
        );
      if (isBase) {
        currentClass = clean;
  ...
  sections.forEach(function (sec) {
    ...
    subsections.forEach(function (sub) {
      ...
      parseItems(subLines.join('\n')).forEach(function (it) {
        allItems.push({ type: 'item', data: it });
      });
    });
    if (allItems.length === 0) {
      parseItems(body).forEach(function (it) {
        allItems.push({ type: 'item', data: it });
      });
    }
    ...
    if (/^(Specie|Talenti|Spell)$/i.test(sectionTitle)) {
      ...
      callouts.forEach(function (c) {
        html += '<div class="mat-callout">💡 ' + c + '</div>';
      });
      html += '</div>';
      return;
    }
    ...
    if (/^Classi/i.test(sectionTitle)) {
      ...
      Object.keys(grouped).forEach(function (cls) {
        ...
        g.subs.forEach(function (s) {
          var source = '';
          var name = s.replace(/\s*\(([A-Z0-9' ]+)\)\s*$/, function (_, src) {
            source = src.trim();
            return '';
          });
          html +=
            '<div class="mat-sub-card">' +
            '<span class="mat-sub-name">' +
            name.replace(/\*\*/g, '').trim() +
            '</span>' +
...
function _renderRegole(md) {
  ...
  if (briefRules.length) {
    ...
    briefRules.forEach(function (rule, i) {
      html +=
        '<div class="regle-brief-card">' +
        '<span class="regle-brief-num">' +
        (i + 1) +
        '</span>' +
        '<span class="regle-brief-text">' +
        rule +
        '</span>' +
        '</div>';
    });
  ...
  if (subpageLinks.length) {
    ...
    subpageLinks.forEach(function (title) {
      html +=
        '<div class="regle-link-card">' +
        '<span class="regle-link-icon">📄</span>' +
        '<span class="regle-link-title">' +
        title +
        '</span>' +
        '</div>';
    });
...
function _renderLore(md, title, icon) {
  if (!md || md.trim().length < 5) {
    return (
      '<div class="lore-page"><div class="lore-empty">' +
...
function _renderLavoro(md) {
  ...
  blocks.forEach(function (block) {
    ...
    lines.forEach(function (line) {
      ...
      if (subM && !subtitle) {
        subtitle = subM[2].trim() || subM[1].trim();
      }
...
function _panIcon(d, coll) {
  return _PAN_ICONS[d.slug] || (coll && coll !== 'pantheon' && coll !== 'lavori' ? '🗂️' : '🛐');
}
function _jsStr(s) {
...
function _renderDeityPage(d, coll) {
  ...
  var hero =
    ...
    '<div class="pan-deity-caption"><span class="pd-name">' +
    _siteEsc(d.n) +
    '</span>' +
  ...
  sections.forEach(function (sec) {
    ...
    if (fields.length) {
      ...
      fields.forEach(function (f) {
        html +=
          '<div class="bst-field"><span class="bst-key">' +
          f.key +
          '</span><span class="bst-val">' +
          f.val +
          '</span></div>';
      });
      html += '</div>';
...
function _renderTimeline(md) {
  ...
  sections.forEach(function (sec) {
    ...
    body.split('\n').forEach(function (line) {
      ...
      if (sm) {
        currentSub = sm[1];
        items.push({ type: 'sub', text: currentSub });
        return;
      }
      var im = line.match(/^-\s+\*\*(.+?):?\*\*\s*:?\s*(.+)/);
      if (im) {
        items.push({ type: 'event', title: im[1], desc: im[2], sub: currentSub });
        return;
      }
      var bm = line.match(/^- (.+)/);
      if (bm) {
        items.push({ type: 'event', title: '', desc: bm[1], sub: currentSub });
        return;
      }
...
function _renderFazioni(md) {
  ...
  sections.forEach(function (sec) {
    ...
    if (fields.length) {
      ...
      fields.forEach(function (f) {
        html +=
          '<div class="faz-field"><span class="faz-key">' +
          f.key +
          '</span><span class="faz-val">' +
          f.val +
          '</span></div>';
      });
      html += '</div>';
...
function _renderOggetti(md) {
  ...
  sections.forEach(function (sec) {
    ...
    if (fields.length) {
      ...
      fields.forEach(function (f) {
        html +=
          '<div class="obj-field"><span class="obj-key">' +
          f.key +
          '</span><span class="obj-val">' +
          f.val +
          '</span></div>';
      });
      html += '</div>';
...
function _renderGalleria(md) {
  ...
  sections.forEach(function (sec) {
    ...
    body.split('\n').forEach(function (line) {
      ...
      var im = line.match(/^!\[([^\]]*)\]\(([^)]+)\)/);
      if (im && currentCat) {
        categories[currentCat].push({ alt: im[1], src: im[2] });
        return;
      }
      var lm = line.match(/^- (.+)/);
      if (lm && currentCat) {
        categories[currentCat].push({ alt: lm[1].split('—')[0].trim(), src: '', caption: lm[1] });
        return;
      }
    ...
    Object.keys(categories).forEach(function (cat) {
      ...
      categories[cat].forEach(function (img) {
        if (img.src) {
          html +=
            '<div class="gal-item"><img src="' +
            img.src +
            '" alt="' +
            img.alt +
            '" loading="lazy" decoding="async">' +
            (img.alt ? '<div class="gal-caption">' + img.alt + '</div>' : '') +
            '</div>';
        } else if (img.caption) {
...
function _renderSchede(md) {
  ...
  sections.forEach(function (sec) {
    ...
    if (fields.length) {
      ...
      fields.forEach(function (f) {
        html +=
          '<div class="sch-field"><span class="sch-key">' +
          f.key +
          '</span><span class="sch-val">' +
          f.val +
          '</span></div>';
      });
      html += '</div>';
