...
window.loadNovitaWidget = function (container) {
  ...
  container.innerHTML = '<div class="arc-news-loading">⏳ Caricamento novità...</div>';
  ...
  fetch('/content/recent.json')
    ...
    .then(function (data) {
      ...
      if (!pages.length) {
        container.innerHTML = '<div class="arc-news-loading">Nessuna novità recente.</div>';
        return;
      }

      var html =
        '<div class="arc-news-grid">' +
        pages
          .map(function (p) {
            var titleSafe = (p.title || '').replace(/'/g, "\\'").replace(/"/g, '&quot;');
            var iconSafe = p.icon || '📄';
            return (
              '<div class="arc-news-card" onclick="gp(\'' +
              p.id +
              "','" +
              titleSafe +
              "','" +
              iconSafe +
              '\')">' +
              '<div class="arc-news-icon">' +
              iconSafe +
              '</div>' +
              '<div class="arc-news-title">' +
              _escHtml(p.title) +
              '</div>' +
              '<div class="arc-news-date">' +
              _relativeDate(p.lastEdited) +
              '</div>' +
              '</div>'
            );
          })
    ...
    .catch(function (err) {
      container.innerHTML = '<div class="arc-news-loading">⚠️ Errore caricamento novità.</div>';
      console.error(err);
    });
};
...
.gs-card .arc-rbadge {
  ...
  z-index: 3;
      ...
      var selectors = ['.lcard', '.fpill', '.loc-card', '.loc-banner', '.cp-icard', '.gs-card'];
