...
/* ════ DATABASE LOCALI ════
   Ogni database Notion viene importato (offline) in un JSON locale sotto
   /content/databases/<key>.json con struttura { pages: [...] }.
   _DB_SOURCES mappa il vecchio id del database Notion alla chiave locale,
   così i contenuti già renderizzati continuano a funzionare senza API. */
var _DB_SOURCES = {
  '2fd0274fdc1c80038889fc072a360bae': 'galleria',
  '3320274fdc1c805090becb2a5a0414e1': 'npc',
  '2f60274fdc1c80fba671c588ba93b116': 'specie',
  '3350274fdc1c808fba5ed9ad1f3b4bb4': 'specie',
  '2fc0274fdc1c800f8ac0d6d03b255cad': 'timeline',
  '2f70274fdc1c80e3bdc7f95f81eb9cc0': 'sottoclassi',
  '2ff0274fdc1c807ea473db02ac4ae391': 'sottoclassi',
  '3040274fdc1c80e0a0dccfa9761bff55': 'biblioteca',
  '3400274fdc1c80178db3dcf6ba7098aa': 'changelog',
};
function _dbKey(dbId) {
  return _DB_SOURCES[dbId] || '';
}
function _dbKindOf(dbId) {
  var k = _dbKey(dbId);
  if (k === 'galleria') return 'galleria';
  if (k === 'npc') return 'npc';
  if (k === 'specie') return 'specie';
  if (k === 'timeline') return 'timeline';
  if (k === 'sottoclassi') return 'sottoclassi';
  if (k === 'biblioteca') return 'biblioteca';
  if (k === 'changelog') return 'changelog';
  return '';
}
/* Carica un database locale. Se la chiave non è registrata o il fetch
   fallisce, risolve a pagine vuote (il renderer mostra l'empty state). */
function _loadLocalDb(dbId) {
  var key = _dbKey(dbId);
  if (!key) return Promise.resolve({ pages: [] });
  return fetch('/content/databases/' + key + '.json')
    .then(function (r) {
      if (!r.ok) throw new Error('HTTP ' + r.status);
      return r.json();
    })
    .then(function (j) {
      return { pages: j.pages || j || [] };
    })
    .catch(function () {
      return { pages: [] };
    });
...
function rt(arr) {
  ...
  return arr
    .map(function (r) {
      ...
      var a = r.annotations || {},
        cls = [];
      if (a.bold) cls.push('rb');
      if (a.italic) cls.push('ri');
      if (a.strikethrough) cls.push('rs');
      if (a.underline) cls.push('ru');
      if (a.code) cls.push('rc');
      ...
      if (r.href) {
        ...
        if (m) {
          var ni = pages.find(function (n) {
            return n.id === m[1];
          });
          return (
            '<a class="rl" onclick="gp(\'' +
            m[1] +
            "','" +
            (ni ? ni.l.replace(/'/g, "\\'") : 'Pagina') +
            "','" +
            (ni ? ni.i : '📄') +
            '\')">' +
            inner +
...
function calloutColor(ic) {
  ...
  if (['⚠️', '🔥', '⚡', '🗡️', '⚔️', '💀'].indexOf(ic) > -1)
    return { bg: 'rgba(180,50,40,.07)', c: 'rgba(210,70,55,.6)' };
  if (['🌊', '💧', '🌙', '⭐', '✨', '🌌'].indexOf(ic) > -1)
    return { bg: 'rgba(40,80,180,.07)', c: 'rgba(70,110,220,.6)' };
...
function _scrubHtmlString(html) {
  ...
  html = html.replace(/\son\w+\s*=\s*(("[^"]*")|('[^']*')|[^\s>]+)/gi, function (m) {
    ...
    if (
      name === 'error' &&
      (val.indexOf('this.parentElement.style.display') > -1 ||
        /this\.parent(?:Element|Node)\.classList\.add\(/.test(val))
    )
      return m;
    ...
    switch (b.type) {
      ...
      case 'heading_1':
        if (d.is_toggleable && b.children) {
          h +=
            '<details class="n-toggle n-th"><summary class="n-h1 n-tsum">' +
            rt(d.rich_text) +
            '</summary>' +
            '<div class="n-tc">' +
          ...
          h += '<h2 class="n-h1 rr">' + rt(d.rich_text) + '</h2>';
        }
        break;
      ...
      case 'heading_2':
        if (d.is_toggleable && b.children) {
          h +=
            '<details class="n-toggle n-th"><summary class="n-h2 n-tsum">' +
            rt(d.rich_text) +
            '</summary>' +
            '<div class="n-tc">' +
            renderBlocks(b.children) +
          ...
          h += '<h3 class="n-h2 rr">' + rt(d.rich_text) + '</h3>';
        }
        break;
      ...
      case 'heading_3':
        if (d.is_toggleable && b.children) {
          h +=
            '<details class="n-toggle n-th"><summary class="n-h3 n-tsum">' +
            rt(d.rich_text) +
            '</summary>' +
            '<div class="n-tc">' +
            renderBlocks(b.children) +
          ...
          h += '<h4 class="n-h3 rr">' + rt(d.rich_text) + '</h4>';
        }
        break;
      ...
      case 'bulleted_list_item':
        h +=
          '<ul class="n-ul"><li>' +
          rt(d.rich_text) +
          (b.children ? '<div class="n-children">' + renderBlocks(b.children) + '</div>' : '') +
          '</li></ul>';
        break;
      ...
      case 'callout':
        ...
        if (isIntro) {
          h +=
            '<div class="n-intro" style="border-left-color:' +
            cc.c +
            ';background:' +
            cc.bg +
            '">' +
            ...
            ic +
            '</div>' +
            '<div class="n-intro-body">' +
            rt(d.rich_text) +
            (b.children ? renderBlocks(b.children) : '') +
            '</div>' +
            '</div>';
        } else {
          h +=
            '<div class="n-callout" style="--callout-bg:' +
            cc.bg +
            ';--callout-c:' +
            cc.c +
            '">' +
            '<div class="n-callout-icon">' +
            ic +
            '</div>' +
            '<div class="n-callout-body">' +
            rt(d.rich_text) +
            (b.children ? renderBlocks(b.children) : '') +
            '</div></div>';
        }
      ...
      case 'toggle':
        h +=
          '<details class="n-toggle"><summary class="n-ts">' +
          rt(d.rich_text) +
          '</summary>' +
          '<div class="n-tc">' +
          (b.children ? renderBlocks(b.children) : '') +
      ...
      case 'code':
        ...
        h +=
          '<pre class="n-code">' +
          (lang ? '<div class="n-code-lang">' + lang + '</div>' : '') +
          '<code>' +
          (d.rich_text || [])
            .map(function (r) {
              return r.plain_text;
      ...
      case 'image':
        ...
        if (src) {
          h +=
            '<figure class="n-image">' +
            '<img src="' +
            src +
            '" loading="lazy" decoding="async" class="n-zoomable" onclick="arcZoom(\'' +
            src +
      ...
      case 'video':
        ...
        if (vsrc) {
          ...
          if (ytm) {
            h +=
              '<div class="n-video"><iframe src="https://www.youtube.com/embed/' +
              ytm[1] +
              '" allowfullscreen loading="lazy"></iframe></div>';
          } else if (vim) {
            h +=
              '<div class="n-video"><iframe src="https://player.vimeo.com/video/' +
              vim[1] +
              '" allowfullscreen loading="lazy"></iframe></div>';
          } else {
      ...
      case 'pdf':
        ...
        if (_pu) {
          ...
          h +=
            '<a href="' +
            _pu +
            '" target="_blank" rel="noopener" class="n-pdf">' +
            '<div class="n-pdf-icon">📄</div>' +
            '<div><div class="n-pdf-label">' +
            pcap +
            '</div>' +
      ...
      case 'link_preview':
        ...
        if (_lpu) {
          h +=
            '<a href="' +
            _lpu +
            '" target="_blank" rel="noopener" class="n-bkm">' +
            '<div class="n-bkm-body"><div class="n-bkm-label">' +
            safeHost(lpu) +
            '</div>' +
            '<div class="n-bkm-url">' +
            lpu +
      ...
      case 'synced_block':
        if (b.children && b.children.length) {
          h += renderBlocks(b.children, false);
        }
        break;
      ...
      case 'child_page':
        ...
        while (i < blocks.length && blocks[i].type === 'child_page') {
          ...
          var cpni = pages.find(function (n) {
            return n.id === cpid;
          });
          ...
          if (cpCoverSafe) {
            cpCards +=
              ...
              cpicon +
              '\')">' +
              '<div class="cp-icard-accent" style="background:linear-gradient(90deg,' +
              cpacc.c +
              ',transparent)"></div>' +
              '<div class="cp-icard-icon" style="color:' +
              cpacc.c +
              '">' +
              cpicon +
              '</div>' +
              '<div class="cp-icard-title">' +
              cptitle +
              '</div>' +
              '<div class="cp-icard-sub" style="color:' +
              cpacc.c +
              '">Apri \u2192</div>' +
              '<div class="cp-icard-arr" style="color:' +
              cpacc.c +
              '">\u25c6</div>' +
              '</div>';
          } else {
            cpCards +=
              '<div class="loc-banner" style="' +
              cpbg +
              '" onclick="gp(\'' +
              cpid +
              "','" +
              ...
              cpicon +
              '\')">' +
              '<div class="loc-banner-accent" style="background:' +
              cpacc.c +
              '"></div>' +
              '<div class="loc-banner-icon" style="color:' +
              cpacc.c +
              '">' +
              cpicon +
              '</div>' +
              ...
              cptitle +
              '</div>' +
              '<div class="loc-banner-sub" style="color:' +
              cpacc.c +
              '">Apri \u2192</div>' +
              '</div>' +
              '<div class="loc-banner-arr" style="color:' +
              cpacc.c +
              '">\u25c6</div>' +
              '</div>';
          }
      ...
      case 'child_database':
        ...
        h +=
          '<div class="n-db-wrap" data-db="' +
          dbRawId +
          '">' +
          (d.title ? '<div class="n-db-title">' + d.title + '</div>' : '') +
          '<div class="n-db-grid" id="db-' +
          dbRawId +
          '"><div class="n-db-loading">⏳ Caricamento...</div></div></div>';
        break;

      case 'table':
        var rows = b.children || [],
          hasH = d.has_column_header,
          hasR = d.has_row_header;
      ...
      case 'column_list':
        ...
        cols.forEach(function (col) {
          h += '<div>' + (col.children ? renderBlocks(col.children) : '') + '</div>';
        });
        h += '</div>';
        break;
      ...
      case 'to_do':
        h +=
          '<div class="n-todo"><span class="n-todo-box">' +
          (d.checked ? '✅' : '☐') +
          '</span>' +
          '<div>' +
          rt(d.rich_text) +
      ...
      case 'bookmark':
        ...
        h +=
          ...
          '" target="_blank" rel="noopener" class="n-bkm">' +
          '<div class="n-bkm-body"><div class="n-bkm-label">' +
          (d.caption && d.caption.length ? rt(d.caption) : safeHost(bu)) +
          '</div>' +
          '<div class="n-bkm-url">' +
          bu +
          '</div></div>' +
...
function _dbArrows(wrap, idx, maxIdx) {
  var prev = wrap.querySelector('.la-prev'),
    next = wrap.querySelector('.la-next');
  if (prev) {
    prev.style.opacity = idx <= 0 ? '0.2' : '1';
    prev.style.pointerEvents = idx <= 0 ? 'none' : 'auto';
  }
  if (next) {
    next.style.opacity = idx >= maxIdx ? '0.2' : '1';
    next.style.pointerEvents = idx >= maxIdx ? 'none' : 'auto';
  }
}
...
async function _loadSingleDb(grid) {
  ...
  try {
    var data = await _loadLocalDb(dbId);
    if (!data.pages || !data.pages.length) {
      grid.innerHTML = '<div class="n-db-loading">Nessun elemento locale disponibile.</div>';
      return;
    }

    var _kind = _dbKindOf(dbId);
    ...
    if (_kind === 'specie') {
      ...
      var gruppi = Object.keys(grouped).sort(function (a, b) {
        return a.localeCompare(b, 'it');
      });
      gruppi.forEach(function (gr) {
        grouped[gr].sort(function (a, b) {
          return a.title.localeCompare(b.title, 'it');
        });
      });

      var sidebarItems = gruppi
        .map(function (gr) {
          return (
            '<li class="sp-class-item" data-specie="' +
            gr +
            '" onclick="spSelectGroup(this,\'' +
            gr +
            '\')">' +
            gr +
            '</li>'
      ...
      var uid = 'dblc-' + dbId;
      if (wrap) {
        wrap.outerHTML = '<div class="n-db-lc-wrap" id="' + uid + '">' + specieHtml + '</div>';
      } else {
        grid.outerHTML = '<div>' + specieHtml + '</div>';
      }
    ...
    if (_kind === 'changelog') {
      ...
      var uid = 'cl-' + dbId;
      if (wrap) {
        wrap.outerHTML = '<div class="n-db-lc-wrap pb-wide-trigger" id="' + uid + '"></div>';
      }
      var clContainer = document.getElementById(uid);
      if (clContainer && window.loadChangelog) {
        window.loadChangelog(clContainer);
      }
      return;
    }
    /* ── BIBLIOTECA ── */
    if (_kind === 'biblioteca') {
      ...
      var uid = 'lib-' + dbId;
      if (wrap) {
        wrap.outerHTML = '<div class="hb-library-container pb-wide-trigger" id="' + uid + '"></div>';
      }
      var libContainer = document.getElementById(uid);
      if (libContainer && window.loadLibraryGallery) {
        window.loadLibraryGallery(libContainer, data.pages);
      }
      return;
    }
    /* ── GALLERIA PG ── */
    if (_kind === 'galleria') {
      ...
      var uid = 'gs-' + dbId;
      if (wrap) {
        wrap.outerHTML = '<div class="gs-container pb-wide-trigger" id="' + uid + '"></div>';
      }
      var gsContainer = document.getElementById(uid);
      if (gsContainer && window.loadGallery) {
        window.loadGallery(gsContainer, data.pages);
      }
      return;
    }
    /* ── NPC ── */
    if (_kind === 'npc') {
      ...
      var uid = 'npc-' + dbId;
      if (wrap) {
        wrap.outerHTML = '<div class="npc-gallery-container pb-wide-trigger" id="' + uid + '"></div>';
      }
      var npcContainer = document.getElementById(uid);
      if (npcContainer && window.loadNpcGallery) {
        window.loadNpcGallery(npcContainer, data.pages);
      }
      return;
    }

    /* ── CAROUSEL GENERICO ── */
    var allBanners = data.pages.every(function (p) {
      return !safeCoverUrl(p.cover);
    });
    var cardsHtml = data.pages
      .map(function (p) {
        var icon = p.icon || '📄';
        ...
        if (coverSafe) {
          return (
            '<div class="loc-card" style="background-image:url(\"' +
            coverSafe +
            '\");background-size:cover;background-position:center" onclick="gp(\'' +
            p.id +
            "','" +
            title +
            "','" +
            ...
            '\')">' +
            '<div class="loc-ov">' +
            '<div class="loc-badge explored">' +
            icon +
            '</div>' +
            '<div class="loc-name">' +
            p.title +
            '</div>' +
            '<div class="loc-sub" style="color:' +
            acc.c +
            '">Scopri →</div>' +
    ...
    if (allBanners) {
      if (wrap) {
        wrap.outerHTML =
          '<div class="n-db-lc-wrap" id="' +
          uid +
          '">' +
          titleHtml +
          '<div class="loc-banner-grid">' +
          cardsHtml +
          '</div></div>';
      } else {
      ...
      if (wrap) {
        wrap.outerHTML =
          ...
          '<div class="loc-track" data-idx="0" style="gap:16px">' +
          cardsHtml +
          '</div>' +
          '</div>' +
...
/* Carica il contenuto di una pagina locale (markdown) e lo rende HTML.
   Le pagine importate da Notion in /content/pages/<key>.json usano lo stesso
   renderer del sito (md-render) o il fallback markdown di notion-nav.js. */
function _loadLocalPage(id) {
  var key = String(id || '').replace(/^pag-/, '');
  return fetch('/content/pages/' + key + '.json')
    ...
    .then(function (j) {
      ...
      var html = '';
      try {
        if (md && typeof window.mdRender === 'function') html = window.mdRender(md);
      } catch (e) {}
      if (!html && typeof _mdToHtml === 'function') {
        try {
          html = _mdToHtml(md);
        } catch (e) {}
      }
...
window.spSelect = function (el, id, title, icon) {
  document.querySelectorAll('.sp-item').forEach(function (i) {
  ...
  _loadLocalPage(id)
    .then(function (html) {
      content.innerHTML = _scrubHtmlString(
        '<div class="n-body">' +
    ...
    .catch(function () {
      content.innerHTML =
        '<div style="color:rgba(200,155,60,.4);font-family:Cinzel,serif;font-size:11px;padding:40px;text-align:center">Errore caricamento</div>';
    });
};
window.spSelectGroup = function (el, gruppo) {
  ...
  tabsEl.innerHTML = _scrubHtmlString(
    sottospecie
      .map(function (p) {
        var titleSafe = p.title
          .replace(/&/g, '&amp;')
          .replace(/"/g, '&quot;')
          .replace(/</g, '&lt;')
          .replace(/>/g, '&gt;')
          .replace(/'/g, "\\'");
        return (
          '<div class="sp-tab" onclick="spSelectTab(this,\'' +
          p.id +
          "','" +
          titleSafe +
          "','" +
  ...
  // Prefetch silenziosa dei primi 3 elementi (possono essere pagine locali)
  sottospecie.slice(0, 3).forEach(function (p) {
    if (typeof window.prefetchPage === 'function') window.prefetchPage(p.id);
  });
