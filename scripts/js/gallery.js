...
function renderGallery(container, pages) {
  ...
  allTags.forEach(function (t) {
    filtersHtml +=
      '<div class="gs-filter-btn" data-filter="' + t + '" onclick="gsFilter(this,\'' + t + '\')">' + t + '</div>';
  });
  filtersHtml += '</div>';
  ...
  pages.forEach(function (p) {
    ...
    gridHtml +=
      ...
      '\')">' +
      (cover
        ? '<div class="gs-card-bg" style="background-image:url(\'' + cover + '\')"></div>'
        : '<div class="gs-card-bg" style="background:' + col.bg + '"></div>') +
...
window.gsSelect = function (id, name) {
  ...
  document.querySelectorAll('.gs-card').forEach(function (c) {
  ...
  detail.innerHTML =
    ...
    (cover
      ? '<div class="gs-detail-cover-img" style="background-image:url(\'' + cover + '\')"></div>'
      : '<div class="gs-detail-cover-img" style="background:' + col.bg + '"></div>') +
    ...
    '<div class="gs-detail-btn" onclick="gsOpenPage(\'' +
    id +
    '\')">⚔ Apri scheda personaggio</div>' +
    '</div>' +
    '</div>';
...
window.gsFilter = function (btn, filter) {
  document.querySelectorAll('.gs-filter-btn').forEach(function (b) {
...
window.loadGallery = function (container, pages) {
  container.innerHTML =
    '<div class="gs-loading"><div class="gs-loading-spin"></div><span>Caricamento eroi...</span></div>';

  var data = Array.isArray(pages) && pages.length ? { pages: pages } : null;
  if (!data) {
    container.innerHTML = '<div class="gs-loading">Nessun eroe registrato nella galleria locale.</div>';
    return;
  }
