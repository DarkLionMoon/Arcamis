...
(function () {
  ...
  function arcPauseCarousel() {
    ...
    if (bar && !document.getElementById('arc-carousel-paused-badge')) {
      ...
      badge.style.cssText =
        'padding:2px 10px;background:rgba(200,155,60,.15);border:1px solid rgba(200,155,60,.3);color:rgba(200,155,60,.8);font-size:7px;letter-spacing:.1em;';
  ...
  async function arcOpenSlideEditor(idx, covers) {
    ...
    var meta = {};
    try {
      meta = JSON.parse(covers[sl.key + '_meta'] || '{}');
    } catch (e) {}
    var btns = [];
    try {
      btns = JSON.parse(covers[sl.key + '_btns'] || '[]');
    } catch (e) {}
    ...
    var btnsHtml = sl.btns
      .map(function (b, bi) {
        var sv = btns[bi] || {};
        return (
          '<div class="ap-section-label" style="margin-top:8px">Bottone ' +
          (bi + 1) +
          '</div>' +
          '<div class="ap-field"><div class="ap-label">Testo</div>' +
          '<input class="ap-input" id="asl-btl-' +
          idx +
          '-' +
          bi +
          '" value="' +
          escH(sv.label || b.defLabel) +
          '" placeholder="Testo…"/></div>' +
          '<div class="ap-field"><div class="ap-label">Link / onclick</div>' +
          '<input class="ap-input" id="asl-bth-' +
          idx +
          '-' +
          bi +
          '" value="' +
          escH(sv.href || b.defHref) +
          '" placeholder="https://… o onclick…"/></div>'
        );
      })
      .join('');
  ...
  window.arcSaveSlideImg = async function (idx) {
    ...
    if (!val) {
      arcToast('Nessuna immagine da salvare', false);
      return;
    }
  ...
  window.arcRemoveSlideImg = async function (idx) {
    ...
    if (prev) {
      ...
      prev.textContent = 'Nessuna immagine';
    }
  ...
  async function arcSaveSlideImgVal(idx, val) {
    ...
    var slide = document.querySelectorAll('.slide')[idx];
    if (slide && ok) {
      slide.style.backgroundImage = "url('" + val + "')";
      slide.style.backgroundSize = 'cover';
      slide.style.backgroundPosition = 'center 40%';
    }
  ...
  function injectPageCarouselButtons() {
    document.querySelectorAll('#pbody .arc-edit-btn').forEach(function (b) {
      b.remove();
    });
    ...
    document.querySelectorAll('#pbody .n-db-card').forEach(function (card, ci) {
      ...
      btn.onclick = function (e) {
        e.stopPropagation();
        arcOpenSingleCardEditor(card, ci);
      };
      card.appendChild(btn);
    });
  }
  ...
  function arcOpenLocCardEditor(card, pageId) {
    ...
    var html =
      '<div class="ap-section-label" style="margin-bottom:8px">' +
      name +
      '</div>' +
      '<div class="ap-preview" id="aploc-prev" style="' +
      (cur ? "background-image:url('" + cur + "')" : '') +
      '">' +
      (cur ? '' : 'Nessuna immagine') +
      '</div>' +
      '<div class="ap-upload"><input type="file" accept="image/*" onchange="arcLocCardUpload(event,\'' +
      pageId +
      '\')"/>' +
      '<div class="ap-upload-txt">🖼 Carica dal PC</div></div>' +
      '<div class="ap-or">oppure URL</div>' +
      '<input class="ap-input" id="aploc-url" type="url" value="' +
      escH(cur) +
      '" placeholder="https://…" ' +
      "oninput=\"document.getElementById('aploc-prev').style.backgroundImage=this.value?'url('+this.value+')':''\" />" +
      '<div class="ap-actions">' +
      '<button class="ap-btn-save" onclick="arcSaveLocCard(\'' +
      pageId +
      '\')">Salva</button>' +
      '<button class="ap-btn-save" style="flex:0;padding:9px 12px;background:rgba(180,40,30,.1);border-color:rgba(180,40,30,.3);color:#ff8888" onclick="arcRemoveLocCard(\'' +
      pageId +
      '\')">✕</button>' +
      '</div>' +
  ...
  window.arcSaveLocCard = async function (pageId) {
    ...
    if (!val) {
      arcToast('Inserisci URL o carica file', false);
      return;
    }
  ...
  async function arcSaveLocCardVal(pageId, val) {
    ...
    if (ok && window._arcEditLocCard) {
      ...
      window._arcEditLocCard.setAttribute(
        'style',
        s + "background-image:url('" + val + "');background-size:cover;background-position:center;"
      );
    }
  ...
  function arcOpenGridEditor(gridEl, gi) {
    ...
    var html = Array.from(cards)
      .map(function (card, ci) {
        ...
        return (
          '<div class="ap-section-label" style="margin-top:' +
          (ci ? '12px' : '0') +
          'px">' +
          name +
          '</div>' +
          '<div class="ap-preview" id="apg-prev-' +
          gi +
          '-' +
          ci +
          '" style="' +
          ...
          (cur ? '' : 'Nessuna') +
          '</div>' +
          '<div class="ap-upload"><input type="file" accept="image/*" onchange="arcGridCardUpload(event,\'' +
          key +
          "'," +
          gi +
          ',' +
          ...
          '<div class="ap-or">oppure URL</div>' +
          '<input class="ap-input" id="apg-url-' +
          gi +
          '-' +
          ci +
          '" type="url" value="' +
          escH(cur) +
          '" placeholder="https://…" ' +
          'oninput="document.getElementById(\'apg-prev-' +
          gi +
          '-' +
          ci +
          "').style.backgroundImage=this.value?'url('+this.value+')':''\" />" +
          '<div class="ap-actions"><button class="ap-btn-save" onclick="arcSaveGridCard(\'' +
          key +
          "'," +
          gi +
          ',' +
          ci +
          ')">Salva</button></div>' +
          '<div class="ap-status" id="apg-status-' +
          gi +
          '-' +
          ci +
          '"></div>' +
    ...
    arcOpenPanel(
      'Griglia immagini',
      html ||
        '<div style="color:rgba(200,155,60,.4);font-style:italic;font-size:13px;padding:20px 0">Nessuna card trovata.</div>'
    );
  ...
  function arcOpenSingleCardEditor(card, ci) {
    ...
    var html =
      '<div class="ap-preview" id="aps-prev-' +
      ci +
      '" style="' +
      (cur ? "background-image:url('" + cur + "')" : '') +
      '">' +
      (cur ? '' : 'Nessuna') +
      '</div>' +
      '<div class="ap-upload"><input type="file" accept="image/*" onchange="arcSingleCardUpload(event,\'' +
      key +
      "'," +
      ci +
      ')"/>' +
      '<div class="ap-upload-txt">🖼 Carica</div></div>' +
      '<div class="ap-or">oppure URL</div>' +
      '<input class="ap-input" id="aps-url-' +
      ci +
      '" type="url" value="' +
      escH(cur) +
      '" placeholder="https://…" ' +
      'oninput="document.getElementById(\'aps-prev-' +
      ci +
      "').style.backgroundImage=this.value?'url('+this.value+')':''\" />" +
      '<div class="ap-actions"><button class="ap-btn-save" onclick="arcSaveSingleCard(\'' +
      key +
      "'," +
      ci +
      ')">Salva</button></div>' +
      '<div class="ap-status" id="aps-status-' +
      ci +
      '"></div>';
  ...
  function arcOpenGalleryCardEditor(card, pageId) {
    ...
    var html =
      ...
      name +
      '</div>' +
      '<div class="ap-preview" id="apgal-prev" style="' +
      (cur ? "background-image:url('" + cur + "')" : '') +
      '">' +
      (cur ? '' : 'Nessuna immagine') +
      '</div>' +
      '<div class="ap-upload"><input type="file" accept="image/*" onchange="arcGalleryCardUpload(event,\'' +
      pageId +
      '\')"/>' +
      '<div class="ap-upload-txt">🖼 Carica dal PC</div></div>' +
      '<div class="ap-or">oppure URL</div>' +
      '<input class="ap-input" id="apgal-url" type="url" value="' +
      escH(cur) +
      '" placeholder="https://…" ' +
      "oninput=\"document.getElementById('apgal-prev').style.backgroundImage=this.value?'url('+this.value+')':''\" />" +
      '<div class="ap-actions">' +
      '<button class="ap-btn-save" onclick="arcSaveGalleryCard(\'' +
      pageId +
      '\')">Salva</button>' +
      '<button class="ap-btn-save" style="flex:0;padding:9px 12px;background:rgba(180,40,30,.1);border-color:rgba(180,40,30,.3);color:#ff8888" onclick="arcRemoveGalleryCard(\'' +
      pageId +
      '\')">✕</button>' +
      '</div>' +
      '<div class="ap-sep"></div>' +
      ...
      '<input type="range" id="apgal-posx" min="0" max="100" value="' +
      escH(curPosX) +
      '" style="width:100%;accent-color:var(--gold,#c89b3c)" oninput="arcGalleryPosPreview(\'' +
      pageId +
      '\')"/>' +
      '<div class="ap-label" style="margin-top:8px">Verticale</div>' +
      '<input type="range" id="apgal-posy" min="0" max="100" value="' +
      escH(curPosY) +
      '" style="width:100%;accent-color:var(--gold,#c89b3c)" oninput="arcGalleryPosPreview(\'' +
      pageId +
      '\')"/>' +
      '<div style="font-family:Cinzel,serif;font-size:9px;color:rgba(200,155,60,.6);margin-top:4px" id="apgal-pos-label">' +
      curPosX +
      '% ' +
      curPosY +
      '%</div>' +
      '</div>' +
      '<div class="ap-actions">' +
      '<button class="ap-btn-save" onclick="arcSaveGalleryPos(\'' +
      pageId +
      '\')">Salva posizione</button>' +
      '</div>' +
  ...
  if (_origGp)
    window.gp = function (id, label, icon, fromPop) {
