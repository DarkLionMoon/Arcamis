...
*/
import { constantTimeEqual, parseCookie } from './_lib/auth.js';
...
export async function onRequest(context) {
  ...
  if (token && KV) {
    ...
    try {
      const stored = await KV.get('admin_session_' + token);
      if (stored && stored !== 'valid' && csrfHeader) {
        const session = JSON.parse(stored);
        csrfValid = !!(session && session.csrf && constantTimeEqual(session.csrf, csrfHeader));
      }
    } catch (_) {}
  }
  ...
  if (request.method === 'POST') {
    ...
    return new Response(JSON.stringify({ error: 'Trigger deploy non supportato da questo endpoint' }), {
      status: 405,
      headers: cors,
    });
  }
